...
- **위키 참조 — 교차 프로젝트 검색 (llm-wiki 사용 중이고 이 프로젝트가 등록돼 있으면)**: `pjc:llm-wiki`의 절차 K(read-only 조회)로 **같은 증상·에러를 다른 프로젝트에서 이미 해결한 적이 있는지** index·grep으로 찾는다(전체 정독이 아니라 관련 항목만 식별·조회). 이 프로젝트의 과거 버그만이 아니라 **다른 프로젝트의 해결 기록까지** 본다 (A 프로젝트에서 고친 버그가 B 프로젝트에 재발하는 경우가 흔하다):
  ...
  - 발견하면 그 **원인·해결책을 가설에 반영**하되, **그대로 복사하지 않고 이 프로젝트(B)의 코드 구조에 맞게 재구현**한다 (프로젝트마다 맥락이 다름). 위키는 읽기만 한다 (갱신은 별도 세션). 위키가 없거나 미등록이면 건너뛴다.
