...
- **대상 repo 의 `intent/` 폴더에 둔다** — `plan.md` 와 같은 레포이고, 코드와 같은 커밋 이력에 남아 「그 요구로 이 diff 가 나왔다」가 한 곳에서 추적된다.
...
- **`pjc:plan` Step 6 에서 사용자 승인을 받은 직후 커밋한다** — 승인 시점을 지나 적으면 그 뒤의 실측이 요구에 섞여 「무엇을 승인했는가」가 흐려진다. `Status:` 를 `approved` 로 바꾸고 `설정: intent — <제목>` 제목으로 **별도 로컬 커밋** 하나를 만든다.
...
- **그 커밋이 그 회차의 BASE 다** — SHA 를 `plan.md` 의 `## Goal` 바로 아래에 `> BASE: <SHA>` 1줄로 적는다. 회차의 diff 범위(`git diff <BASE>..HEAD`)를 완료 리뷰와 다음 회차가 이 한 줄로 얻는다. `plan.md` 는 추적되지 않으므로 **이 SHA 가 없으면 회차 경계를 되찾을 방법이 커밋 제목 추측뿐**이다.
