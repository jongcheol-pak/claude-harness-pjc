...
에러 메시지·스택트레이스·예외 체인을 끝까지 읽고, 메시지의 식별자를 코드에서 확인한다.
...
- **위키 교차 검색**(vault 있을 때) — 같은 증상·에러를 다른 프로젝트가 이미 해결했는지 찾는다. **스킬을 발동하지 말고 `../llm-wiki/references/lookup-rules.md` 하나를 Read한다**(절차 K, read-only). vault 판정의 정본은 `../WIKI.md`이고, 검색 순서·재구현 원칙·`deprecated` 제외는 그 절차 파일이 정본이다.
...
가설이 둘 이상이면 각 가설의 관련 심볼·호출부·설정을 `grep`으로 직접 찾는다 — **가설의 검증·판정은 위임하지 않는다.**
...
근본 원인과 그것이 증상을 만드는 메커니즘(상관이 아니라 인과)을 설명할 수 있어야 통과한다.

### 통과 후 갈림 — 이 스킬이 고치는가

**원인 규명까지가 이 스킬의 몫이다.** 수정이 **단일 파일·소규모**(경량 경로와 같은 축)면 아래 Phase 4로 직접 간다. **다중 파일에 걸치거나 로직·시그니처를 바꾸거나 새 심볼을 만들면 `pjc:plan`으로 넘긴다** — 규명한 원인·증거·처방 후보를 그대로 넘기면 그쪽이 계획을 쓰고 `pjc:implement`가 실행한다. 그 규모의 변경을 여기서 밀면 계획 없이 여러 파일이 바뀌고, 그것은 `pjc:plan`의 발동 조건 자체다.

## Phase 4 — 수정 구현

**진입 조건은 둘이다** — 표준 경로는 Phase 1–3을 모두 통과한 뒤, **경량 경로는 1-A만 거친 뒤**(위 「경량 경로」가 열거한 조건을 전부 만족한 경우)다.
...
근본 원인에만 직접 대응한다. 무관한 리팩토링·서식 변경 금지. 범위 초과의 정지 조건은 아래 「Halt 조건」이 정본이다.

### 4-D. 검증

`pjc:implement`의 검증 절차를 따른다.
...
- **수정 중 변경 범위가 plan 의 task 범위 또는 사용자가 지목한 범위를 넘음** — Phase 4-B(최소 수정)의 정지 조건이다. plan 없이 단독 발동한 경로에는 `plan.md` 가 없으므로 사용자가 지목한 범위로 판정한다
