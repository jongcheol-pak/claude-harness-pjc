...
- **가설별 증거 위치 수집**: 가설이 둘 이상이면 각 가설의 관련 심볼·호출부·설정을 `grep`으로 직접 찾는다. 가설의 검증·판정은 위임하지 않는다.
### Phase 2 통과 조건
...
- 빌드 / 테스트 / 변경 파일에 해당하는 정적 검사
- **4-A에서 버그를 재현하던 회귀 테스트가 통과하는지 확인한다** — 이것이 이 수정의 유일한 직접 증거다
- 이번 수정이 건드린 심볼의 다른 사용처가 여전히 성립하는지 확인한다

- **수정마다 리뷰 서브에이전트를 부르지 않는다** — 회귀 테스트가 GREEN이면 그것이 판정이다. 리뷰가 필요한 규모(다중 파일·설계 변경)면 그것은 디버깅이 아니라 계획할 일이므로 `pjc:plan`으로 넘긴다.
