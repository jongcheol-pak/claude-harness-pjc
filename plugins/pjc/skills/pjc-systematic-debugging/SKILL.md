...
- **관련 기능의 feature 페이지 "관련 지식·레시피" 섹션**에 "해결한 문제: {증상} → {원인} → {해결}" 형태로 기록한다 (검색 키워드가 되도록 에러 메시지·증상을 명시). **일반화가 서면 첫 발견이어도 `30_knowledge/patterns/` 승격 대상**이다 — 판정 축은 *"원인·기전이 이 프로젝트의 스택·코드에 종속되는가"*이고 종속적이거나 애매하면 여기까지만 기록한다(판정 축·`confidence` 초기값의 정본은 llm-wiki 절차 B-2 2-1).
