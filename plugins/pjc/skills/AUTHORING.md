...
상충 시 우선순위: **글로벌/프로젝트 `CLAUDE.md` 절대·보안 규칙 > `DESIGN.md` > 기존 스킬의 고유 규칙 > 이 가이드.** 이 가이드는 그것들을 대체하지 않고 보완한다. **`DESIGN.md` 를 위에 두는 것은 그 문서가 이미 선언한 관계다** — 한쪽만 적으면 이 파일을 먼저 연 세션이 그 선언을 모른다.
...
- SKILL.md 본문: **500줄 미만 권장** — 초과하면 저빈도 상세를 `references/`로 분리(progressive disclosure). **근거가 두 문장을 넘으면 `references/`로 내린다**(`skills/DESIGN.md` 1절). 5,000토큰 권장은 `skills/DESIGN.md` 4절의 문서 예산 표가 대체한다 — SKILL.md 12,000 B가 그 값이다.
