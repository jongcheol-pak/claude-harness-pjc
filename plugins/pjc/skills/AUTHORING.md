...
| 공식 | 우리 | 왜 갈렸는가 |
|---|---|---|
| reference 파일이 **100줄**을 넘으면 목차 | `BUDGET.md`「나눈 뒤에 지킬 것」의 **10,000 B** | 줄 길이가 파일마다 33~82자로 갈려 줄 수는 분량을 재지 못한다. **예산 축이 재는 단위에 맞춘다** — v1.273.0 에 그 축이 바이트에서 **문자 수**로 옮겨 갔으므로 이 임계도 같은 단위로 재산정할 자리다(`[다음 회차]`). 종전 근거였던 *"한글이라 줄 수와 바이트가 3배로 갈린다"* 는 단위 전환으로 무효가 됐다 |
