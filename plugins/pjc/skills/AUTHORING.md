...
- SKILL.md 본문: **500줄 미만 권장** — 초과하면 저빈도 상세를 `references/`로 분리(progressive disclosure). **근거 서사(「왜 그 규칙인가」)를 옮길지는 `docs/harness-conventions.md`의 「rationale 이관 임계」가 정본**(항목 300자 · 파일 신설 1,200자 — 미달이면 옮기지 않는 것이 정상이다). 5,000토큰 권장은 `skills/DESIGN.md` 4절의 문서 예산 표가 대체한다 — SKILL.md 12,000 B가 그 값이다.
