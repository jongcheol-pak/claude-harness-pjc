...
<!-- 이번 제외 — 이번 plan에선 안 하지만 향후 진행할 작업.
     ...
     **각 항목은 대장 등재 판정 상태를 접두 마커로 담는다** — `- [등재]` / `- [미등재:<게이트 기호>]` / `- [미판정]`.
     계획 단계에서 적는 항목의 기본값은 `- [미판정]`이고, implement-task F-6.5가 게이트 판정 후 앞 둘 중 하나로 바꾼다.
     **어느 마커를 언제 쓰는지·왜 필요한지의 정본은 implement-task references/phase-f-detail.md의
     F-6.5 「판정 결과를 plan 항목에 마커로 남긴다」**이며 여기 복제하지 않는다(판정 규칙을 두 곳에 적으면 갈린다).
     아래 placeholder에는 마커를 달지 않는다 — 아직 항목이 아니라 자리표시자다(hook의 미판정 계수에서 제외된다). -->
...
<!-- plan이 도입하는 신규 심볼(함수/클래스/컴포넌트)마다 기존 유사 구현 검색 결과를 기록 (plan-feature Step 4-D).
     ...
     `검색어·스코프`·`hit` 열은 절대 규칙 2 「부재 판정」의 산출물이다 — 실제로 돌린 검색어를
     전부 적고(2개 이상: 이름 + 동작 키워드) 스코프와 hit 수를 남긴다. 이 열이 있어야
     plan-reviewer 항목 1이 그 검색을 직접 반증할 수 있다(검색 결과 칸만 있으면 "없음"이라는
     결론만 남아 검증할 대상이 없다).
     신규 심볼이 없는 plan(기존 코드 수정만)은 표 대신 "- 해당 없음 (신규 심볼 0개)" 한 줄. -->
| 신규 심볼 | 검색어·스코프 | hit | 유사 기존 구현 검색 결과 | 재사용/신규 사유 |
|---|---|---|---|---|
...
## Open Questions
<!-- 각 선택지 끝에 〔근본〕/〔우회〕 판정을 근거 1구와 함께 붙이고 ★은 〔근본〕에 준다.
     ★은 **질문당 정확히 1개** — 전부 〔근본〕이면 그중 최선 하나에만 주고 나머지를 고르지 않은
     이유를 근거에 적는다(전부에 붙이면 ★이 아무것도 가리키지 않는다)
     (plan-feature Step 8 「추천(★) 기준」). 답변을 반영하면 [x]로 바꾸되 ★이 어디였는지·
     그 판정(〔근본〕/〔우회〕)·사용자가 무엇을 골랐는지를 남긴다 — 답 요약만 남기고 ★을 지우면
     plan-reviewer 항목 2가 검사할 대상이 사라진다. 미해결 질문만 [ ]로 남긴다. -->
- [ ] Q1: <질문>
  - A) <선택지> 〔**근본**〕 <근거 1구>
  - B) <선택지> 〔**우회**〕 <근거 1구>
  - **★ = A** (판정: 근본). <추천 근거 1줄 — 복잡도·작업량 트레이드오프 병기>
  - **사용자 선택 = <A/B>** (<YYYY-MM-DD>)   <!-- 답변 후 [x]로 바꾸며 이 줄을 채운다 -->
```
