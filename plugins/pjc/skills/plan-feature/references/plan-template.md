...
<!-- 주석·docstring은 3분류 대상이 아니다 — 이번 diff가 건드린 코드에 딸린 것은 **언제나 필수**이며
     행으로 적을 필요 없이 해당 task에서 함께 고친다(implement-task 규칙 4-1). 3분류에 「선택」으로
     올리지 말 것 — plan-feature Step 4-E의 같은 규정이 정본이다. -->
