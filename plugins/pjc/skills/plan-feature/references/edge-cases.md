...
각 항목은 (i) 사전 해소(자율 루프가 그 지점에서 멈추지 않도록 plan에 결정을 박아 둠), (ii-a) 사전 승인 가능(비파괴 의존성/구조/스키마 등 알려진 승인 필요 항목 → `## 사전 승인 항목`에 등록, plan 승인 시 일괄 위임), 또는 (ii-b) 위임 불가 Halt 명시(파괴적·외부/비가역·인증정보 필요 신규 외부 서비스·돌발 결정 — `## 불가피한 Halt (위임 불가)`에 적고 사전결정으로 우회하지 않음) 중 하나로 적는다.
