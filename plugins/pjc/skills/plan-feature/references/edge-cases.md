...
> **Type 게이트 (plan-reviewer 항목 11과 정합)**: **Type C/D는 필수**. **Type A/B는 경감** — 파괴적 작업·새 의존성·외부/비가역·인증정보를 유발하는 시나리오가 있을 때만 명시하고, 없으면 "없음"으로 통과한다(순수 문서 Type A·trivial Type B는 대개 멈춤 시나리오가 없어 plan-reviewer도 항목 11을 A/B에 적용하지 않는다). 단 Type A/B라도 그런 시나리오가 하나라도 있으면 아래 (ii-a)/(ii-b)로 **반드시** 적는다.
