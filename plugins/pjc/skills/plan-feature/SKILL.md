...
- **예외 (도구 사용 불가)**: subagent 호출이 **기술적으로 불가**한 환경이면(세션 정책상 금지·도구 미제공·즉시 거부) `implement-task`의 `references/recovery.md` "Reviewer 호출 실패 대응" **B 분기**를 따른다 — `plan-reviewer.md`의 판정 항목을 체크리스트로 직접 대조 + 의무 3종(사용자 보고 · 기록 · 승인 프롬프트에 "검증 깊이 저하 — plan-reviewer 미실행" 명시). **이 예외는 아래 Type A/B 대체와 축이 다르다**: 아래는 **task 등급**에 따른 면제(Type C/D면 대체 불가)이고, 이것은 **도구 가용성**에 따른 것이라 Type C/D에서도 적용된다 — 즉 아래의 "Type C/D는 대체 불가"를 도구가 막힌 상황까지 금지하는 것으로 읽지 않는다(그렇게 읽으면 plan이 영원히 통과할 수 없다). 발동 조건이 애매하면 예외를 쓰지 말고 사용자에게 묻는다. **정책 금지(B-1)면 체크리스트 대체로 강등하기 전에 사용자에게 1회 승인을 구한다** — 승인되면 정상 호출로 Step 9를 정식 수행한다(대체는 승인이 안 될 때만).
