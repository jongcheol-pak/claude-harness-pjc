...
- **스킬 개선 큐 확인 (하네스 레포 세션에서만)**: 이 세션의 대상이 **하네스(플러그인) 레포**이면 — 판정은 `cwd` 아래 `plugins/pjc/.claude-plugin/plugin.json` 존재 — vault 루트 **`skill-feedback.md`**를 읽어 `[SKILL-IMPROVE]` 항목을 **할 일 후보**로 조회한다(`pjc:llm-wiki` 절차 K 5-1이 저장처·형식의 정본). 다른 프로젝트 세션에서는 조회하지 않는다 — 그 항목들은 하네스 레포에서만 착수할 수 있어, 무관한 계획에 끌어오면 노이즈다.
