...
- **질문 유형에 맞는 선택지 형식을 쓴다 (중요)**:
  ...
  - **범위·대상 질문**(여러 항목 중 무엇을 포함할지): 고정 조합(a만 / a+b / 모두)만 제시하면 사용자가 원하는 조합(예: a+c)이 없을 때 답을 못 해 작업이 막힌다. 따라서 **개별 항목을 나열하고 사용자가 자유롭게 조합·선택하도록** 한다. 추천 조합은 ★로 제시하되, "원하는 항목만 자유롭게 지정 가능(예: 'a와 c만')"을 명시한다.
    ```
    Q. 동일 패턴인 a/b/c에도 같은 개선을 적용할까요?
      개선 대상을 자유롭게 선택하세요 (여러 개 가능):
      - a (이번 요청 — 필수)
      - b — <영향/근거 한 줄>
      - c — <영향/근거 한 줄>
      ★ 추천: a+b+c 모두 (일관성). 원하는 것만 골라도 됩니다 (예: "a와 c만", "a만").
    ```
  - 범위 질문에서 "모두/일부/하나" 같은 묶음만 강요하지 말 것 — 항목 단위로 제시해 어떤 조합이든 답할 수 있게 한다.
