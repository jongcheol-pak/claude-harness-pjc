...
- 통과 후에만 다음 단계.
- **호출 실패 시**: `plan-reviewer`는 Opus라 과부하가 잦을 수 있다. `implement-task`의 "Reviewer 호출 실패 대응" 규칙을 따른다 — **과부하(529)**면 재시도 후 계속 실패 시 Sonnet 대체 실행 + "검증 깊이 저하 가능" 명시(A 분기), **도구 사용 불가**면 재시도가 무의미하므로 위 Step 9 예외의 체크리스트 대체(B 분기).
