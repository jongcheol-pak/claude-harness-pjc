...
- **ⓑ 옵션 상한에 굴복하지 않는다.** 제시할 선택지가 **5개 이상이면 도구로 자르지 말고** 위 마크다운 형식으로 전량 제시한다("개수 제한 없음"은 도구 제약보다 우선하는 원칙이다). 도구를 쓸 때는 **"원하는 조합이나 다른 안은 Other로 자유 입력 가능"** 을 질문 본문에 명시한다. 범위·대상 질문은 `multiSelect`를 쓰되, 항목이 4개를 넘으면 마크다운으로 간다(형식 정본은 `references/decision-points.md` D2).
