...
- **Deferred 대장 확인 — 목적이 둘이고 읽는 방식이 다르다**: `docs/plans/deferred.md`의 `## 대기`(주제별 그룹이므로 관련 그룹만). 무관하거나 파일이 없으면 조용히 통과한다(빈 기록 강제 금지). 형식·쓰기·소진 규정은 implement-task `references/phase-f-detail.md` F-6.5 정본.
  - **① 할 일 후보 (주제 매칭)** — 관련 항목이 있으면 plan에 반영한다(재수용 task로 채택, 또는 이번에도 Deferred 유지를 명시).
  - **② 전제 반증 소스 (전제 문장별 역질의)** — 이 plan이 **참으로 삼는 전제**마다 *"대장에 이것을 부정하는 항목이 있는가"* 를 묻는다. 쌓인 관측 사실이 전제를 반박할 때가 있고, ①과 달리 **주제가 달라도** 걸린다(주제 매칭만 하면 "무관"으로 지나친다).
