...
**분해 전 — 기준 디자인 확보 (필요 시 사용자에게 요청)**: 분해는 디자인을 **실제로 본 상태**에서만 정확하다. 아래에 해당하면 Step 8 일괄 질문으로 미루지 말고 **이 자리에서 즉시 요청**한다 — 기준 디자인은 분해의 **입력**이라, 없으면 표 자체를 만들 수 없다(질문 분산 방지 규칙의 예외).
