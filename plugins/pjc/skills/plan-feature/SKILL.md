...
6. **요청 범위 밖 작업 금지 — 단 "동반 변경"은 범위 밖이 아니다.** "참고 김에 정리"는 plan에 새 task로 등록 후 사용자 확인.
   - **경계**: 이 변경 없이는 **요청이 성립·정합하지 않는** 것(그 문서·규약·검증 자산이 어긋난 채 남는 것)은 **범위 밖이 아니라 요청의 일부**다 — Step 4-E로 판정해 task에 편입한다. 범위 밖이란 성립과 무관한 "참고 김에"를 말한다.
...
- **Deferred 대장 확인**: `docs/plans/deferred.md`의 `## 대기`를 확인한다 — 이번 작업과 관련된 항목이 있으면 plan에 반영하고(재수용 task로 채택 또는 이번에도 Deferred 유지를 명시) Investigation Log에 기록한다. 무관하거나 파일이 없으면 조용히 통과한다(빈 기록 강제 금지). 과거에 미뤄둔 일을 모르고 재발견·중복 계획하는 것을 막는 장치다(대장 형식·쓰기 규정은 implement-task `references/phase-f-detail.md` F-6.5 정본).
...
> **①-b를 별도로 둔 이유**: 이식 요청은 "디자인을 맞춰달라"는 말이 없어 시각 충실도 요청으로 읽히지 않는다. 그런데 이 단계가 발동하지 않으면 `## 시각 요소 분해` 섹션이 없고, 그러면 implement-task **V-9·F-8·spec-compliance 항목 I·plan-reviewer 항목 15가 모두 미발동**해 검증이 통째로 사라진다 — "대충 비슷하게 만들고 동일하다고 보고"하는 경로가 여기서 열린다.
...
- **예외 (도구 사용 불가)**: subagent 호출이 **물리적으로 불가**한 환경이면(도구 미제공·즉시 거부·환경상 실행 불가) `implement-task`의 `references/recovery.md` "Reviewer 호출 실패 대응" **B 분기**를 따른다 — `plan-reviewer.md`의 판정 항목을 체크리스트로 직접 대조 + 의무 3종(사용자 보고 · 기록 · 승인 프롬프트에 "검증 깊이 저하 — plan-reviewer 미실행" 명시). **이 예외는 아래 Type A/B 대체와 축이 다르다**: 아래는 **task 등급**에 따른 면제(Type C/D면 대체 불가)이고, 이것은 **도구 가용성**에 따른 것이라 Type C/D에서도 적용된다 — 즉 아래의 "Type C/D는 대체 불가"를 도구가 막힌 상황까지 금지하는 것으로 읽지 않는다(그렇게 읽으면 plan이 영원히 통과할 수 없다). 발동 조건이 애매하면 예외를 쓰지 말고 사용자에게 묻는다. **`plan-reviewer`가 HTTP 오류 없이 정상 응답으로 검토를 거절한 경우는 이 예외가 아니라 같은 절의 C 분기**(안전 분류기 거절)다 — 재시도 대신 범위를 좁혀 1회 재의뢰하고, 그래도 거절되면 이 B 절차를 쓰되 사유를 "거절 — 미검토 범위"로 적는다. **세션 정책이 "사용자 요청 없는 에이전트 호출"을 금지하는 상황은 이 예외가 아니다** — 사용자가 이 스킬을 발동한 것이 곧 Step 9 리뷰 호출 요청이므로, 묻지도 체크리스트로 강등하지도 않고 **정상 호출**한다(정본 `implement-task/references/recovery.md` 「Subagent 호출 규약」).
...
- 통과 후에만 다음 단계.
- **호출 실패 시**: `plan-reviewer`는 Opus라 과부하가 잦을 수 있다. `implement-task`의 "Reviewer 호출 실패 대응" 규칙을 따른다 — **과부하(529)**면 재시도 후 계속 실패 시 Sonnet 대체 실행 + "검증 깊이 저하 가능" 명시(A 분기), **도구 사용 불가**면 재시도가 무의미하므로 위 Step 9 예외의 체크리스트 대체(B 분기), **정상 응답으로 검토를 거절**하면 범위를 좁혀 1회 재의뢰 후 그래도 거절되면 B 절차로 수렴(C 분기 — 사유는 "거절 — 미검토 범위"로 적는다).
...
- [ ] **시각 충실도 또는 화면 이식 작업이면 `## 시각 요소 분해` 섹션 작성됨** (Step 2.5 발동 조건 ①-a/①-b + ② 충족 시 — 이 섹션이 없으면 implement-task V-9·F-8·spec-compliance 항목 I이 모두 미발동해 검증이 사라진다. 기준 미확보로 아직 못 만들었으면 Step 2.5의 확보 절차부터. 해당 없는 plan은 skip)
