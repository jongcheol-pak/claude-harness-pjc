...
- **독립 read-only 조사(위치·패턴 찾기)가 2개 이상이면 `explorer` subagent 병렬 위임을 기본으로 한다** (메인 컨텍스트 보호). 단일·소규모라도 메인 컨텍스트를 아껴야 하면 위임 가능.
  ...
  - **위임 상한 (과위임 방지)**: 위의 병렬 권장은 "독립 조사가 여러 개일 때"에 한한다. ① **병렬 상한** — 한 turn의 explorer는 서로 독립이고 **답을 각각 쓸 질문**만큼만 띄운다. 같은 질문을 여럿에 나눠 던져 교차 확인하려는 중복 위임은 확신을 주는 게 아니라 서로 다른 요약을 만들어 판단을 흐린다. ② **소작업 다중 위임 금지** — 파일 한둘을 읽거나 grep 한 번이면 끝나는 조사는 위임하지 않는다(왕복 비용이 직접 하는 것보다 크다). **단 이 절 첫 불릿이 허용한 "메인 컨텍스트를 아껴야 할 때의 단일·소규모 위임"은 그대로 예외다** — 금지 대상은 컨텍스트 보호가 목적인 위임이 아니라, 직접 하면 될 일을 습관적으로 넘기는 위임이다. ③ **검증 목적 위임 금지** — Step 9의 `plan-reviewer`를 **제외한** "이 plan이 괜찮은지 봐 달라"식 위임은 하지 않는다. plan의 근거는 메인이 직접 읽어 확인한 것이어야 한다(위 「explorer 결과 취급」과 같은 이유).
