...
2. **기존 plan.md 확인**: 이미 `plan.md`가 있으면 다음을 구분한다 (항상 묻지 않는다):
   - **이전 계획이 완료됨**(모든 task 완료, notes에도 반영됨)이 명백하면 → 묻지 않고 새 계획으로 교체한다 (완료된 plan.md는 휘발성 잔재이므로 보존 가치 없음 — notes가 영구 기록을 이미 담았다). **단 교체 전에 기존 plan의 `## Deferred / Follow-up`에 미처리 항목이 남았는지 확인한다** — task 체크박스가 전부 `[x]`여도 Deferred는 "다음에 할 일"이라 완료가 아니다. 남아 있으면 이번 작업과 관련된 항목은 새 plan의 `## Deferred / Follow-up`으로 이관하고, 무관한 항목은 notes.md 최신 항목에 "미처리 Deferred" 목록으로 옮긴 뒤 교체한다(조용히 덮어써 유실하지 않음 — implement-task F-6.5의 notes 포함 의무와 이중 안전망).
     - **PRD 연결 plan은 완료 판정에 Phase G 통과까지 포함한다**: 기존 plan에 `**PRD**:` 줄이 있으면 task가 전부 `[x]`여도 그것만으로 완료가 아니다 — PRD 기반 작업의 완료는 **Phase G(요구 재검증) 통과 = active Must FR 100%**까지다. `## Phase Ledger`가 `Phase F 통과`를 담고 notes 최신 항목에 Phase G 충족표(Must 100%)가 있을 때만 완료로 보고 교체한다. **`## Phase Ledger` 마커가 없거나 `Phase G 재루프 N회차`(루프 진행 중)이거나 notes에 충족표가 없으면** — task가 전부 `[x]`여도 Phase G가 미완결일 수 있으므로 완료로 단정하지 말고 아래 '완료 여부가 불분명' 분기로 처리한다(조용히 교체 금지, 사용자에게 확인). 마지막 task 직후 세션이 끊겨 Phase F/G가 실행되지 않은 plan을 완료로 오인해, 요구 재검증 없이 폐기하는 것을 막는다. (PRD 없는 plan은 Phase F가 최종이므로 기존 기준 그대로 — task 완료 + notes 반영이면 완료.)
