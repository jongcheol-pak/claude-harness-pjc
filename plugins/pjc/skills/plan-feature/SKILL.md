...
1. **미커밋 변경 확인**: `git status`로 커밋되지 않은 변경이 있는지 본다. **단, 미커밋이 있다고 항상 묻지 않는다** — 다음을 구분한다:
   ...
   - **'폐기'는 데이터 손실이므로 Claude가 직접 실행하지 않는다** — `git clean`/`git checkout --` 등 되돌릴 수 없는 명령은 `block-destructive` hook이 차단하기도 한다. 폐기를 원하면 사용자가 직접 실행하도록 안내하고, Claude는 커밋·stash까지만 돕는다.
