...
- (ii) **불가피한 Halt 명시** — 파괴적 작업·새 의존성·외부 인증·승인 필수 등 자동 진행이 금지된 항목은 "불가피한 Halt"로 plan에 명시한다.

둘 다 아닌 예측 halt는 자율 루프에서 추측을 유발하므로 plan을 보강해 (i)로 해소한다. **단, 불가피한 Halt(ii)를 사전결정으로 우회해 자동 실행으로 바꾸지 않는다** — 안전 게이트는 보존하고, 회피 가능한 중단(결정 누락)만 제거한다.
