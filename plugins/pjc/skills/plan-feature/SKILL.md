...
- **Deferred 대장 확인 — 목적이 둘이고 읽는 범위가 다르다**: `docs/plans/deferred.md`의 `## 대기`. 파일이 없거나 걸리는 항목이 없으면 조용히 통과한다(빈 기록 강제 금지). 형식·쓰기·소진 규정은 implement-task `references/phase-f-detail.md` F-6.5 정본.
  ...
  - **③ 소진 batch 착수 판정 (잔량이 임계를 넘었는가)** — `▶ 현행 잔량` 앵커가 **100건을 넘거나**, `## 대기` 구간 `^- \[` **날짜 접두의 최솟값이 30일을 넘으면** 이 plan에 **batch task를 넣는다**(F-6.5 「대장 소진 batch 규약」 ⓪이 정본). **최고령은 앵커에 없으므로 위 정규식으로 도출**하며, **현재 날짜를 확인할 수 없으면 잔량 축만 적용**한다. 이 판정을 여기 두는 이유는 **Step 1이 대장을 여는 유일한 정기 지점**이기 때문이다 — 규약 파일에만 적어 두면 임계를 넘어도 아무도 보지 않아, 실제로 batch가 세 회차 동안 0회인 채 대기가 94 → 129건이 됐다.
...
- **2회 후에도 BLOCKER/MAJOR가 잔존하면 → 사용자에게 에스컬레이션**(**2라운드에도 동일 지적이 잔존하는지 메인이 판정**한다 — 상한이 2면 3라운드가 없어 `RECURRING`이 도달 불가이므로, 그 표시를 에스컬레이션의 **입력으로 삼지 않는다**. 리뷰어가 `RECURRING`을 보내오면 그대로 인용하되, **없어도 2회 잔존이면 에스컬레이션한다**): 무엇이 왜 3회 반복됐는지 + 선택지(수정 방향 승인 / 범위 축소 / 직접 지침)를 제시하고 지시를 기다린다. **자동 통과·무한 재시도 금지** — plan을 통과시키지 못한 채 implement-task로 넘어가지 않는다(교착 방지: 종결은 통과 아니면 에스컬레이션 둘 중 하나).
