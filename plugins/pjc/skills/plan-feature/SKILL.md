...
- **스킬 개선 큐 확인 (하네스 레포 세션에서만)**: 이 세션의 대상이 **하네스(플러그인) 레포**이면 — 판정은 `cwd` 아래 `plugins/pjc/.claude-plugin/plugin.json` 존재이며 **상위 디렉터리를 탐색하지 않는다**(세션 cwd는 repo 루트인 것이 정상이고, 상위 탐색을 넣으면 하위 폴더에서 연 무관한 세션까지 하네스로 오판한다. `implement-task` 쪽 hook과 **같은 기준**이어야 한다 — 갈리면 한쪽만 발화한다) — vault 루트 **`skill-feedback.md`**를 읽어 `[SKILL-IMPROVE]` 항목을 **할 일 후보**로 조회한다(`pjc:llm-wiki` 절차 K 5-1이 저장처·형식의 정본). 다른 프로젝트 세션에서는 조회하지 않는다 — 그 항목들은 하네스 레포에서만 착수할 수 있어, 무관한 계획에 끌어오면 노이즈다.
  - **이 조회가 없으면 큐는 유입만 되고 착수되지 않는다** — 실측으로 12건이 최장 22일 방치됐고, 그 원인은 판정 부족이 아니라 **하네스 세션이 큐를 여는 지점이 없었다는 것**이다(위키 세션은 "사용자 지시가 있을 때만" 제거하므로 스스로 착수를 만들지 못한다).
