---
name: plan-feature
description: This skill plans a code change before any code is written — it decomposes the work into tasks, pre-resolves every decision branch, and defines verifiable acceptance, producing a single plan.md. Use it when the user requests a code change beyond trivial edits. Triggers on Korean dev phrases ("계획", "설계", "기능 추가", "feature 추가", "리팩토링", "구현", "수정", "변경", "여러 곳", "전체", "만들어줘", "앱 만들어", "도구 만들어") and English equivalents ("plan", "design", "implement", "refactor", "build an app", "create a tool"). Building a whole new app or product triggers the large-scale path (PRD step) inside this skill. Also triggers when the request lists multiple sub-tasks (bullets or numbered items) even without explicit keywords. Use plan-feature when the change spans multiple files, alters logic flow, changes a signature, adds a new function/class/method, refactors a resource layer (i18n, theming, DI), or has unclear cross-file impact. A single defect needing root-cause investigation goes to pjc-systematic-debugging first; if the fix grows to span multiple files or change a signature, plan it here. Bug/error/crash reports (버그/에러/크래시/안 됨/"왜 안 돼") route to pjc-systematic-debugging, not here. If an approved plan.md already exists and the user only wants to execute it (구현/이대로 진행/계속/T<N> 진행/go), use implement-task — this skill writes the plan, it does not run it. For a single component that has a dedicated skill — a Domain/Application service (add-domain-service), a screen/page/dialog ViewModel (add-viewmodel), or a wiki page (llm-wiki) — let that skill handle it; use plan-feature when the work spans beyond one such component. DO NOT trigger for clearly trivial edits - single-line UI text/label changes, icon/image swaps, color or size value changes with no logic/structure change, README/문서 typo fixes, comment additions, single-line config edits (.editorconfig, .gitignore), single-line resource edits (strings.xml, Resources.resx for a single key), or any small code edit of 3 lines or fewer that does NOT add a new function/class/method or change a signature (Claude edits those directly; the impact-warn hook validates cross-file impact). For ambiguous cases (single bug fix of unclear scope, refactoring whose extent is unknown) do NOT silently force a plan - ask the user "A) edit directly / B) make a plan". See SKILL body for full Trivial Bypass criteria, decision rounds, and recommended-answer format with ★.
argument-hint: "<요청 설명>"
---
