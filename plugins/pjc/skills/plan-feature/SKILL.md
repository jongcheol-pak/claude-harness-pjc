...
- **2회 후에도 BLOCKER/MAJOR가 잔존하면 → 사용자에게 에스컬레이션**(**2라운드에도 동일 지적이 잔존하는지 메인이 판정**한다. `plan-reviewer`의 RECURRING 임계도 **2회 연속**으로 함께 내렸으므로(`plan-reviewer.md:306`) 그 표시가 오면 그대로 인용하되, **표시 유무와 무관하게 2회 잔존이면 에스컬레이션한다** — 판정 주체는 메인이고 리뷰어 표시는 보조 근거다. 임계를 함께 내리지 않았다면 3회 연속 기준의 RECURRING이 **영원히 도달 불가**가 되어 이 경로가 죽었을 것이다): 무엇이 왜 2라운드 연속 잔존했는지 + 선택지(수정 방향 승인 / 범위 축소 / 직접 지침)를 제시하고 지시를 기다린다. **자동 통과·무한 재시도 금지** — plan을 통과시키지 못한 채 implement-task로 넘어가지 않는다(교착 방지: 종결은 통과 아니면 에스컬레이션 둘 중 하나).
