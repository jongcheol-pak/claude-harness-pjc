...
**현행 격리 기준선 (2026-08-06, run `20260806-122901` · 56건 · opus)**: 발동률 **0.833**(판정 pos 30건 / `inconclusive` 1건은 분모 제외) · 오발동률 **0.000**(판정 neg 24건). ⚠ **이 값은 앞의 `0.6`·`0.750`과 분모가 다르다** — should-trigger 케이스의 턴 소진분을 분모에서 빼기 시작한 뒤의 수치이므로(위 「턴 상한」), 그 전 값과 직접 빼서 증감으로 읽지 말 것. 같은 정의로 잰 직전 값은 `20260805-175450`의 **0.750**(판정 pos 28건)이고 `compare_evals.py`가 이 둘을 대조하면 **개선 3건 / 회귀 1건**이다.

⚠ **이 0.833은 현재 케이스 파일로 잰 값이 아니다** — 그 run 직후 `rec-pos-1`·`dbg-pos-1`·`plan-pos-9` 3건을 교정했고 그중 2건이 개별 재측정에서 `pass`로 바뀌었다(각 케이스 `why`의 *"재교정(2026-08-06, T3 ④-1)"* 참조). **지금 파일로 전량을 다시 돌리면 더 높게 나온다** — 다음 회차가 A/B 기준선으로 쓸 때는 이 run이 아니라 **교정 후 전량 run을 새로 떠서** 쓸 것.
