...
4. **요청한 것만, 확인된 범위 안에서, 최소한으로 수정.**
   ...
   - 작업 중 발견한 다른 문제는 plan.md follow-up에 추가만 (**자기 유발이 아닌 것에 한한다 — 규칙 4-1**). **단 F-6.5 「등재 게이트」의 ⓑ 조건**(ⓑ-1 이 task의 `Files` 목록 안 + 검증 매핑 표에 명령 있음 / ⓑ-2 **이번 회차가 그 결함 때문에 실제로 틀렸고 그 사실이 선행 기록에 남아 있음** + 같은 검증 조건 + Halt 경계 미접촉)**을 충족하면 추가가 아니라 그 자리에서 고친다** — 대장에 올리지 않는다. **그리고 「추가」도 무조건은 아니다 — 그 게이트의 다섯 범주를 통과해야 대장에 오른다**(`references/phase-f-detail.md` F-6.5)(등재가 공짜로 취급되면 1분이면 될 것도 다음 회차로 밀린다). 발견한 것이 **pjc 스킬 자체의 결함·마찰**이면 `pjc:llm-wiki` 절차 K 5-1의 `[SKILL-IMPROVE]` 큐에도 1줄 기록한다(vault 없으면 그 규약의 폴백을 따름).
   - 한 번에 한 task만 수행.
...
- **둘 중 하나라도 BLOCKER/MAJOR → Phase I로 복귀, 수정 후 (수행된) 리뷰를 다시 병렬 재실행**(재실행 범위는 아래 **증분 재리뷰 표**를 따른다)**.** 둘 다 OK/MINOR일 때만 다음 단계 (MINOR → follow-up 등록). **단 그 MINOR가 이번 변경이 유발한 결함이면 등록이 아니라 수정 대상이다 — 규칙 4-1**(심각도 라벨이 원인 귀속을 바꾸지 않는다). follow-up으로 등록할 수 있는 MINOR는 규칙 4-1의 이연 가능 3종에 해당하는 것(기존 문제·범위 외·개선 제안)뿐이다. **그 3종에 들어도 대장 등재 전에 F-6.5 「등재 게이트」를 통과해야 한다** — `(판정 유보)`·**그 자리에서 고칠 것**(ⓑ-1 계획된 범위 안 / ⓑ-2 이번 회차가 유발한 오작동)·**증상만 막는 수정**·**외부 사건 트리거 조건부**·**액션이 아닌 기록성 관측** 다섯은 등재하지 않는다(각 범주의 판정 조건·예외·4조합 표 정본은 `references/phase-f-detail.md` F-6.5 — 여기 복제하지 않는다). **follow-up 등록은 최종 통과 run 기준** — 중간 run에서 본 MINOR는 최종 run에서 재평가하며(수정으로 위치가 바뀔 수 있음), 중간 결과로 중복 등록하지 않는다. **quality 리뷰의 SUGGEST(설계 소견 — code-quality-reviewer 항목 J)도 동일하게 최종 run 기준**으로 plan.md `## Deferred / Follow-up`에 `[SUGGEST]` 접두 1줄씩 등록한다(동일 파일·동일 요지는 1건으로 디듑) — verdict 무영향이므로 수정·재리뷰 없이 루프를 계속한다.
- **수정을 시작하기 전에 고칠 자리를 먼저 센다 — 지목된 자리부터 손대지 않는다.** 지적이 **자리를 둘 이상** 지목했거나 문면·규약의 **통일·정합을 요구**하면, 전수 스캔으로 자리를 세고 일괄 수정한 뒤 재스캔으로 잔존 0을 확인한다. 발동 조건 둘·3단계 절차·기록 형식은 `references/recovery.md` 「리뷰 결과 처리」 정본. **이 순서를 지키지 않아 동일 MAJOR 3회 Halt까지 간 사건이 있다**(v1.193.0 T3).
