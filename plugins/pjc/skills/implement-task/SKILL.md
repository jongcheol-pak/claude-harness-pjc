...
**⚠ 완료 커밋 전에 `git status`로 이 task의 `Files` 밖 변경이 섞여 있지 않은지 확인하고, 있으면 `git stash --include-untracked`로 격리하거나 `git add <파일>`로 대상을 지정한다.** 리뷰는 비동기라 판정문을 기다리는 동안 다른 파일을 만지게 되고, 그러면 다음 `git add -A`가 그것까지 담는다 — v1.222.0 회차에 **2회 발생**했다(T2 커밋에 T6 픽스처 · T10 커밋에 T11 구현 3파일). 한 번은 stash로 막았으나 다음 task에서 그 절차를 빠뜨려 재발했다: **기억에 의존하는 절차라 반복된다.**
