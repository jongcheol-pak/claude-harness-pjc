...
**②와 원리는 같고 표면만 다르다** — ②는 "진행하겠다"는 예고라도 남기지만 ⑤는 **완료 사실만 서술**해 규칙을 어긴 흔적조차 없어 보인다. 그러나 도구 호출 없이 텍스트만 내면 turn이 끝나는 것은 동일하며(②의 설명 참조), **이 유형은 위 `:506`이 권장하는 한 줄 마커가 그대로 정지 문구가 될 수 있어 더 잘 발생한다**. 실제로 사용자가 *"중간 보고를 하면 무조건 자율루프가 멈춘다"* 로 보고한 현상이 이것이다.
