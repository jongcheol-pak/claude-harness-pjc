...
10. **파괴적 작업 · 새 의존성 → 자동 실행 금지, Halt.**
    ...
    - **사전 승인과의 관계 (위임 경계)**: 위 파괴적 작업(force push·rm -rf·DB drop/DROP/TRUNCATE·WHERE 없는 DELETE/UPDATE·스키마 삭제·migration reset·권한/보안 변경)은 **plan의 `## 사전 승인 항목`에 적혀 있어도 일괄 사전승인 대상이 아니다 — 항상 Halt(위임 불가)**. 사전승인으로 진행되는 것은 비파괴 패키지 의존성·구조 변경·비파괴 스키마(CREATE/ADD)뿐이다.
