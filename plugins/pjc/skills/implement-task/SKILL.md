...
> **이 규칙은 문서 규칙으로 작동한다 — 기계 뒷받침(`require-evidence` Stop hook의 조건부 차단)은 v1.148.0 T2에서 추가한다.** 그 검사가 들어가면 미완료 task가 남은 자율 루프에서 위 예고 문구로 종료를 시도할 때 종료를 막고 이어서 진행하도록 되돌리며, 문구는 **띄어쓰기·어미 변형까지 관용 매칭**할 예정이다(표기를 바꿔 피할 대상이 아니라는 뜻). **현재는 그 검사가 없으므로 "hook이 막아 줄 것"이라 가정하지 말 것** — 지금 이 규칙을 지키는 것은 전적으로 이 문서를 읽는 쪽의 몫이다. **이 목록이 향후 hook 판정의 정본**이므로 여기를 고치면 hook 패턴도 함께 고친다.
