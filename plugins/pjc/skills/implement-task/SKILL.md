...
4. **요청한 것만, 확인된 범위 안에서, 최소한으로 수정.**
   ...
   - 작업 중 발견한 다른 문제는 plan.md follow-up에 추가만 (**자기 유발이 아닌 것에 한한다 — 규칙 4-1**). **단 F-6.5 「등재 게이트」의 범위 안 조건**(수정 대상이 이 task의 `Files` 목록 안 + plan의 `Verification Strategy`에 이미 있는 명령으로 재검증)**을 충족하면 추가가 아니라 그 자리에서 고친다** — 대장에 올리지 않는다(등재가 공짜로 취급되면 1분이면 될 것도 다음 회차로 밀린다. 판정 정본은 `references/phase-f-detail.md` F-6.5). 발견한 것이 **pjc 스킬 자체의 결함·마찰**이면 `pjc:llm-wiki` 절차 K 5-1의 `[SKILL-IMPROVE]` 큐에도 1줄 기록한다(vault 없으면 그 규약의 폴백을 따름).
   - 한 번에 한 task만 수행.
