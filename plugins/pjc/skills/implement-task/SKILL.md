...
- "새 세션에서 이어서 진행하시면 온전한 컨텍스트로 이어집니다"
- "컨텍스트가 길어져 후반 품질이 떨어질 수 있으니 새 세션을 권합니다"
- "/clear 후 다시 호출해 주시면 같은 지점에서 재개됩니다"
...
> **`require-evidence` Stop hook이 ②③④를 기계로 뒷받침한다 (②는 v1.148.0~, ③④는 v1.149.0~)** — 미완료 task가 남은 자율 루프에서 위 문구로 종료를 시도하면 종료를 막고(`decision: block`) 이어서 진행하도록 되돌린다. 문구는 **띄어쓰기·어미 변형까지 관용 매칭**하므로 표기를 바꿔도 걸린다(회피 대상이 아니라 지켜야 할 규칙이라는 뜻).
>
...
>
> **이 ②③④ 목록이 hook 판정 문구의 정본**이므로 여기를 고치면 `require-evidence.ps1`의 `$rxAdvance`(②)·`$rxHandoff`(③)·`$rxManualAsk`(④) **셋 전부**를 함께 고친다.

**예외 — 아래는 정당한 정지이며 ②③④에 해당하지 않는다**: Halt 보고(`## ⛔ 작업 중단`) · 최종 보고(`🎉`) · F-8 확인 게이트(`## ⏸️ 구현 완료 — 확인 대기`) · Phase 0 사전 승인 확인(`## ⏸️ 사전 승인 확인`) · 규칙 12의 외부 작업 승인 요청(`## ⏸️ 외부 작업 승인 요청`) · 리뷰어 과부하 시 선택 요청(`## ⏸️ 리뷰 인프라 선택 요청`). 이들은 **루프가 정상적으로 도달한 종착점**이거나 규칙이 명시한 개입 지점이다.
