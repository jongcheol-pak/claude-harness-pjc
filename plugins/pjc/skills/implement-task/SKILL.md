...
5. **대화 출력 예산 — task당 두 줄.** 화면(대화)에 내는 것은 **task 시작 1줄 + 완료 1줄**뿐이다:
   ...
   - **예외는 여기 열거하지 않는다** — 「금지 표현」 절 끝의 **정당 정지 목록이 정본**이다(그 전수 목록의 정본은 `docs/hook-conventions.md`). 더해 **루프 시작 전에 내는 공시는 전부**(effort 확인·시작 전 컨텍스트 공시·Phase 0 사전 승인 공시) 이 예산 밖이고 — 예산은 "task당"이라 루프 이전을 다루지 않는다 — **규칙이 명시한 질문**(F-6.5 위키 반영 능동 확인 등)도 밖이다.
...
- 두 리뷰는 **첫 판정 시 최종 diff에 전체 수행** — 어느 것도 생략·약화하지 않는다. **예외는 둘뿐이다**: ⓐ 위 **증분 재리뷰 ①·②**(재리뷰에만 적용 — 첫 리뷰는 항상 전량) ⓑ "Reviewer 호출 실패 대응"의 **A(과부하 529)·B(도구 불가)·C(거절)** fallback. **어느 쪽이든 축소·약화 사실을 반드시 명시한다**(ⓐ는 커밋 `Review:` 줄, ⓑ는 보고와 Progress Log). 실패 경로에서 V-6이 재실행되는 토큰 비용은 품질 우선으로 감수한다.
...
**전수 목록(현재 10곳)은 `docs/hook-conventions.md` 「조건 4 — 정당 정지 신호 2층」이 정본이고, 여기에는 대표 예시만 둔다** — 새 개입 지점을 추가할 때 **정본 한 곳만** 고치면 되게 하기 위함이다. 사본을 전수 나열하면 두 목록이 어긋나며, 실제로 v1.154.0 이전에 **정본 6곳 ↔ 여기 8곳으로 역전**돼 있었다(정본이 더 좁은 상태 — 그 차이가 곧 오차단 경로다).
