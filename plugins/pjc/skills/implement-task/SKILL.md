...
- ✅ **사용자 개입은 다음 경우에만**:
  ...
  3. **리뷰 인프라 장애** — Sonnet reviewer(V-5/V-6)가 과부하(529) 재시도 소진 시의 선택 요청(재시도/자체 검증/대기 — 정본 `references/recovery.md` "Reviewer 호출 실패 대응" A. Halt가 아니라 선택 후 루프 계속). **도구 사용 불가**(subagent 호출이 물리적으로 불가능한 환경)도 같은 개입 지점이나 **선택 요청이 아니라 공시**다 — 같은 정본의 B 분기(체크리스트 대체 + 의무 3종)를 따르고 루프는 계속한다. **단 "세션 지침이 사용자 요청 없는 에이전트 호출을 금지"하는 상황은 개입 지점이 아니다** — plan 승인·스킬 발동이 곧 리뷰 호출 요청이므로 **묻지 않고 정상 호출**한다(정본 「Subagent 호출 규약」). 리뷰 지점마다 승인을 구하면 위 "task 사이에 묻지 않는다"가 무너져 루프가 멈춘다.
...
> **`require-evidence` Stop hook이 이 규칙을 기계로 뒷받침한다 (v1.148.0~)** — 미완료 task가 남은 자율 루프에서 위 예고 문구로 종료를 시도하면 종료를 막고(`decision: block`) 이어서 진행하도록 되돌린다. 문구는 **띄어쓰기·어미 변형까지 관용 매칭**하므로 표기를 바꿔도 걸린다(회피 대상이 아니라 지켜야 할 규칙이라는 뜻).
>
> 다만 **"hook이 막아 줄 테니 괜찮다"고 가정하지 말 것** — 그 검사는 오차단을 피하려 조건을 좁게 잡았다(6조건 AND: 미완료 task + 루프 발동 흔적 + 예고 문구 + 정당 정지 신호 없음 + **사용자가 중단·한정을 지시하지 않음** + 차단 3회 미만). 판정 불가는 전부 통과하므로 **1차 방어선은 여전히 이 문서 규칙**이고 hook은 실수를 한 겹 더 막아 줄 뿐이다. **이 목록이 hook 판정의 정본**이므로 여기를 고치면 `require-evidence.ps1`의 `$rxAdvance`도 함께 고친다.

**예외 — 아래는 정당한 정지이며 ②에 해당하지 않는다**: Halt 보고(`## ⛔ 작업 중단`) · 최종 보고(`🎉`) · F-8 확인 게이트(`## ⏸️ 구현 완료 — 확인 대기`) · 규칙 12의 외부 작업 승인 요청 · 리뷰어 과부하 시 선택 요청. 이들은 **루프가 정상적으로 도달한 종착점**이거나 규칙이 명시한 개입 지점이다.
