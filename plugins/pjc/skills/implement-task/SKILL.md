...
"T<N>부터 계속" 같은 요청으로 시작하는 경우의 **진입 절차·판정 규칙은 `references/recovery.md` 「재개 진입 (중간 체크포인트에서 이어하기)」가 정본**이다(6단계 진입 절차 · 경량 위키 참조 · task 내부 재개 판정 · Phase Ledger 판정 · 분할 plan 호출).
