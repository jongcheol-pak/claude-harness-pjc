...
4. **요청한 것만, 확인된 범위 안에서, 최소한으로 수정.**
   ...
   - 작업 중 발견한 다른 문제는 plan.md follow-up에 추가만 (**자기 유발이 아닌 것에 한한다 — 규칙 4-1**). **단 F-6.5 「등재 게이트」의 범위 안 조건**(수정 대상이 이 task의 `Files` 목록 안 + plan의 `Verification Strategy`에 이미 있는 명령으로 재검증)**을 충족하면 추가가 아니라 그 자리에서 고친다** — 대장에 올리지 않는다. **그리고 「추가」도 무조건은 아니다 — 그 게이트의 다섯 범주를 통과해야 대장에 오른다**(`references/phase-f-detail.md` F-6.5)(등재가 공짜로 취급되면 1분이면 될 것도 다음 회차로 밀린다. 판정 정본은 `references/phase-f-detail.md` F-6.5). 발견한 것이 **pjc 스킬 자체의 결함·마찰**이면 `pjc:llm-wiki` 절차 K 5-1의 `[SKILL-IMPROVE]` 큐에도 1줄 기록한다(vault 없으면 그 규약의 폴백을 따름).
   - 한 번에 한 task만 수행.
...
진행 보고 (사용자 확인 요청 금지). **대화에 내는 것은 task당 이 두 줄뿐이다**(컨텍스트 관리 규칙 5) — 시작 줄은 그 task의 Phase P 첫 도구 호출과, 완료 줄은 다음 task의 첫 도구 호출과 같은 turn에 낸다:
```
🔧 T<N> 시작 — <무엇을 하는지 한 줄>
```
```
