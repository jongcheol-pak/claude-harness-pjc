...
- **같은 자리에 F-6.5의 대장 반영 결과도 1줄 남긴다** — `F-6.5 대장 반영: 등재 N / 게이트 제외 M`. plan.md는 gitignore + 다음 회차 교체라, 이 기록이 없으면 **압축·재개 후 그 회차가 Deferred를 실제로 이관했는지 판정할 수단이 없다**(`## Retry Ledger`가 재시도 카운터를 압축 생존시키는 것과 같은 축). 형식·기록 시점은 `references/phase-f-detail.md` F-6.5 「Phase Ledger에 반영 결과를 남긴다」가 정본.
