...
>
> **v1.154.0부터 그 뒷받침은 문면과 무관하다** — **루프가 실제로 도는 중**(마지막 사용자 발화 이후 스킬이 발동된 구간)이면 hook은 정지 문구를 보지 않고 **Strong 마커의 부재만으로** 차단한다. 위 4정규식은 루프가 도는 중이 아닐 때의 폴백으로 남아 있다. 즉 *"목록에 없는 표현을 쓰면 빠져나간다"*는 더 이상 성립하지 않으며, **정당한 정지임을 나타내는 유일한 신호는 마커**다(아래 예외 목록·마커 규약).
>
...
**전수 목록(현재 10곳)은 `docs/harness-conventions.md` 「조건 4 — 정당 정지 신호 2층」이 정본이고, 여기에는 대표 예시만 둔다** — 새 개입 지점을 추가할 때 **정본 한 곳만** 고치면 되게 하기 위함이다. 사본을 전수 나열하면 두 목록이 어긋나며, 실제로 v1.154.0 이전에 **정본 6곳 ↔ 여기 8곳으로 역전**돼 있었다(정본이 더 좁은 상태 — 그 차이가 곧 오차단 경로다).
...
> - **실제 사고**: v1.177.0에서 F-7 재검증의 판정 전 산출물을 최종 판정으로 보고 Phase F 통과를 선언한 뒤 **병합·태그·릴리즈까지 진행**했는데, 뒤늦게 도착한 판정문이 **BLOCKER 1 / MAJOR 3**이었다. 지적은 patch 릴리즈로만 닫을 수 있었다.
