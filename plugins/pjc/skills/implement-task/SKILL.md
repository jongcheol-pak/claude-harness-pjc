...
> **`require-evidence` Stop hook이 이 규칙을 기계로 뒷받침한다** — 미완료 task가 남은 자율 루프에서 위 예고 문구로 종료를 시도하면 종료를 막고 이어서 진행하도록 되돌린다. hook은 이 문구들을 **띄어쓰기·어미 변형까지 관용 매칭**하므로 표기를 바꿔도 걸린다(회피 대상이 아니라 지켜야 할 규칙이라는 뜻). **이 목록이 그 판정의 정본**이므로 여기를 고치면 hook의 패턴도 함께 고친다.
