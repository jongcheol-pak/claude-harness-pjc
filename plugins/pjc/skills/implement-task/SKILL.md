...
5. **대화 출력 예산 — task당 두 줄.** 화면(대화)에 내는 것은 **task 시작 1줄 + 완료 1줄**뿐이다:
   ...
   - **예외는 여기 열거하지 않는다** — 「금지 표현」 절 끝의 **정당 정지 목록이 정본**이다(그 전수 목록의 정본은 `docs/hook-conventions.md`). 더해 **루프 진입 전 공시**(effort 확인·시작 전 컨텍스트 공시)와 **규칙이 명시한 질문**(F-6.5 위키 반영 능동 확인 등)도 이 예산 밖이다.
