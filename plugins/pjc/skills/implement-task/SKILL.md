...
**정상 경로와 실패 3분기 모두 `references/recovery.md` 「Subagent 호출 규약 (공통) — 동기 실행」·「Reviewer 호출 실패 대응 (과부하·도구 불가·거절) — 모든 subagent 호출 공통」이 정본**이다(등급별 대체 매트릭스·의무 3종 포함).
...
- **실패는 세 갈래다.** **A 과부하(529)**: 재시도 2회 후 등급별 분기(Opus는 Sonnet 대체 가능·깊이 저하 명시 의무 / **Sonnet은 Haiku 대체 금지** — 사용자 선택 / Haiku는 재시도만). **B 도구 불가**: 재시도가 무의미하므로 정의 파일의 판정 항목을 체크리스트로 대조 + 의무 3종. **C 거절**: 재시도 대신 범위를 좁혀 1회 재의뢰, 그래도 거절이면 B 절차에 사유를 "거절 — 미검토 범위"로 적는다.
