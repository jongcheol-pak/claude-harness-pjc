...
- `code-quality-reviewer` subagent 호출 (위 V-5에서 **병렬로 함께 호출**). 자체 검토 금지 — **예외는 호출 자체가 불가한 환경뿐이며 그때는 "Reviewer 호출 실패 대응" B 분기**(체크리스트 대체 + 의무 3종). **기존 plan의 Type 라인에 남은 `(quality-review)` 플래그는 no-op**(종전 opt-in 표기 — 이미 기본이라 중복 명시일 뿐, 오류 아님). 호출은 V-5와 함께 **동기 호출**(`recovery.md` 「Subagent 호출 규약」).
...
**위 목록은 예시다. 판정 기준은 문면이 아니라 형태다** — 도구 호출 없이 텍스트만 내고 turn을 끝내면 **어떤 어휘를 썼든** 위반이다. **자기 문장이 목록에 없다는 것은 통과의 근거가 되지 않는다.** 실제로 *"T9~T13을 진행했습니다. 컨텍스트가 상당히 차서 여기서 상황을 정리해 보고합니다."* 로 정지한 사례가 관측됐고, 이 문장은 ②③④⑤ 어느 목록에도 없었다 — 목록을 대조해 "해당 없음"으로 판단하는 방식 자체가 이 실패의 원인이다.
...
**여기서도 목록은 예시이고 판정 기준은 형태다**(②의 같은 문단 참조). 진행 상황을 서술하는 방식은 무한하므로 — "T9~T13을 **진행했습니다**", "상황을 **정리해 보고합니다**", "여기까지의 결과를 **공유합니다**" — 목록에 있는 어휘를 피했다는 것은 아무 의미가 없다. **도구 호출 없이 텍스트만 내고 turn을 끝냈다면 그것이 곧 이 유형이다.**
