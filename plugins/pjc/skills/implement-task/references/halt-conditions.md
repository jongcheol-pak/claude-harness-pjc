...
| 계획 결함 | plan.md의 가정이 실측과 불일치 — **plan `## 시각 요소 분해`의 `**원본**:` 경로에 접근할 수 없는 경우도 이 행**(경로 재확인 요청). V-9는 이때 `⏳ 원본 미접근` 같은 새 표기를 만들지 않는다(별도 문자열은 `⏳ 미확인` 리터럴에 걸린 F-8 게이트를 우회한다 — `phase-v9-detail.md` 4) |
