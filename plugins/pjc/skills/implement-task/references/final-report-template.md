...
- `**다음 plan**:` 있음 (이 plan = 분할 첫 part) → **안내 출력 전에, 다음 part plan 파일 상단(분할 포인터 줄 아래)에 `## 이전 part 핸드오프` 섹션을 append**한다 — 다음 part 세션은 `/clear`로 시작해 이번 세션의 암묵지를 잃으므로, plan에 남지 않는 지식을 물려준다. 내용: 함정(건드리면 깨지는 지점)·기각된 접근(시도했다 접은 방식과 이유)·검증 지름길(빠른 확인 방법) 각 1줄, **총 5줄 상한**(물려줄 것이 없으면 "특이사항 없음" 1줄). 시크릿·민감 정보 금지(plan 민감 정보 규칙 동일). 다음 part 파일이 없거나 접근 불가면 append를 생략하고 최종 보고에 그 사실을 1줄 남긴다.
