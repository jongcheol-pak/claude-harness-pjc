...
- **분량**: `notes.md`가 400줄 초과 시, 가장 오래된 항목부터 **200줄 이하가 될 때까지** 이동 (트리거 400 / 목표 200 — 경계 진동 방지를 위해 넉넉히 줄인다).
이동 시 `notes.md` 하단 `## 아카이브 인덱스`에 `- {YYYY-MM}.md: {주요 작업 키워드}`를 추가/갱신한다.
- **항목 단위로만 자른다**: 200줄은 근사 목표다. 한 항목(`- {날짜}:` ~ 하위 불릿까지의 완결 블록)은 라인 중간에서 쪼개지 않는다. 200줄에 가깝되 넘지 않는 선에서 항목 경계로 멈춘다 (한 항목이 커서 못 내려가면 그 항목까지만 옮기고 멈춤 — 쪼개느니 약간 큰 게 낫다).
