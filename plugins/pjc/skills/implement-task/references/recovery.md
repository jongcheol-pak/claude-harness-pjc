...
**두 유형을 구분한다 — 정책 금지는 승인으로 열릴 수 있다.** 둘을 묶어 두면 "물어보면 정식 게이트를 살릴 수 있었던" 상황에서도 검증이 조용히 강등된다:

- **B-1. 정책 금지** (세션 정책·상위 지침이 subagent 호출을 **사용자 요청 없이는** 금지) — **대체로 강등하기 전에 사용자에게 1회 승인을 구한다**(예: *"이 세션 지침상 subagent 호출에 사용자 요청이 필요합니다. `<reviewer명>`을 호출할까요?"*). **승인되면 정상 호출로 진행해 정식 게이트를 살린다** — 이 경우 B 분기는 발동하지 않는다. 거부·무응답일 때만 아래 절차로 간다. 승인 요청은 그 세션에서 **1회면 족하다**(reviewer 종류마다 되묻지 않는다 — 매 리뷰마다 묻는 것은 자율 루프의 "task 사이 질문 금지"와 충돌한다).
- **B-2. 물리적 불가** (도구 미제공·호출 즉시 거부·환경상 실행 불가) — 승인으로 열리지 않으므로 **묻지 않고** 아래 절차로 간다.
