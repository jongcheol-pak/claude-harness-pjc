...
**식별**: 호출이 성공하고 **도구 호출도 정상적으로 기록**됐는데(30여 회 쓴 경우가 관측됐다) **최종 텍스트가 비어** 돌아온 경우다. 중간 서술에서 끊긴 형태도 여기 포함된다.

**`incomplete`와 구분한다.** incomplete는 **부분 판정이 있고** 무엇을 못 봤는지가 응답에 담겨 있어 그 부족분만 메우면 된다. D는 **판정문 자체가 0**이라 메울 대상조차 없다 — 그래서 처리가 다르다. A(과부하)와도 다르다: 전송 계층이 성공했고 재시도할 이유가 없다(모델이 조사를 이미 마쳤다). C(거절)와도 다르다: 거부 의사가 아니라 **출력만 유실**된 것이므로 입력을 바꿀 필요가 없다.
