...
3. 좁힌 재의뢰도 거절되면 **B 분기의 대체 절차를 그대로 적용**한다(정의 파일 체크리스트 대조 + 의무 3종). 다만 최종 보고에는 "도구 불가"가 아니라 **"reviewer 거절 — 미검토 범위: `<파일>`"**로 사유와 범위를 정확히 적는다. 무엇이 검토되지 않았는지가 사용자에게 필요한 정보다.
