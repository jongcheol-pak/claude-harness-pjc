...
`AGENTS.md`는 Claude Code 가 세션마다 프로젝트 지침으로 로드하므로(SessionStart hook `session-context` 는 그 목차를 싣는다) **한 번 기록하면 다음 세션부터 재확인이 사라진다.** 이 스킬은 「기록 측」만 담당한다.
...
2. **`AGENTS.md`만 대상** — `CLAUDE.md`는 수정하지 않는다(지침 파일이라 위험). **예외는 Step 5의 이관처 하나뿐이고, 그 이관처에 `CLAUDE.md`는 들어가지 않는다** — 스크립트가 후보에서 배제한다(`relocate-agents.py` 판정 ⓓ). **새 사실을 이관처에 직접 적는 것도 여전히 금지**다(§2).
...
| 배포·릴리즈 규약 | `## Build & Test`의 `Release` 항목 (절을 새로 만들지 않는다) |
| 변경 파일 패턴 → 필수 검증 매핑 | `## Build & Test`의 검증 매핑 표 (**여기 있어야 `pjc:implement`의 게이트가 켜진다**) |
| DB 접근 방법 | `## 데이터 접근` |
| 파일·산출물 관리 | `## 산출물·파일 관리` |
| 테스트 비대상 계층·의도된 제약 | `## Conventions` |

**섹션이 없으면 신설한다.** 위치는 `../AGENTS-BOUNDARY.md`의 **「생성물 골격」 뼈대 순서**가 정본이다 — 그 문서가 절 구성과 배치 순서를 함께 정하므로, 기존 파일의 순서가 뼈대와 다르면 따라갈 대상이 아니라 그 자리에서 뼈대 쪽으로 맞춘다. 섹션이 이미 있으면 그 안에 항목을 추가·갱신한다.

## 기록하지 않는 것

**무엇을 담고 무엇을 담지 않는지의 경계도, 담지 않는 것의 대체 저장처도 `../AGENTS-BOUNDARY.md`의 저장처 표가 정본**이다 — 그 문서가 *"여기를 가리키기만 하고 각자 재서술하지 않는다"*로 못박은 대상이라 여기서 다시 적지 않는다.

- **위키 `conventions.md` 와 매핑 표의 `## Conventions` 는 이름만 같고 대상이 다르다**(§3).
- **기록 요청이 그 경계에 걸리면** 거절하지 말고 **대체 저장처를 제안**한다.
- **이미 들어가 있는 것을 발견하면** 규칙 7의 승인 게이트를 거쳐 제거를 제안한다.

## 실행 절차

> **상한 신호만으로 들어온 경우**("AGENTS.md가 너무 커졌어" · hook의 「주입 상한 임박」)는 기록할 사실이 없으므로 **Step 1 확인 후 곧바로 Step 5**로 간다 — Step 2~4는 기록할 내용이 있을 때의 단계다.
...
- 이관 후: <N'>B (여유 <M-N'>B)
- 되돌리려면: `docs/.agents-presplit/{날짜}-{시각}/`의 **두 사본을 함께**
