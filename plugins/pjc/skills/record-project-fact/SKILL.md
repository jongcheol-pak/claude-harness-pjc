...
| 파일·산출물 관리 | `## 산출물·파일 관리` | 없으면 `## 데이터 접근` **뒤에** 신설(`## Repository Structure`는 더 이상 만들지 않는다 — 금지 목록) |
...
> 섹션 신설 위치는 bootstrap 템플릿과 동일하게 맞춘다(데이터 접근·산출물·파일 관리 모두 `## Build & Test` 뒤). 섹션이 이미 있으면 그 안에 항목을 추가/갱신한다.
