...
| 목적지 | 무엇이 가는가 | 판정 |
|---|---|---|
| **위키** | 프로젝트 성격 · 기술 스택 · 구조 · 아키텍처 상세 · 기능 명세 · 크로스 세션 규약 | 그 프로젝트를 **설명**하는 것. 코드를 읽으면 복구되고 계속 누적된다 |
| **레포 상세 문서** | 명령이 무엇을 대조하는가 · 함정 · 근거 서술 | **결과를 해석할 때** 필요한 것. 레포 안에 있어야 하고 위키에 두면 코드와 함께 안 움직인다 |
...
**③ 잔류 절도 축약 대상이다 — 절 단위 판정만으로는 줄지 않는다.**

경계 표가 「남는다」고 한 절 안에도 옮길 것이 있다. 가르는 축은 하나다 — ***명령을 실행할 때 필요한가, 결과를 해석할 때 필요한가.*** 앞은 남기고 뒤는 레포 상세 문서로 보낸다.

> 표본의 `## Plan Location`은 **잔류 절인데 41,555B**였고 남을 것은 경로 4줄이었다. 나머지는 진행 상태·다음 작업·세션 인계 서술이라 경계 표의 「어디에도 안 감」이었다.
## 출력 형식
