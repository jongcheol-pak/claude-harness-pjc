...
| 파일·산출물 관리 | `## 산출물·파일 관리` | 없으면 신설(`## Repository Structure`는 더 이상 만들지 않는다 — 금지 목록). **위치는 그 프로젝트 스택의 bootstrap 템플릿 순서를 따른다** — `## 데이터 접근` 앞뒤 어느 쪽인지가 스택마다 다르다(UI 계열 4종은 산출물이 앞, 나머지 7종은 데이터 접근이 앞) |
...
| **프로젝트 정보** — 기술 스택·디렉터리 구조·아키텍처 개요·기능 목록 | 위키 프로젝트 허브(`20_projects/…`) |
...
> **⚠ 위 5행과 허용 매핑 표의 `## Conventions`는 이름이 겹치지만 대상이 다르다.** AGENTS.md의 `## Conventions`에 남는 것은 **파일을 쓸 때마다 적용되는 규약**(인코딩·줄바꿈·주석 언어)과 **테스트 비대상 선언**뿐이다. 그 밖의 규약·함정이 누적되는 서술은 위키 `conventions.md`가 정본이다.
