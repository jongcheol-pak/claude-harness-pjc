...
않지만 순서를 맞춰 두면 한쪽을 읽은 사람이 다른 쪽을 읽을 때 되짚을 것이 없다):
  ...
  ⓑ 잔류 — `## 위키` · `## Build & Test` · `## Conventions` · `## 데이터 접근` ·
     `## 산출물·파일 관리` · `## DO NOT` · `## Plan Location` · `## OS/플랫폼`은 절 단위로
     통째 남긴다. **줄 단위로 가르지 않는다**: 명령만 남기고 「언제 어떤 조건에서 쓰는지」를
     설명하는 산문을 떼면 남은 명령이 그대로 오용된다.
     **`## OS/플랫폼`도 잔류다** — `AGENTS-BOUNDARY.md` 「AGENTS.md 내용 경계」가 조건부 잔류로
     규정한다(검증·배포가 특정 OS에 묶이거나 최소 도구 버전이 동작 조건일 때만 두는 절이라,
     두었다는 것 자체가 그 레포에서 매 검증에 걸린다는 뜻이다).
  ...
  ⓘ 이관 불가 — 잔류 절만으로 이미 상한을 넘거나 옮길 절이 하나도 없으면 **아무것도 옮기지
     ...
     새 경계에서 잔류 8종은 AGENTS.md 절의 사실상 전부라 이것은 예외가 아니라 **정상 귀결**이다 —
...
# 이관 대상이 0인 것은 새 경계에서 예외가 아니라 정상 귀결이다(잔류 8종이 AGENTS.md 절의 거의 전부).
...
KEEP_SECTIONS = ("위키", "Build & Test", "Conventions", "데이터 접근",
                 "산출물·파일 관리", "DO NOT", "Plan Location", "OS/플랫폼")
