...
def unreached_lines(orig_raw, parts, declared_removals=()):
    ...
    for p in parts:
        reached |= norm(p)
    reached |= {d.strip() for d in declared_removals if d.strip()}
    out, seen = [], set()
    for l in orig_raw.decode("utf-8", "replace").splitlines():
        s = l.strip()
        if s and s not in reached and s not in seen:
            seen.add(s)
            out.append(s)
    return out
...
def verify(agents_raw, dest_raw, dest_rel, limit, orig_raw, declared_removals=()):
    ...
    if missing:
        problems.append("원문 %d줄이 어디에도 도달하지 않았다 — 예: %s"
                        % (len(missing), " / ".join(m[:40] for m in missing[:3])))
