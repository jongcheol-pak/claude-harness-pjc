...
| hook 스크립트 `scripts/*.ps1` | **15,000 B** | 한 번에 Read해 고칠 수 있는 크기. 넘으면 판정 데이터를 스크립트 밖으로 내린다(실측: 62,830 B와 57,059 B에서 수정마다 전체 재독이 필요해졌다) |
...
| E4 | 하니스 자기 게이트를 무력화하지 않는다 | (전제) | `guard-write` | E1~E3이 성립하는 조건이다 |
| E5 | plan 없이 코드를 고치지 않는다 | `plan` 「이 스킬을 건너뛰는 경우」 | `guard-write` | plan 승인이 자율 루프의 위임장이다 |
| E6 | `AGENTS.md`에 진행 상태·세션 인계를 넣지 않는다 | `AGENTS-BOUNDARY.md` | `guard-write` | 주입 상한을 잠식하고, 넣은 세션은 자기가 넣은 줄 모른다 |
...
| E10 | 설치본과 레포 버전이 어긋난 것을 안다 | (운용) | `session-context` | 워킹트리를 고쳐도 설치본이 구버전이면 검증이 헛돈다 |
