...
| 가이드 문서 (`DESIGN.md`·`AUTHORING.md`) | **13,000 B** | **통지** | 자동 로드되지 않고 스킬을 고칠 때만 사람·모델이 열어 보므로 런타임 예산을 쓰지 않는다. 상한을 두는 이유는 읽는 부담뿐이다. **12,000 B에서 올렸다** — 이 표에 관리 대상이 한 행 늘면 표를 담은 문서도 그만큼 커진다. 문서가 스스로 차오른 것이 아니므로 「넘을 때마다 상한을 올린다」와는 다른 축이다 |
| hook 스크립트 `scripts/*.ps1` | **25,000 B** | **통지** | 한 번에 Read해 고칠 수 있는 크기. 넘으면 판정 데이터를 `rules/*.json`으로, 근거 주석을 `rules/*-rationale.md`로 내린다. **15,000 B에서 올렸다** — v1.225.0 실측에서 근거와 데이터를 전부 내려도 판정 로직과 차단 메시지가 17,000~22,000 B 남았고, 그보다 줄이려면 검사를 빼야 한다(착수 시점 최대 62,830 B) |
...
`skills/plan/` · `skills/implement/` · `agents/`. 나머지 셋에 §1 형식을 일괄 적용하지 않는다 — `llm-wiki`·`pjc-systematic-debugging` 은 회차 32 가 **4축 전수 판정**을 마친 결과이고(그 커밋 본문), `record-project-fact` 는 **미판정**이다.
