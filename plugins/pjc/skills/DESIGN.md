...
**어느 파일에 걸리는지는 §6이 정한다** — 다시 정하면 두 절이 갈린다.

**볼드 선두 불릿이 전부 규칙은 아니다. 넷은 세지 않는다**(상수는 `check-harness-consistency.py`).

- **코드펜스 안은 예시다** — 이 레포는 자기 서식을 코드블록으로 예시하므로 펜스를 무시하면 견본이 규칙으로 잡힌다.
- **`evals/fixtures/` 아래는 판정될 입력이다** — 세면 검사기가 제 시험지를 채점한다.
- **점수·등급 라벨은 척도다** — 지시가 아니라 눈금이다.
- **꺾쇠 자리표시자를 담은 줄은 템플릿이다** — 채워질 빈칸이다.

**넷 밖에서 갈리면 「그 줄을 지우면 지시가 사라지는가」로 판정한다** — 아니면 서술이다(스키마 필드 정의·절차 단계).
...
**새 규칙에는 그것이 없어서 실제로 일어난 일을 1건 이상 댄다 — 요구와 적는 자리는 `plan/references/plan-template.md`「작업 단계」의 `[면제 ⑤]` 가 정본이고, 그 범위는 §6 이 아니라 예산 표 대상 전체다** — 규약 총량을 막는 것이 크기 임계가 아니기 때문이다(2026-09-12 실측: `BUDGET.md` 가 옛 상한을 넘겨 자라는 동안 통지 등급이라 한 번도 `exit 1` 이 아니었고, 그 초과를 만든 것은 **그 예산 규약을 고치던 회차 자신**이며 그 파일은 §6 밖이다).
...
RIGHT: (판정 기준은 wiki-sync.md에 있다)
```

**한 사실은 한 곳에만 적는다.** 같은 규정을 두 파일에 복제하면 한쪽만 고쳐져 갈린다. 다른 곳에서 필요하면 복제하지 말고 파일명으로 가리킨다.

**예외 — 배포 대상에 상위 규약이 없는 규칙의 의도적 이중화.** 이 플러그인은 MIT·마켓플레이스 배포물이라(`plugins/pjc/.claude-plugin/plugin.json`) **글로벌 `~/.claude/CLAUDE.md` 가 없는 사용자에게도 간다** — 그 사용자의 세션에서 상위 규약은 「이미 로드돼 있는 것」이 아니라 **없는 것**이므로, 지우면 대체 없이 사라진다. **대상은 셋이다**: `implement/SKILL.md` 의 「멈추는 넷」·「커밋」과 `implement/references/report-format.md` 의 보고 항목. **개별 판정이 두 번 같게 나와 규약으로 올린 것이다** — 2026-09-07 `code-style.md` 중복 4항목의 현행 유지 종결과 2026-09-17 감사의 「멈추는 넷」 제거 후보 철회이고, 그 두 판정의 기록은 `intent/2026-09-17-audit-decisions-closed.md` 에 있다(하니스 레포에만 있는 파일이다). **감시 축은 아직 없다** — 상대가 레포 밖(`~/.claude/CLAUDE.md`)이라 `check-harness-consistency.py` 가 읽지 못한다. 그래서 **지금 잴 수 있는 것은 하니스 쪽 사본의 실재뿐**이고, 축을 만드는 트리거는 **두 사본이 같은 상황을 다르게 판정하는 것 — 곧 내용 모순**이다. **개수가 갈리는 것은 트리거가 아니다** — 둘은 재는 대상이 다르다. 글로벌 종료 조건은 *검증을 언제 멈추는가*이고 「멈추는 넷」은 *자율 루프의 실행을 언제 멈추는가*라, 번호를 맞추면 다른 것을 같다고 적게 된다. 실제 위험은 **같은 정지를 한쪽은 허용하고 한쪽은 금지하는 것**이고, 그것이 관측되면 축을 만든다.
...
· 5.5 대조: `platform.claude.com/docs/ko/build-with-claude/prompt-engineering/prompting-claude-opus-5-5` — 가이드가 Opus 5 패턴을 「여전히 합리적인 출발점」이라 밝혀 아래 3-1·3-2 는 유지하고, 5.5 가 더한 조기 정지 단락은 `implement/SKILL.md` 「진행 중 사용자와의 소통」에 원문으로 둔다.
...
- **진행 보고 규약은 각 스킬이 자기 실행 형태에 맞게 갖는다** — 자율 루프는 task당 한 줄, 위키 세션은 절차당 한 줄이다. 공통 문구를 두면 그 둘 중 하나는 반드시 어긋나므로 **이 항목만 가이드 원문을 싣지 않는다**. `implement` 가 싣는 5.5 단락은 진행 보고가 아니라 turn 을 어떻게 끝내는가의 규약이라 이 항목의 예외가 아니다.
...
| # | 조항 | 출처 | 담당 hook | 근거 |
|---|---|---|---|---|
| E1 | 파괴적 명령을 실행하지 않는다 | `implement` 「멈추는 넷 — 그리고 목록 밖의 넷」 | `block-destructive` | 실행되면 되돌릴 수 없다 |
...
- **차단을 직접 내는 단일 검사 hook은 다른 검사와 한 파일에 섞지 않는다** — `block-destructive`는 `AGENTS.md` DO NOT이 이름으로 지목해 차단 동작 변경을 규정하므로, 섞으면 그 규정의 대상 경계가 흐려진다. **dispatch 는 대상이 아니다** — `guard-bash`는 판정을 dot-source 로 모으는 자리라 차단원이 그 안의 개별 판정이다.

## 6. 적용 범위

`skills/plan/` · `skills/implement/` · `agents/`. **§2·§3도 같은 범위다**(**§4가 가리키는 예산 표와 §5는 스킬 트리 전체가 대상이다** — 예산 표의 대상·상한은 `BUDGET.md` 「예산 표」가 정본이라 여기 다시 적지 않고, hook 표는 그 밖의 스킬까지 잰다) — 다만 §2가 막는 것은 **편집으로 밀리는 번호**라, 번호가 기계 대조의 키인 참조(`wiki-schema` §7-N ↔ `lint.py`)와 **번호가 헤딩에 박힌 이름**인 참조(`## §1 Iron Law이 …` — 위치 파생이 아니라 표제의 일부라 절을 넣어도 밀리지 않는다)는 그 대상이 아니다. **나머지 셋(`llm-wiki`·`pjc-systematic-debugging`·`record-project-fact`)은 전수 판정 결과 §1 형식의 대상이 아니다** — 근거는 그 회차 커밋 본문에 있다.
