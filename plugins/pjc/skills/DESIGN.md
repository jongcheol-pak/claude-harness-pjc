...
**새 규칙에는 그것이 없어서 실제로 일어난 일을 1건 이상 댄다 — 「이론상 문제가 될 수 있다」는 근거가 아니다** — 규약 총량을 막는 것이 크기 임계가 아니기 때문이다(2026-09-12 실측: `BUDGET.md` 가 예산 상한의 **83% → 115% → 122%** 로 자라는 동안 통지 등급이라 한 번도 `exit 1` 이 아니었고, 115% 를 만든 것은 **그 예산 규약을 고치던 회차 자신**이며 같은 회차가 재는 단위를 바꿔 그 초과를 덮었다). 출구에 자가 없으므로 입구에서 잰다.
