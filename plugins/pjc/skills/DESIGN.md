...
| 가이드 문서 (`DESIGN.md`·`AUTHORING.md`) | **13,000 B** | 자동 로드되지 않고 스킬을 고칠 때만 사람·모델이 열어 보므로 런타임 예산을 쓰지 않는다. 상한을 두는 이유는 읽는 부담뿐이다. **12,000 B에서 올렸다** — 이 표에 관리 대상이 한 행 늘면 표를 담은 문서도 그만큼 커진다. 문서가 스스로 차오른 것이 아니므로 「넘을 때마다 상한을 올린다」와는 다른 축이다 |
...
| 근거 문서 `scripts/rules/*.md` | **20,000 B** | 코드에서 내린 근거를 받는 그릇이라 `references/*.md`와 hook 스크립트 사이다. 넘으면 주제를 쪼갠다 |
