...
**단 plan에 없던 기능 변경이 필요해지면 멈춘다** — 근본 해결이 요구 범위 밖의 동작 변경을 요구하는 상황이며, 이는 1번(범위 이탈)에 준한다. 무엇이 왜 필요한지와 대안을 함께 보고한다.
...
- **자기 작업 검증에 서브에이전트를 쓰지 않는다** — 리뷰어 호출은 전 task 완료 후 `completion-reviewer` 1회뿐이다.
