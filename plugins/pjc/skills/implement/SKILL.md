...
**넷을 정지 조건의 전부로 읽지 않는다 — 목록 밖에서 멈추는 자리가 셋 더 있다**: 아래 「plan에 없던 기능 변경」 · 「커밋」의 시크릿 경고 · `references/final-review.md`의 같은 지적 3회. 넷은 **루프가 스스로 세는 카운터**의 수이고, 셋은 그 밖에서 각자 걸린다.
...
- **task 완료마다 현재 브랜치에 로컬 커밋한다** — 제목은 `T<N>: <무엇을>`, 본문에 무엇을·왜·검증 결과를 적는다. 이 커밋은 plan 승인에 포함되므로 다시 묻지 않는다.
...
- **`## Deferred / Follow-up`의 항목을 전부 판정한다 — 등재 전에 「지금 고칠 것인가」를 먼저 가른다**(판정 4조건은 `../plan/references/plan-template.md`의 「Deferred / Follow-up」). 등재분은 그 레포의 **Deferred 대장**(이 레포는 `docs/plans/deferred.md`)으로 옮긴다 — **대장이 없는 레포면 등재가 아니라 `[미등재:대장 없음]`으로 닫고 최종 보고에 싣는다.**
