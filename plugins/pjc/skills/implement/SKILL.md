...
- **새로 만든 케이스는 수정 전 상태에서 실패하는지 먼저 확인한다** — 그 변경을 되돌린 상태에서 1회 돌려 red 를 본다. 통과만 확인한 케이스는 자기 구현을 기대값으로 옮긴 것일 수 있고, 그런 케이스는 재는 것이 없다. 되돌리기 어려우면 **기대값을 한 글자 어긋나게 해 실패를 보는 것**으로 대신한다. 이것은 새 단계가 아니라 위 2번의 일부다.
  - **되돌리는 수단이 규약이다 — red 확인은 그 task 를 커밋한 뒤에 하고, 되돌릴 때 파일 전체가 아니라 깨뜨린 그 한 줄만 되돌린다.** `git checkout -- <파일>` 은 쓰지 않는다 — red 확인은 「고쳐 놓고 잠깐 깨뜨린다」라 **그 시점의 워킹트리가 유일한 사본**이고, 같은 파일에 다른 task 의 미커밋 편집이 있으면 그것까지 함께 사라진다(실해: 2026-09-15 회차가 그렇게 T6·T7 편집을 잃었다).
