...
def _rows_guides(pages):
    ...
    달라 병기 검사도 §7-16(평문)과 §7-27(wikilink)로 갈려 있었다(후자는 이 통합으로 폐지). 통합 표는 **첫 컬럼을 평문
...
def main():
    ...
    if "index.md" in pages:
        ...
        # §7-27(가이드/레시피 섹션 가이드 행 병기)은 **폐지**됐다 -- §7-16에 흡수.
        ...
        #  넓어져 **위 §7-16 하나가 전 행을 본다**. 폐지한 번호는 재사용하지 않는다.
