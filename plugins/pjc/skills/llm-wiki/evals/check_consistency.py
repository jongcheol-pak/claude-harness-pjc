...
TRIGGER_ALLOWLIST = [
    ...
    (LINT_PY, ["#  auto-split이 대신할 수 없는 세션 판단이라", "임박까지 기다리면 늦기 때문이다"], "위 상수 근거의 이어지는 줄"),
    ...
    (LINT_PY, ["#  없는 세션 판단이라, 95%에서 알리면", "그 사이 편집 한 번이 곧 초과가 된다"], "ⓑ 발화 시작점이 근접인 근거"),
...
TRIGGER_ALLOWLIST_BASELINE = 90   # 큐 4종 폐기로 화이트리스트 2건 소멸(대상 문면이 사라졌다) — 그 전은 92
