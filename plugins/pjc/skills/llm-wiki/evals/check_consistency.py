...
TRIGGER_ALLOWLIST = [
    ...
    (LINT_PY, ["# 이 경로에는 임박 계층이 없어", "log.md는 초과 전"], "SPECIAL_BUDGET 임박 분기 구현 주석"),
    ...
    (LINT_PY, ["# L-4: 위 임박 분기가 budget_split", "조건식은 임박과"], "억제 강등 INFO 분기 구현 주석"),
    (LINT_PY, ["#   동일하고 억제 여부만 다르다", "82%짜리가 「임박」으로 오표기된다"], "위 분기 주석의 이어지는 줄"),
...
def check_trigger_locality():
    ...
    for path, kind in _trigger_scan_scope():
        ...
        for entry in TRIGGER_ALLOWLIST:
            ...
            #  이제 조건어마다 그것을 덮는 조각을 요구한다. **아직은 `--trigger-report` 전용이다** —
            #  전건 재작성이 끝나 잔여가 실제 신호가 되면 그때 위반으로 승급한다(main 편입).
