...
TRIGGER_ALLOWLIST = [
    ...
    (SCHEMA_MD, ["소비 대상은 두 태그뿐이다", "허브 구조 개조·예산 초과 등"], "절차 M 보류 8종의 상황 범주 열거 — 예산은 그중 한 항목명"),
    ...
    (LINT_PY, ["#  **convention의 「구역화 필요」 WARN이 이 비율부터 난다**", "§7-2 발동 ⓑ"], "BUDGET_NEAR_RATIO가 자체 신호를 갖게 된 근거"),
    (LINT_PY, ["#  auto-split이 대신할 수 없는 사람 판단이라", "임박까지 기다리면 늦기 때문이다"], "위 상수 근거의 이어지는 줄"),
    ...
    (LINT_PY, ["#  없는 사람 판단이라, 95%에서 알리면", "그 사이 편집 한 번이 곧 초과가 된다"], "ⓑ 발화 시작점이 근접인 근거"),
    (LINT_PY, ["`source-stub`처럼 **어느 처방도 없는 타입이 조용해진다**", "초과 전까지 무신호"], "prescribable docstring — 산문 술어만 보면 생기는 사각"),
    (LINT_PY, ["# ⓑ' 처방이 없는데 convention도 아니다", "종전 임박 WARN을 그대로 낸다"], "ⓑ' 갈래 구현 주석"),
    ...
    (LINT_CASES_JSON, ['"rationale": "§7-2 발동 판정의 **OR 두 항**과', "「잔여 조건」만으로 발화하고", "「비율 조건」만으로 발화한다", "종전 「예산 임박」 WARN을 그대로 받고(ⓑ′)", "임박에서 침묵한다(ⓐ)", "내려가지 않는지 고정한다", "**신호 층위**(v1.207.0): 같은 임박이어도 `--auto-split`이 처리할 수 있으면", "이고 `rollover_log`가 자동 수행하므로", "초과 WARN은 그대로 남으므로 침묵 범위가 초과까지 번지지 않는지는 `log-guard`가 잰다"], "임박 OR 두 항 + 신호 층위 케이스 실증 서술"),
    (LINT_CASES_JSON, ['"rationale": "§7-2 발동 ⓑ(구역화 필요)의 **양성 + 델타 음성**', "발화선이 근접(85%)인 것이 이 케이스의 핵심이다", "임박에서만 내는 구현은 85%인 이 파일에 침묵해 FAIL한다", "96%인 이 파일에 WARN을 내며 FAIL한다", "구역화는 사람 판단이라 임박(95%)에서 알리면 그 사이 편집 한 번이 곧 초과가 된다"], "구역화 필요 양성·델타 음성 케이스 실증 서술"),
    (LINT_CASES_JSON, ['"rationale": "**개수 근사로는 못 잡는 경계**를 고정한다', "처방 가능으로 보고 **침묵한다**", "실제 게이트는 `len(secs) >= 2`", "조용한데 처리도 안 되는", "어떤 근사로도 0이라 침묵 분기에 안 들어간다"], "전체 섹션 1개 경계 케이스 실증 서술"),
    (LINT_CASES_JSON, ['"rationale": "「분리 불가 판정」(budget_split)', "판정 시점 이내라 임박 WARN이 억제되고", "억제가 풀려 임박이 재발화한다", "'한 번 판정하면 초과까지 무신호'가 된다", "여유 330<500)이라 임박 조건을 충족하는데", "`eff_chars`를 넘기는가」다", "`chars`를 넘기도록 바꾸면", "이 파일만 임박 WARN을 내며"], "억제·재판정 케이스 실증 서술"),
    (LINT_CASES_JSON, ['"rationale": "§7-2 convention 예산(12000자) 초과 WARN + 처방 힌트', "초과 시점에 수리 경로", "정해진 타입은 임박 WARN과 초과 WARN 양쪽에", "하며, 초과 시점에 침묵하면"],
...
TRIGGER_ALLOWLIST_BASELINE = 93   # 회차 45: rollover_target 신설로 목표치 근거 주석 3건 추가
