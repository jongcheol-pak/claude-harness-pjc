...
#  첫 제외는 v1.304.0 의 `wiki-ops-rationale.md` 다 — 그 파일은 규정을 **서술**하지 않고
...
#  덮지 못한 조건어는 위반이다. 조각의 폭은 **그 줄에서 그 조건어를 지목할 수 있는 만큼**이며
#  어절 경계를 지켜 자른다(문자 단위로 깎으면 읽을 수 없고 문면 수정에도 더 자주 깨진다).
#  **다른 조건어나 성격이 다른 서술을 함께 덮지 않는 것**이 판정선이다 — 넓게 덮으면 그 뒤에
#  진짜 트리거 서술이 숨어 이 검사가 막으려던 것을 되살린다.
#  줄 번호가 아니라 **줄 내용**으로
...
TRIGGER_ALLOWLIST = [
    ...
    (TEMPLATES_MD, ["<!-- 섹션이 길어지면 `### ` 소제목으로 구역화한다", "6,000자를 초과하면서 소제목이"], "feature 골격 구역화 주석 첫 줄 — 섹션 축"),
    (LINT_PY, ["# index.md 분할 신호 임계", "index.md 초과는 B/F 세션이"], "INDEX_BODY_LINES 상수 주석 — index 축"),
    ...
    (LINT_PY, ["「본문(frontmatter 포함 전체)」이라 생성 구역만 세면", "임계를 넘긴 채 멈춘다"],
    ...
    (LINT_PY, ["없으면 None(그 경우 종료 기준은", "「예산 이내」가 아니다"], "budget_state target 필드 설명 — §7-2 종료 조건 구현"),
    (LINT_PY, "**발동은 §7-2 조건이고, 목표치는", "rollover_log 발동 판정 — §7-2 조건을 정의하지 않고 인용"),
    (LINT_PY, ["처방이 **어디까지 내려가야 멈추는가**", "§8 log · §2.8 decision-log"],
     "rollover_target docstring — §8·§2.8 목표치를 모은 자리(조건 정의가 아니라 인용)"),
    (LINT_PY, ["**명시 목표보다 LOG_ROLLOVER_SLACK만큼 더 내려간다**", "§4 7번 기록 1줄이"],
     "rollover_target의 log 여유분 근거 — §8 기록 1줄이 만드는 재발동"),
    (LINT_PY, ["이 타입에는 명시 목표치가 없다", "딱 맞추면"],
     "rollover_target의 decision-log 목표 근거 — §2.8"),
    ...
    (LINT_PY, ["#  「최근 변경」 절만 훑으므로 여기 놓인 항목은", "영영 아카이브로 넘어가지 않고"],
    ...
    (LINT_PY, ["#  이 상수를 지우면 안 되는 이유", ": 임박은 `이 비율 이상"], "위 상수 근거의 이어지는 줄"),
    (LINT_PY, ["#  선행 게이트가 빠지면 예산이 작은 타입", "잔여 조건만으로 임박이 된다"], "위 상수 근거의 이어지는 줄"),
    (LINT_PY, ["#  **v1.207.0부터 이 계층이 곧 WARN은 아니다**", "auto-split이 실제로 옮길 수 있으면"], "BUDGET_CRITICAL_* 근거 — 처방 가능 여부로 가른다"),
    (LINT_PY, ["#  침묵하고, 옮길 경계가 없을 때만 신호가 난다", "§7-2 발동 ⓐ"], "위 상수 근거의 이어지는 줄"),
    (LINT_PY, "#  「초과까지 몇 % 남았다」를 반복하는 것이 소음이었기 때문이다", "위 상수 근거의 이어지는 줄"),
    (LINT_PY, ["#  몇 % 남았다」를 반복할 이유가 없다", "§7-2 발동 ⓐ와 같은 취지"], "SPECIAL_BUDGET 침묵 주석의 이어지는 줄"),
    (LINT_PY, ["#  **침묵은 임박까지다**", "위 `over` 분기의 WARN은 그대로 남는다"], "SPECIAL_BUDGET 침묵 범위 한정 주석"),
    (LINT_PY, ["#  사람이 할 일이 없는데 「초과까지 몇 % 남았다」를 내면", "내면 그것은 소음이고"], "ⓐ 침묵 갈래 구현 주석"),
    (LINT_PY, ["`source-stub`처럼 **어느 처방도 없는 타입이 조용해진다**", "초과 전까지 무신호"], "prescribable docstring — 산문 술어만 보면 생기는 사각"),
    (LINT_PY, ["# ⓑ' 처방이 없다", "종전 임박 WARN을 그대로 낸다"], "ⓑ' 갈래 구현 주석"),
    (LINT_PY, ["#  주체가 없는 파일까지 조용해지면", "초과 직전까지 아무도 모른다"], "위 갈래 주석의 이어지는 줄"),
    (LINT_PY, "# 예산 임박 임계 — 선행 게이트가 참인 파일 중", "BUDGET_CRITICAL_* 상수 근거 — §7-2가 명시 예외로 둔 자리"),
    (LINT_PY, ["# 예산 값은 두되 **초과 신호만** 내는 타입", "근접·임박 계층과 `--auto-split` 분할에서 빠진다"],
     "BUDGET_OVER_ONLY_TYPES 상수 근거 — 값을 남기고 소비 경로만 빼는 이유(v1.303.0)"),
    (LINT_PY, ["#  왜 값을 지우지 않는가: `BUDGET`에서 빼면", "초과 WARN까지 함께 죽는다"], "위 상수 근거의 이어지는 줄"),
    (LINT_PY, ["# **`BUDGET_OVER_ONLY_TYPES`(convention)도 같은 이유로 빠진다**", "그 타입은 초과 신호만 내고"],
     "RELOCATE_TYPES가 그 상수에서 파생하는 근거 — 신호 술어와 처방 술어의 단일 출처"),
    (LINT_PY, ["#   **`BUDGET_OVER_ONLY_TYPES`는 여기서도 빠진다**", "그 타입에 근접·임박 계층이"],
     "ⓒ L-4 갈래의 같은 제외 — 근접 분기만 막으면 신호가 이름만 바꿔 되살아난다"),
    (LINT_PY, ["#   없으므로 「억제로 강등된 임박」이라는 상태 자체가", "성립하지 않는다"], "위 제외 주석의 이어지는 줄"),
    (LINT_PY, ["#   분기만 막으면 임박인 파일이 이 갈래로 흘러", "같은 신호가 이름만 바꿔 되살아난다"], "위 제외 주석의 이어지는 줄"),
    (LINT_PY, "#  타입(source-stub 1800)에서 72%짜리가 임박이 된다", "위 상수 근거의 이어지는 줄"),
    (LINT_PY, ["#  §4의 이동·분리 처방이 성립하지 않는다", "판정을 기록하면 임박 WARN을"], "BUDGET_REJUDGE_MARGIN 상수 근거"),
    (LINT_PY, ["#  판정 시점 문자 수(budget_split_chars) 대비", "이 마진을 넘게 자라면"], "위 상수 근거의 이어지는 줄"),
    (LINT_PY, ['#  영구 면제로 두면 "한 번 판정하면', '초과까지 무신호"가 되어'], "위 상수 근거의 이어지는 줄"),
    (LINT_PY, ['"""「분리 불가 판정」이 유효해', "임박 WARN을 억제해야 하는지"], "budget_split 억제 판정 함수 docstring"),
    (LINT_PY, ["억제는 임박에만 적용되고", "초과 WARN은 그대로 난다. 억제된 임박은"], "위 docstring의 이어지는 줄"),
    (LINT_PY, ["# v1.207.0 — 임박에서 침묵한다", "이 경로의 처방은 §8 월별 롤오버"], "SPECIAL_BUDGET 임박 침묵 구현 주석"),
    (LINT_PY, ["#  `rollover_log`가 자동 수행하므로", "「초과까지"], "위 분기 주석의 이어지는 줄"),
    (LINT_PY, "#   영구 '예산 초과' WARN을 만드는 것을 막는다", "억제 로직 구현 주석"),
    (LINT_PY, ["# 수리 경로가 정해진 타입은 초과 시점에도", "병기한다 — 임박 WARN에서만"], "초과 WARN 힌트 병기 구현 주석"),
    (LINT_PY, "#  안내하고 초과 WARN에서 침묵하면", "위 주석의 이어지는 줄"),
    (LINT_PY, ["# v1.207.0 — 임박 계층을 **처방 가능 여부**로 가른다", "§7-2 발동 ⓐ/ⓑ"], "임박 4갈래 분기 구현 주석"),
    (LINT_PY, ["# ⓒ L-4: 위 분기가 budget_split 억제로 건너뛴", "여기로 내려온다"], "억제 강등 INFO 분기 구현 주석"),
    (LINT_PY, ["#  판정 술어는 `relocatable`이며", "`relocate_sections`가 실제로 쓰는"], "위 분기 주석의 이어지는 줄 — 술어 공유 근거"),
    (LINT_PY, ["#   침묵시키지 않는 이유: 억제를 영구 면제로", '초과까지 무신호"가 되어'], "위 분기 주석의 이어지는 줄"),
    ...
    (LINT_CASES_JSON, ['"rationale": "§7-2 펜스 제외 판정', "예산(8500자) 초과 WARN이 나야", "'예산 초과: <경로>' 정밀 문자열로"], "펜스 제외 판정 케이스 실증 서술"),
    (LINT_CASES_JSON, ['"rationale": "§7-14 순번 sub-index', "본문 400줄 임계 초과라"], "index 축 케이스 실증 서술"),
    (LINT_CASES_JSON, ['"rationale": "§7-2 발동 판정의 **OR 두 항**과', "처방 가능 여부에 따른 신호 층위",
                       "「잔여 조건」만으로 발화하고", "「비율 조건」만으로 발화한다",
                       "`feat-silent.md`는 **더 큰 96%인데도 완전히 침묵한다**",
                       "「작아서 조용한 것」과 「처방이 있어서 조용한 것」이 겉으로 같으므로",
                       "여유 500자를 넘기므로", "근접·임박 계층에서 빼면서"],
     "임박 OR 두 항 + 신호 층위 양성·델타 음성 케이스 실증 서술"),
    (LINT_CASES_JSON, ['"rationale": "**개수 근사로는 못 잡는 경계**를 고정한다', "처방 가능으로 보고 **침묵한다**",
                       "실제 게이트는 `len(secs) >= 2`", "조용한데 처리도 안 되는",
                       "근접·임박 계층에서 빠져"],
     "전체 섹션 1개 경계 케이스 실증 서술"),
    (LINT_CASES_JSON, ['"rationale": "「분리 불가 판정」(budget_split)', "판정 시점 이내라 임박 WARN이",
                       "억제가 풀려 임박이 재발화한다", "'한 번 판정하면 초과까지 무신호'가 된다",
                       "auto-split 처방이 없어 ⓐ 침묵으로 새지 않는다",
                       "근접·임박 계층에서 빠지면서"],
     "억제·재판정 케이스 실증 서술"),
    (LINT_CASES_JSON, ['"rationale": "§7-2 convention 예산(12000자) **초과 WARN + 처방 힌트**', "그 **델타 음성**을 한 픽스처에서 잰다",
                       "힌트가 「무효 항목을 제거한다」 하나로 좁혀진다", "아무 신호도 나지 않는다",
                       "초과에서만 발화하기 때문이다",
                       "13,059자로 초과라 WARN이 나고", "11,520자(96%)로 임박 구간인데", "근접·임박·억제 세 갈래", "한계를 이미 넘은 대상은"],
     "convention 초과 WARN·힌트 병기 + 델타 음성 케이스 실증 서술 — 「초과 WARN」은 §7-2가 내는 신호의 이름이고 이 케이스가 실증하는 대상이다"),
    (LINT_CASES_JSON, ['"rationale": "**항목 구간의 경계를 코드펜스가', "「뒷부분」으로 넘기고"],
     "dec-fenced 케이스 실증 서술 — 경계 오인의 결과(항목이 롤오버에서 빠짐)이지 예산 조건이 아니다"),
    (LINT_CASES_JSON, ['"rationale": "**롤오버가 훑지 않는 구역에', "영영 월별 파일로 넘어가지 않고"],
     "log-stray 케이스 실증 서술 — 절 오배치의 결과이지 예산 조건이 아니다"),

    # ── budget_split 부속 — 「분리 불가 판정」 필드 정의(§3). 처방 불가 판정의 기록 규약이지
    #    발동 조건 서술이 아니다.
    (SCHEMA_MD, ["- **`budget_split` 3필드 (선택", "임박 메시지의 `{현재}/{예산}자`", "효과는 **임박 WARN이 「분리 불가", "강등**되는 것이며 초과 WARN은 그대로 난다", "「한 번 판정하면 초과까지 무신호」", "`BUDGET_REJUDGE_MARGIN`을 넘게 자라면"], "§3 budget_split 필드 정의"),
]
...
TRIGGER_ALLOWLIST_BASELINE = 92   # convention을 예산 신호 계층에서 빼며 ⓑ 갈래 관련 4건이 소멸하고
                                  #  BUDGET_OVER_ONLY_TYPES 상수·파생·ⓒ 제외 주석 6건이 새로 들어왔다(-4 +6).
                                  #  그 전은 90(큐 4종 폐기 직후), 그 전은 92.
...
def check_trigger_locality():
    ...
    for path, kind in _trigger_scan_scope():
        ...
        for entry in TRIGGER_ALLOWLIST:
            ...
            #  이제 조건어마다 그것을 덮는 조각을 요구한다. 전건 재작성으로 잔여가 0이 되어
            #  **`main()`이 이 목록을 위반으로 판정한다** — 남으면 기본 실행이 exit 1이다.
            ...
            for no in hits:
                ...
                for frag in frags:
                    # 조각 매칭을 정규식·퍼지 매치로 **일반화하지 않는다** — 대표 앵커와 같은
                    #  부분 문자열 판정을 쓴다. 한 열거 안에서 매칭 규약이 둘로 갈리면
                    #  「이 조각은 어느 규칙으로 읽히나」를 매번 따져야 한다.
