...
30. `[기계]` 이동·분리 도달 경로 정합 (§7-30 — ⓐ 허브 `## 아카이브` 포인터 ↔ `changes.md` 양방향 ⓑ `conventions.md` `## 하위 문서` 목록 ↔ 하위 파일 양방향 ⓒ guide·entity·concept의 인덱스 등록 ⓓ guide 허브 `## 하위 문서` ↔ 하위 양방향(허브를 가리키는 링크 기준 — 위치 무관). `--fix` 비대상)
...
- **예산 신호(§7-2)의 소비 지점이 여기다.** *누가*: 이 lint 세션이 **`--auto-split`을 호출해서**. *언제*: 주 작업 완료 후. *무엇을*: **§7-2가 발동하면 §4 처방을 승인 없이 자동 수행한다** — 명령은 `python "<skill>/scripts/lint.py" "<vault>" --auto-split`이고, 세션이 하는 일은 호출·결과 검증(§4 5번)·사후 보고(6번)다(나눌 하위가 없으면 `budget_split` 판정으로 닫는다 — 판정도 처방의 한 결과다). 이 지점을 명시하기 전에는 규약이 *「80% 도달 시 처방을 수행한다」* 고만 적고 **주어가 없어**, INFO 99건 속에서 당시 80% INFO 기준 근접 10건이 누구의 일도 아닌 채 남았다(실측 — feature 하나가 여유 28자까지 갔다).
...
- 사용자 승인 후 수정. **단 다음 셋은 승인 대상이 아니다** — ① **§7-14의 index/sub-index 분할**(§4 2단계 category + 3단계 순번 파일) ② **§7-2 발동으로 소비 지점(F-2·A-4·B-3)이 착수한 산문 타입 하위 분리 및 그 판정 결과인 `budget_split` 3필드 부착**(위 F-2 예산 신호 소비 지점) ③ **롤오버**(project 허브·decision-log·`log.md` — 보존 이동이라 원문 손실이 없다, §2.2·§2.8·§8). **시점은 각 절이 정한다** — decision-log·`log.md`는 **§7-2 발동**, **project 허브는 §2.2의 「6번째 항목」이라 문자 예산과 무관**하다. 셋 다 주 작업 완료 후 자동 수행하고 사후 보고한다(§4 분할 수행 절차 1번·6번). **참조 무결성 3종(§7-23·§7-24·§7-19 stale 행)은 수기 편집 대신 `python "<skill>/scripts/lint.py" "<vault>" --fix`로 대체할 수 있다** — **단 생성 마커가 있는 vault에서는 §7-23이 빠진 2종이다**(그 섹션은 `--build-index`가 담당 — D11, 정본은 schema §7 서두)(자동 백업·[FIXED] 요약 출력 — 대상·안전장치 정본은 schema §7 서두, 승인 전 실행 금지).
...
6. **`log.md` 기록 — 위키 파일이 실제로 변했을 때만**(4의 파일백 또는 5의 question 생성): `- [YYYY-MM-DD] [KNOWLEDGE] {만든 페이지} — {질의 요지}.` **조회만 하고 끝난 질의는 기록하지 않는다** — F-2와 같은 조건이며(`references/wiki-ops-rules.md`의 log 기록 조건이 정본), log를 쓰는 것 자체가 vault 쓰기라 비 git vault에서는 사전 백업까지 연쇄 발동한다. **이 단계를 두는 이유는 4·5가 위키를 바꾸는데 G에만 기록 단계가 없어 그 산출물이 log에 흔적을 남기지 않았기 때문이다**(A~F·M은 전부 갖는다).
...
2. 태그별 잔량을 **파일별로 나눠** 집계·보고한다(F-0 동형 — `skill-feedback.md` 잔량도 함께 보고한다. **자동 경로(2-b)는 이 파일을 소비하지 않지만 보고는 한다** — 코드 세션이 큐 상태를 보는 유일한 지점이라 여기서 침묵하면 스킬 개선 후보가 쌓이는 것을 아무도 모른다). **소비 규칙은 `references/procedures-content.md` B-1 0이 정본** — 소비 전 그 절만 부분 Read(Grep/offset)해 따른다(요약만으로 소비 금지): [DECISION]·[PROJECT-FACT]는 대상 프로젝트 즉시 소비·타 프로젝트 동의 게이트·중복 검사·미등록 표식·제거 시점 재읽기-병합 그대로, [SKILL-IMPROVE]는 보고만(제거는 사용자 지시 시). 이후는 **발동 경로에 따라 갈린다**.
   ...
   **2-c. 자동 경로의 금지·보류 8종 (전부 "큐 유지 + 사유 보고")** — 물어야만 진행할 수 있는 지점들이며, **이 목록이 B-1 0 상속에 우선한다**(B-1 0은 여전히 *"고치거나 지운다"*·*"확인한다"* 를 지시하므로, 우선순위를 적지 않으면 자동 경로가 정본 위임을 따라 그대로 수행하게 된다):
   ...
   ④ **`conventions.md`에서 §7-2 발동** — 추가로 발동하면 **그 프로젝트의 `[PROJECT-FACT]`만** 보류한다(주제별 하위 분리는 경계를 사람이 정해야 한다). `[DECISION]`은 목적지가 `decisions.md`라 이 조건과 무관하며, 그쪽은 §2.8 롤오버(자동 허용)로 해소된다 — ③도 같다.
