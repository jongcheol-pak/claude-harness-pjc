...
- **`--fix`로 대체할 때도 부르기 전에 절차 커밋을 먼저 한다**(같은 근거 — 「커밋 시점」 ①) — **이 자리가 한 절차의 두 번째 호출이다.** 바로 위 `--auto-split`의 처방 결과가 미커밋으로 남아 있고 `lint.py`가 그 둘의 동시 실행을 금지해 별도 호출이므로, 사이에 커밋이 없으면 이 호출이 거부된다.
