...
- 사용자 승인 후 수정. **단 다음 둘은 승인 대상이 아니다** — ① **§7-14의 index/sub-index 분할**(§4 2단계 category + 3단계 순번 파일) ② **임박(§7-2 WARN) 도달로 소비 지점(F-2·A-4·B-3)이 착수한 산문 타입 하위 분리**(위 F-2 예산 신호 소비 지점). 둘 다 주 작업 완료 후 자동 수행하고 사후 보고한다(§4 분할 수행 절차 1번·6번). **참조 무결성 3종(§7-23·§7-24·§7-19 stale 행)은 수기 편집 대신 `python "<skill>/scripts/lint.py" "<vault>" --fix`로 대체할 수 있다**(자동 백업·[FIXED] 요약 출력 — 대상·안전장치 정본은 schema §7 서두, 승인 전 실행 금지).
