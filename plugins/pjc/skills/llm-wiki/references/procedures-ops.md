...
- **예산 신호(§7-2)의 소비 지점이 여기다.** *누가*: 이 lint 세션이. *언제*: 주 작업 완료 후. *무엇을*: **임박(WARN)이면 §4 처방을 승인 없이 자동 수행한다**(나눌 하위가 없으면 `budget_split` 판정으로 닫는다 — 판정도 처방의 한 결과다). 이 지점을 명시하기 전에는 규약이 *「80% 도달 시 처방을 수행한다」* 고만 적고 **주어가 없어**, INFO 99건 속에서 10건이 누구의 일도 아닌 채 남았다(실측 — feature 하나가 여유 28자까지 갔다).
