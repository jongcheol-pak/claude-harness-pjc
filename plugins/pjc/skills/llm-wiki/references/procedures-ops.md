...
#### F-0. 보조 스크립트 (먼저 실행)
- **큐 잔량 확인 (보고만 — 소비는 F-2 승인 후)**: vault 루트 **두 큐 파일**을 읽어 잔량을 집계·보고한다(어떤 태그가 몇 건, 어느 프로젝트인지 — **파일별로 나눠 보고**한다) — `pending.md`의 `[K-DRIFT]`·`[DECISION]`·`[PROJECT-FACT]`·`[K-MISS]`·`[SYMPTOM]`·`[K-ROUTE]`와 **`skill-feedback.md`의 `[SKILL-IMPROVE]`**. 각 파일이 없으면 그 파일은 건너뛴다. **F-0에서는 소비(제거·표식 부착·decisions.md 반영)를 하지 않는다** — 실제 소비는 **F-2 결과 보고에서 사용자 승인과 함께** 처리한다(수정 승인 게이트에 합류 — 큐 소비도 위키를 바꾸는 쓰기이므로 lint 수정 승인과 같은 지점에서 함께 확인받는다). 소비 시점의 규칙은 **B-1 0의 소비 규칙과 동일**하다(정본 `references/queue-consume-rules.md` B-1 0 — 여기 재열거 안 함): K-DRIFT는 반영 후 제거, SKILL-IMPROVE는 위키 미반영·"플러그인 개선 후보" 보고 후 재보고 억제, DECISION은 대상 프로젝트 decisions.md 추가 후 제거·다른 프로젝트는 동의 게이트·미등록 프로젝트 등록 확인, PROJECT-FACT는 대상 프로젝트 `conventions.md`(schema §2.9) 반영 후 제거(파일 없으면 생성 + 허브 링크 1회, 게이트 규칙은 DECISION과 동형), K-MISS는 레포 근거 대조 후 feature/recipe 반영 또는 기각 사유 보고 후 제거(수요 신호 — 자동 생성 아님), SYMPTOM은 증상별 인덱스(schema §6) 등재 게이트 3종 검증 후 반영 — 해법 페이지 부재면 보류(recipe/question 선행)·미검증 원인이면 기각 후 제거, K-ROUTE는 vault 경로면 그 절 제목을 고친 뒤 제거·repo 경로면 큐에 유지(하네스 세션 몫). 미처리 잔량은 이번 점검 보고에 포함하고 관련 프로젝트 정합 확인에 반영한다.
...
- 이슈가 있으면 `30_knowledge/questions/lint-{YYYYMMDD}.md`에 리포트 페이지 작성(상세 보존), 요약은 사용자에게. **단 예산 계열(§7-2·§7-14)은 그 요약에서 뺀다** — 그 항목들은 아래 소비 지점이 자동으로 처방하는 **세션의 입력**이지 사용자가 결정할 것이 아니다(리포트 페이지에는 종전대로 남는다). (이 리포트는 발견 다건이면 길어지는 게 정상이라 lint 예산 검사에서 제외된다 — is_lint_report.)
- **`--auto-split`을 부르기 전에 절차 커밋을 먼저 한다**(`references/wiki-ops-rules.md` 「커밋 시점」 ①) — 바로 위에서 만든 리포트 페이지 `questions/lint-*.md`가 미커밋인 채로 두면 체크포인트 커밋이 그것까지 삼키고, 지금은 `lint.py`가 그 상태를 거부해 처방이 아예 돌지 않는다.
- **예산 신호(§7-2)의 소비 지점이 여기다.** *누가*: 이 lint 세션이 **`--auto-split`을 호출해서**. *언제*: 주 작업 완료 후. *무엇을*: **§7-2가 발동하면 §4 처방을 승인 없이 자동 수행한다** — 명령은 `python "<skill>/scripts/lint.py" "<vault>" --auto-split`이고, 세션이 하는 일은 호출·결과 검증(§4 5번)이고, **성공은 보고하지 않는다**(6번 「보고 억제」 — 실패·보류만 사유 1줄). **나눌 하위가 없으면 `budget_split` 판정으로 닫는다 — 판정도 처방의 한 결과다.** **처방이 스크립트에 없는 갈래(ⓑ·ⓑ′)도 세션이 직접 수행한다** — ⓔ convention 구역화 · ⓕ 처방 없는 타입의 상세 이관(`source-stub`→feature/entity 이관 + 포인터 · `question`→관련 페이지 흡수 + `status: resolved`). 정본은 `references/wiki-ops-rules.md` 「세션 처방 둘」이다. 이 지점을 명시하기 전에는 규약이 *「80% 도달 시 처방을 수행한다」* 고만 적고 **주어가 없어**, INFO 99건 속에서 당시 80% INFO 기준 근접 10건이 누구의 일도 아닌 채 남았다(실측 — feature 하나가 여유 28자까지 갔다).
  - **안전장치는 §4 「분할 수행 절차」를 그대로 쓴다** — 착수 직전 상태 잡기(git vault면 체크포인트 커밋 · 비 git vault면 사본) → 수행 → 결과 검증 → **실패 시 원복 후 보고(자동 재시도 금지)**. 완료 후 **성공은 사용자에게 알리지 않고**(§4 6번 「보고 억제」) `log.md`에 기록한다 — 승인을 받지 않는 대신 되돌릴 수 있어야 하고, 그 수단은 보고가 아니라 **`log.md` 기록 + 그 착수 직전 상태**다.
- **`--fix`로 대체할 때도 부르기 전에 절차 커밋을 먼저 한다**(같은 근거 — 「커밋 시점」 ①) — **이 자리가 한 절차의 두 번째 호출이다.** `lint.py`가 `--auto-split`과 `--fix`의 동시 실행을 금지해 별도 호출이고, 바로 위 `--auto-split`의 처방 결과가 **미커밋으로 남아 있을 수 있다.** 남는지는 재점검 pass가 도는가로 갈린다 — 돌면 그 진입 체크포인트가 앞 pass 결과를 담아 깨끗해지고, 안 돌면 그대로 남는다(2026-09-16 실측: `log-guard` 픽스처는 clean, `prose-split`은 미커밋 3건). **갈리는 것을 절차가 매번 재지 않도록 언제나 커밋한다** — 남지 않았으면 커밋할 것이 없어 무해하고, 남았는데 건너뛰면 이 호출이 거부된다.
- 사용자 승인 후 수정. **단 다음 다섯은 승인 대상이 아니다**(정본 열거는 schema §7 결과 처리) — ① **§7-14의 index/sub-index 분할**(§4 2단계 category + 3단계 순번 파일) ② **§7-2 발동으로 소비 지점(F-2·A-4·B-3)이 착수한 산문 타입 하위 분리 및 그 판정 결과인 `budget_split` 3필드 부착**(위 F-2 예산 신호 소비 지점) ③ **롤오버**(project 허브·decision-log·`log.md` — 보존 이동이라 원문 손실이 없다, §2.2·§2.8·§8). **시점은 각 절이 정한다** — decision-log·`log.md`는 **§7-2 발동**, **project 허브는 §2.2의 「6번째 항목」이라 문자 예산과 무관**하다. ④ **convention 구역화**(주제별 `## ` 헤딩 부여) ⑤ **처방 없는 타입의 상세 이관**(`source-stub`→feature/entity + 포인터 · `question` 흡수 + `status: resolved`) — **④⑤가 「세션 처방 둘」**이고 `--auto-split`이 못 해서 세션이 직접 한다(`references/wiki-ops-rules.md` 「세션 처방 둘」). 다섯 다 주 작업 완료 후 자동 수행하고 **성공은 보고하지 않는다**(§4 분할 수행 절차 1번·6번). **참조 무결성 3종(§7-23·§7-24·§7-19 stale 행)은 수기 편집 대신 `python "<skill>/scripts/lint.py" "<vault>" --fix`로 대체할 수 있다** — **단 생성 마커가 있는 vault에서는 §7-23이 빠진 2종이다**(그 섹션은 `--build-index`가 담당 — D11, 정본은 schema §7 서두)(자동 백업·[FIXED] 요약 출력 — 대상·안전장치 정본은 schema §7 서두, 승인 전 실행 금지).
- **두 큐 파일 소비도 이 승인 지점에서 함께 처리**(F-0 잔량 보고 → 여기서 소비): 승인 시 B-1 0 규칙대로 `pending.md`의 K-DRIFT·DECISION(대상 프로젝트)·PROJECT-FACT(대상 프로젝트)·K-MISS(대상 프로젝트)·SYMPTOM(대상 프로젝트)·K-ROUTE(vault 경로 항목)와 **`skill-feedback.md`의 SKILL-IMPROVE**를 소비한다. 소비 전 `references/queue-consume-rules.md` 를 Read 해(15KB 단일 파일이라 그대로 한 번에 읽힌다 — 절 단위 부분 Read 가 필요 없다) 제거 시점 재읽기-병합·중복 검사·롤오버·표식 규칙을 따른다(요약만으로 소비 금지). 다른 프로젝트 [DECISION]·[PROJECT-FACT]·[K-MISS]·[SYMPTOM]는 B-1 0의 동의 게이트를 따른다.
...
1. **위키가 진실원천 — 안다고 생각해도 모델 기억으로 답하지 않는다.** 반드시 `index.md`(기능별 인덱스 포함)로 관련 페이지를 식별하고 실제로 읽는다. **`index.md` 상단에 sub-index 파일 목록(`index-{카테고리}.md` 등)이 있으면**(인덱스가 비대해 파일 분할된 경우), 관련 카테고리의 sub-index도 함께 읽어 거기서 식별한다 — `index.md`만 보고 분할된 인덱스를 빠뜨리지 않는다(절차 K 2와 동일). **결정·이력 질문**("이 기능 왜 안 했지" 등)이면 해당 프로젝트 `decisions.md`(schema §2.8)를, **오류·증상 질문**("~가 안 된다/안 뜬다" 등)이면 `## 증상별 인덱스`(schema §6)를 우선 식별한다 — **단 증상 행 인용 시 사례 프레이밍 필수**: 행의 원인은 그 사례에서 검증된 것이므로 "{프로젝트} 사례에서 이 증상의 검증된 원인은 …였다"로 답하고, 사용자의 현재 사건 원인으로 단정하지 않는다(같은 증상 ≠ 같은 원인 — schema §6 조회 해석 규칙). **두 큐 파일(`pending.md`의 DECISION·K-DRIFT·PROJECT-FACT·K-MISS·SYMPTOM·K-ROUTE + `skill-feedback.md`의 SKILL-IMPROVE)은 참조·근거 대상이 아니다 — 절차 K 2의 큐 제외 규칙("절차 K·G의 참조·검색(전체 Grep 보조 포함) 결과에서 제외")이 Query에도 동일 적용된다**(대상 프로젝트 `[DECISION]`의 "미반영 대기" 존재 보고만 예외, 위키 지식처럼 인용 금지).
...
위키 작업(A~J·L·M) 중 이 스킬·규칙 번들과 실제 위키가 어긋난 것을 발견하면, **번들 파일을 무승인으로 직접 수정하지 않고** vault 루트 **`skill-feedback.md`**의 `[SKILL-IMPROVE]` 큐에 기록한 뒤 사용자에게 보고한다(**기록 게이트 3조건은 절차 K 5-1이 정본** — 셋을 충족하지 않으면 큐에 넣지 않는다)(절차 K 5-1(`references/queue-rules.md`)·B-1 0(`references/queue-consume-rules.md`)). 이유:
...
사용자가 "위키 되돌려줘 / 잘못 갱신됐다 / 복구해줘"라고 요청하거나, 위키 작업 중 직전 갱신이 잘못됐음을 인지했을 때 사용한다. 비 git vault의 사전 백업(사전 준수 사항·wiki-schema §8)을 복구 수단으로 소비하는 절차다. **git 관리 vault면 이 절차 대신 git(diff·checkout)으로 복구한다** — **§4 분할 원복도 같다**(종전에는 `-presplit` 사본이 예외였으나 체크포인트 커밋이 그 자리를 대신한다 — §4 분할 수행 절차 1번·5번). 다만 **되돌림은 대상 목록으로 한정하고 `git reset --hard`·`git clean -fd` 같은 전역 명령을 쓰지 않는다**(근거는 §4 5번) — 단 `git checkout`/`git restore`는 **미커밋 변경을 되돌려 없앤다.** 되돌리기 전 `git status`로 미커밋 변경을 확인하고, 되돌릴 대상 파일 외에 미커밋 갱신이 있으면 사용자에게 알려 유실을 막는다. **vault에 커밋이 하나도 없으면(git init만 하고 미커밋)** git 이력이 없어 checkout 복구가 불가능하다 — 다만 그 vault는 **비 git vault로 취급**돼 사전 백업이 떠 있으므로(정본은 `SKILL.md`의 「비 git vault 사전 백업」) 아래 1번의 백업 사본으로 복구한다. 사본까지 없을 때만 복구 불가를 사실대로 보고하고, 앞으로를 위해 vault commit을 권한다.

1. **백업 확인**: `90_archive/backup/`의 `{YYYY-MM-DD}` 폴더(수정 백업)에서 대상 파일의 사본을 찾는다. **git vault에서는 이 폴더가 아예 만들어지지 않는다** — 사전 백업도 `--fix` 백업도 면제이고 §4 분할 직전 상태는 **체크포인트 커밋**이 잡으므로, 사본이 없는 것이 정상이며 **git으로 복구한다**(위 문단 · §4 5번의 대상 목록 한정 원복). **§4 분할 되돌리기도 같다** — 종전에는 `-presplit` 폴더가 git vault에도 남았으나 그 자리를 커밋이 대신한다. 비 git vault라면 `{날짜}-{시각}/` 사본 폴더를 쓴다(롤오버는 **원본과 아카이브 파일 둘 다** 들어 있다). **삭제된 프로젝트/파일 복구라면 `{YYYY-MM-DD}-deleted/` 폴더도 함께 확인**한다(§8 삭제 백업 분리). 사용자가 시점을 지정하지 않았으면 존재하는 날짜 폴더 목록을 제시해 어느 시점으로 되돌릴지 확인한다. 사본이 없으면(백업 도입 전 수정분·신규 생성 파일·30일 지나 정리된 수정 백업) **복구 불가를 사실대로 보고**한다 — 있는 것처럼 지어내지 않는다.
...
1. §0 시작 절차 수행 후 vault 루트 **두 큐 파일을 읽는다** — `pending.md`(여섯 태그)와 **`skill-feedback.md`(`[SKILL-IMPROVE]` 전용)**. 각각 없거나 비어 있으면 그 파일은 "소비할 큐 없음"으로 둔다. **빈 위키 예외**: `index.md`/`log.md`가 없어도 §0-2의 J 부트스트랩을 발동하지 않는다(절차 G·K 1과 동형 — 큐 정리 요청의 부작용으로 vault 골격을 만들지 않는다). **두 파일이 모두 없거나 비어 있을 때만 "소비할 큐 없음"을 보고하고 종료**한다 — 부트스트랩·lint·ingest로 확대하지 않는다(범위 고정). **한쪽만 비어 있으면 종료하지 않고** 남은 파일 기준으로 2번(파일별 잔량 보고)까지 진행한다 — `pending.md`가 비었다고 종료하면 `skill-feedback.md` 잔량이 **보고조차 되지 않아**, 코드 세션이 스킬 개선 후보가 쌓이는 것을 영영 모르게 된다. **`pending.md`는 있는데 `index.md`가 없으면**(미부트스트랩 vault에 큐만 쌓인 상태 — 코드 세션 큐잉은 index 존재를 요구하지 않아 실제로 생길 수 있다) 태그별 잔량만 보고하고 소비하지 않는다(**이 분기도 `skill-feedback.md` 잔량 보고를 막지 않는다** — `[SKILL-IMPROVE]`는 위키에 반영하지 않는 보고 전용이라 `index.md`·허브를 요구하지 않는다) — 소비 목적지(허브·decisions.md·index)가 없어 반영이 불가능하다. 부트스트랩(절차 J)·프로젝트 등록(절차 A) 또는 ingest(절차 B)가 선행돼야 함을 안내하고 종료한다(확대는 사용자 지시 시 별도 진행).
2. 태그별 잔량을 **파일별로 나눠** 집계·보고한다(F-0 동형 — `skill-feedback.md` 잔량도 함께 보고한다). **소비 규칙은 `references/queue-consume-rules.md` B-1 0이 정본** — 소비 전 그 파일을 Read 해 따른다(15KB 단일 파일이라 한 번에 읽힌다)(요약만으로 소비 금지): [DECISION]·[PROJECT-FACT]는 대상 프로젝트 즉시 소비·타 프로젝트 동의 게이트·중복 검사·미등록 표식·제거 시점 재읽기-병합 그대로, [SKILL-IMPROVE]는 보고만(제거는 사용자 지시 시). 잔량 보고 후 **사용자 승인을 받아** 소비한다.
3. **레포 대조가 필요한 항목은 소비하지 않는다(경량 경계)**: [K-DRIFT] 반영과 [K-MISS]의 feature/recipe 생성·보강, [SYMPTOM]의 증상별 인덱스 등재(게이트 ③ 해법 페이지 실존·게이트 ② 원인 검증에 위키/레포 대조 필요), **[K-ROUTE]의 절 제목 수정**(그 페이지를 열어 절 구성을 봐야 한다)은 대조가 전제라 — 해당 항목은 큐에 유지하고 "해당 프로젝트 ingest(절차 B) 필요 n건"으로 보고한다. M이 소화하는 것은 사용자 발화가 직접 근거인 [DECISION]·[PROJECT-FACT](와 보고성 [SKILL-IMPROVE])다.
4. 완료 후 `log.md`에 1줄 기록(`- [YYYY-MM-DD] [QUEUE] {소비한 파일} 큐 소비: {태그별 n건} 반영, {n}건 보류(ingest 필요).` — 파일명은 실제 변경한 것을 적는다: `pending.md` / `skill-feedback.md` / 둘 다)하고 잔량·보류 사유를 보고한다. lint·망라 재대조는 수행하지 않는다(F·B의 몫).
