...
# budget_split_chars: 0         #   lint 예산 판정과 같은 기준의 문자 수(=근접/임박 메시지의 {현재} 값,
...
<!-- 항목 불변: 기록된 결정은 수정·삭제하지 않는다 — 결정이 바뀌면 "번복" 항목을 새로 추가(같은 주제어 재사용, wiki-schema §2.8).
     어휘 고정: 채택 | 보류 | 기각 | 번복. 최신이 위. 예산 6000자 초과 시 오래된 항목부터 90_archive 원경로 이동.
...
<!-- 이 프로젝트에서 작업할 때 알아야 할 크로스 세션 규약·함정. 절차 K가 매 작업 전에 읽는다.
     ...
     예산 12000자 초과 시: ① 무효 항목 제거 → ② 주제별 하위 파일(conventions-{주제}.md) 분리
       → ③ 하위 재초과 시 같은 규칙으로 재분할(무제한, wiki-schema §4 "재분할 일반 규칙").
