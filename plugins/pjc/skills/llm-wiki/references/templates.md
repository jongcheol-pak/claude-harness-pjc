...
> 규칙·예산의 정본은 형제 문서 `wiki-schema.md`다 — 값이 갈리면 그쪽이 이긴다(`SKILL.md` §0-3 에 따라 해당 § 만 부분 Read 한다).
...
repo_url: "<remote origin URL — 선택. 프로젝트 식별의 1차 앵커. `git remote get-url origin` 출력 그대로. 등록(A-2 2)·ingest 결측 백필(B-2 0-2) 시 기록. 미설정도 유효. git 아니거나 remote 없으면 줄째로 생략 — §2.2>"
...
> **단 「압축 금지 4요소」(성립 조건·예외·적용 범위·채택 이유)는 얇게 쓰는 대상이 아니다 — schema §1.**
...
<!-- 이 기능을 구성하는 파일을 빠짐없이 담는 완전한 지도(지도는 두껍게) — 각주는 근거, 이 섹션은 지도 (lint §7-21이 경로 실존을 기계 검사).
     ...
     진입점 심볼(선택): 역할 뒤 '(진입점: `심볼`)' — 조회가 파일 전량 Read 없이 도달한다.
     실제로 읽고 확인한 이름만 적고 행 번호는 쓰지 않는다 (schema §2.3·§7-21) -->
- `ViewModels/{기능}ViewModel.cs` — 화면 로직 (진입점: `{기능}ViewModel`)
- `Views/{기능}Page.xaml` — 화면 레이아웃

## 동작(사용법)
사용자 관점 동작 흐름·옵션. 코드·문서로 확인된 내용만 단정 — 실행해야만 확인 가능한 서술은 `(미검증)` 표기.

<!-- 섹션이 길어지면 `### ` 소제목으로 구역화한다(§2.3·§7-32) — 6,000자를 초과하면서 소제목이
     하나도 없으면 lint이 INFO로 알린다. 통짜 섹션은 절차 K가 필요한 대목만 읽지 못하고
     섹션 전체를 읽게 만든다. 처방은 분리가 아니라 **소제목 추가**다(내용은 그대로 둔다). -->
### {하위 주제}
{그 주제의 서술}

## 구현 방법
핵심 클래스/패턴/데이터 흐름을 **얇게 — 방향만**(산문은 얇게). 코드를 문장으로 재서술하지 말고 세부는 각주로 코드를 가리킨다. **성립 조건·예외·적용 범위·채택 이유는 줄이지 않는다**(schema §1 「압축 금지 4요소」 — 줄 수 상한 없음). 구현 상세 주장은 레포 근거 각주 `[^src-...]` 필수(각주에 소스 파일 경로 병기).
...
- [YYYY-MM-DD] {주제} — **{채택|보류|기각|번복}**: {요지·근거 1줄 — 기각·보류면 실제 방향을 여기 먼저 쓴다}
...
<!-- 이 프로젝트에서 작업할 때 알아야 할 크로스 세션 규약·함정. 절차 K가 매 작업 전에 읽는다.
     ...
     wiki-schema §7-2 발동 시: ⓪ `## ` 섹션이 없거나 옮길 후보가 없으면 먼저 구역화(주제별 `## ` 헤딩) → ① 무효 항목 제거 → ② ①로도 해소되지 않으면 주제별 하위 파일(conventions-{주제}.md) 분리
     ...
     줄 끝 ({근거})는 의무 — 어휘 고정: 실측 | 사용자 지시 | 문서 대조(wiki-schema §3 「근거 통제 어휘」).
       이 타입은 origin·confidence를 두지 않아 이 자리가 유일한 신뢰도 축이고, lint §7-34가 미보유 건수를 집계한다. -->
- [YYYY-MM-DD] {규약·주의사항 한 줄} ({실측|사용자 지시|문서 대조})

## 하위 문서
<!-- 주제별 분리가 있었을 때만 둔다(없으면 이 섹션 자체를 생략).
     전 하위 파일을 나열해 조회 홉 1을 유지한다 — 목록 없이 쪼개면 분할이 곧 유실이다.
     **이 섹션은 전 산문 타입 공통 규약이다**(§4 "재분할 일반 규칙" ②) — convention뿐 아니라
       feature·project·entity·concept·guide도 하위를 만들면 같은 형식으로 나열한다.
     **하위 파일 상단에는 복귀 링크 `> 상위 문서: [[{진입 파일}|{표시 이름}]]`를 둔다** —
       그 줄이 "분리해 나온 하위"의 판정 신호이고, 하위만 열고 들어온 세션의 돌아갈 길이다.
     목록↔실파일 정합은 lint §7-30이 타입별로 기계 검사한다(ⓑ convention · ⓓ guide ·
       ⓔ 그 밖의 산문 타입 — ⓔ는 정본 포인터의 **절 이름**까지 대조한다).
