...
> **이 파일은 위키를 *쓰는* 세션(절차 A~F·I·M)만 읽는다.** 코드 작업 세션의 절차 K(read-only 참조)와 §0(경로 결정)은 `SKILL.md` 본체에 남아 있으므로 이 파일을 Read할 필요가 없다 — 그것이 이 분리의 목적이다(코드 세션은 하니스 최빈 경로라 본체 전문이 매번 상주한다).
>
