...
7. **통제 어휘 위반**: `platform`·`origin`·`confidence`·`category` 값이 고정 어휘(§3)를 벗어나면 ERR — category 오타(`Personal` 등)는 sub-index 분할 라우팅·경로 규약을 어긋나게 한다. **이 항목은 두 판정을 포함한다**: ⓐ 값 위반(ERR — 네 필드 공통, category는 값이 있을 때만 검사·부재는 경로 규약과 이중 방지 위해 미검사) ⓑ 필수 필드 누락(WARN — `origin`/`confidence`, `ORIGIN_REQUIRED_TYPES` 대상). (lint.py 검사)
...
9. **미래/이상 날짜·updated 누락**: `updated`·본문 날짜가 오늘보다 미래거나 비정상인지 검사 (데이터 위생). **이 항목은 두 판정을 포함한다**: ⓐ 값 이상(미래 날짜 ERR·형식 이상 WARN) ⓑ **필드 부재 WARN**(`UPDATED_REQUIRED_TYPES` = origin 필수 타입 + question + decision-log, `90_archive` 제외) — 필드가 없으면 신선도(§7-3)·미래날짜 검사가 조용히 건너뛰어져 추적 사각이 된다(source-stub은 불변 스텁이라 `ingested`를 쓰므로 제외). (lint.py 검사)
