...
- **정리(자동, 승인 불필요)**: 백업 수행 시 **30일 지난 `{YYYY-MM-DD}` 폴더는 삭제**한다 — 백업본은 원본이 최신 상태로 존재하는 복사본이라 삭제해도 정보 손실이 없고, 방치하면 vault가 비대해진다. 날짜는 추정하지 말고 확인 후 사용한다(확인 불가면 정리는 건너뛴다 — 백업 자체는 날짜 무관하게 수행).
- **lint 관계**: `90_archive/` 하위는 lint 검사에서 자동 제외되므로 백업 파일이 WARN을 만들지 않는다(§7).
- **복구(백업 소비)**: 백업본으로 되돌리는 절차는 SKILL 절차 L — 날짜 폴더에서 사본 확인(없으면 복구 불가 사실대로 보고) → 현재본과 차이 제시 → 사용자 확인 → 복구 직전 상태를 오늘 날짜로 재백업 후 원위치 복사 → lint 권고 + log 기록. 백업은 이 절차의 존재로 "만들기만 하는 사본"이 아니라 발견 가능한 복구 수단이 된다.
