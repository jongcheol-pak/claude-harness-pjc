...
- **예산**: ~150줄. 초과 시 **오래된 항목부터** `90_archive/` 하위에 원경로를 재현한 파일로 append 이동(§8 아카이브 구조 재사용, 시간순 append — log 롤오버와 동형으로 **부활 없음**) — **항목 단위로만 자르고 쪼개지 않는다**(하위 불릿 포함 블록이 한 항목). 보존 이동이므로 승인 불필요. **이동 시 decisions.md 하단에 `## 아카이브` 포인터 1줄을 유지·갱신한다** — `- 이전 이력: 90_archive/{원경로}/decisions.md ({가장 오래된 날짜}~{가장 최신 이동 날짜}, {누적 n}건)`. 이 포인터가 없으면 조회(절차 K 2·G 2b)가 현행 파일만 읽고 오래된 결정을 "기록 없음"으로 오답한다(검색 진입점 — log.md `## 아카이브 인덱스`와 동일 원리).
