...
### 필수 단계
0a. **pending.md 큐 소비**: vault 루트 `pending.md`가 있으면 먼저 읽는다 — `[K-DRIFT]`는 갱신 범위에 반영 후 제거, `[SKILL-IMPROVE]`는 위키에 반영하지 않고 사용자 보고, **`[DECISION]`은 해당 프로젝트 `decisions.md`(§2.8)에 항목 추가 후 제거**(미등록 프로젝트면 등록 여부를 사용자에게 확인). (절차: B-1 0 — `references/procedures-content.md`)
...
25. **pending.md 잔량 집계**: `[K-DRIFT]`/`[SKILL-IMPROVE]`/`[DECISION]` 태그별 미처리 건수를 INFO로 집계(0건·파일 없음이면 생략) — 소비를 유도한다(ingest는 B-1 0, lint는 F-0 보고 후 F-2 승인 시; §6 큐 규약). (lint.py 검사)
