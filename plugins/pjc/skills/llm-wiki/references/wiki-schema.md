...
- **예산**: ~40줄
- resolved 시 관련 페이지에 내용을 흡수하고 frontmatter를 `status: resolved`로 표시한다(SKILL B-2 3-1의 `resolved: {날짜}` 필드 표기도 동등하게 닫힘으로 인정 — lint §7-12·§7-23 공통 판정). **페이지 자체는 삭제하지 않고 보존한다**(해결 이력도 지식 — 같은 질문 재조사 방지).
...
- **복구(백업 소비)**: 백업본으로 되돌리는 절차는 SKILL 절차 L — 날짜 폴더에서 사본 확인(없으면 복구 불가 사실대로 보고) → 현재본과 차이 제시 → 사용자 확인 → **복구 직전 상태를 별도 폴더 `90_archive/backup/{YYYY-MM-DD}-pre-restore/`로 재백업**(당일 원백업 `{YYYY-MM-DD}/`을 **덮어쓰지 않는다** — 덮어쓰면 복원하려던 좋은 버전이 사라져 복구가 무효가 된다) → 원위치 복사 → lint 권고 + log 기록. 백업은 이 절차의 존재로 "만들기만 하는 사본"이 아니라 발견 가능한 복구 수단이 된다.
