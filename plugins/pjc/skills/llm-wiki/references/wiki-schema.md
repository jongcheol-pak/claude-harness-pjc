...
- 출처 표기: 인라인 각주 `[^src-이름]` + 소스 스텁 링크 병기. **구현 상세 각주에는 근거 소스 파일 경로(레포 상대경로, 백틱)를 병기**한다 — 예: `[^src-foo]: [[10_sources/personal/src-foo|소스: Foo]] — ViewModels/BarViewModel.cs, Views/BarPage.xaml`. 경로는 §2.3 경로 표기 규칙을 따른다 — **물리 경로 + brace 축약 금지(파일마다 개별 병기)**. 여러 파일을 `{a,b}.md`로 묶으면 lint §7-20이 확장하지 못해 경로 부재 WARN이 된다.
...
2. **sub-index 생성**: `index-personal.md`·`index-work.md`(vault의 기존 category 기준 — 새 분류 금지)를 만들고, frontmatter `type: index` + 본문에 `## 기능별 인덱스` 헤딩을 둔다(§7-14 행수 측정 기준 헤딩).
3. **행 이동(잘라내기)**: `index.md` 기능별 인덱스에서 **project feature 행만** 각 프로젝트 category의 sub-index로 옮긴다. **recipe 행·프로젝트 테이블·`## 미해결 질문`·`## 가이드 / 레시피` 등 비분할 섹션은 본체에 그대로 둔다**(위 표 규정).
4. **본체에 목록 등록**: `index.md` 상단에 sub-index 파일 목록을 명시한다 — 절차 K·Query가 `index.md`만 보고도 분할 파일에 도달해야 한다(§7-15가 미등록을 WARN).
...
23. **미해결 질문 인덱스 동기**: `type: question`(`90_archive/` 제외) ↔ `index.md` 본체 `## 미해결 질문` 표(비분할 섹션 — §4)를 양방향 대조. ⓐ open(`status != resolved`) question이 표에 **미등록**이면 WARN(등록 누락 = 잊힘·유실 위험 — 단 `lint-*` 리포트 페이지는 질문이 아니라 제외) ⓑ resolved question이 표에 **잔존**하면 WARN(stale — index에서만 제거하고 **페이지는 보존**, B-2 3-1·§2.7). resolved 페이지의 '삭제' 행위 자체는 스냅샷 검사로 탐지 불가 — 삭제 금지는 절차 규칙(§2.7·SKILL 사전 준수 사항)이 담당한다. (lint.py 검사)
...
- **WARN 10건 이상이면 lint.py가 `[WARN]` 헤더 아래 프로젝트별 집계 1줄을 함께 출력**한다(대규모 정비 세션의 우선순위 파악용 — 항목 자체는 불변, 출력 요약만 추가)
