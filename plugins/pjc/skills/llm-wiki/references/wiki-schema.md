...
2. **예산 준수**: 모든 파일 줄 수 확인, 초과 시 압축/분리. **guide는 `guide_kind`(§2.6)가 예산을 선택하므로 값이 부재하거나 통제어휘 밖이면 WARN** — 침묵하면 기본 200줄이 적용돼 recipe 120줄 예산이 조용히 우회된다(WARN 문구에 "기본 200줄 적용됨"을 병기해 가시화)
