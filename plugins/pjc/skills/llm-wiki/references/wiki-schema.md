...
12. **(미검증)·미해결 question 집계**: ⓐ `20_/30_/40_` 콘텐츠 페이지 본문(frontmatter 제외, `90_archive` 제외)의 `(미검증)` 표기 발생 수·파일 수 ⓑ `type: question` ∧ 미해결 파일 수를 **정보(INFO) 등급**으로 집계 리포트(0건이면 생략). 닫힘 판정은 `status: resolved` 또는 `resolved:` 필드(B-2 3-1) 어느 쪽이든 인정하고, `lint-*` 리포트 페이지는 질문이 아니라 집계에서 제외한다 — **§7-23 등록 요구와 동일 기준**(집계↔등록 모순 방지). `10_sources`는 불변 스텁이라 검증 루프 대상이 아니므로 제외. (lint.py 검사 — §11 사용자 검증 후보 공급)
