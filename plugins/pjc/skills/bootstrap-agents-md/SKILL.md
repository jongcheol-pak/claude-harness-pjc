...
6. **프로젝트 정보 절을 만들지 않는다.** 프로젝트 성격·기술 스택·디렉터리 구조·아키텍처 개요는 **위키가 정본**이므로 생성물에 `## Stack`·`## Repository Structure` 같은 절을 새로 만들지 않는다(템플릿에도 없다). **무엇을 담고 무엇을 담지 않는지의 정본은 `docs/harness-conventions.md`의 「AGENTS.md 내용 경계」다** — 이 스킬은 그 표를 재서술하지 않는다.
7. **생성물에 설명을 길게 쓰지 않는다.** AGENTS.md의 독자는 사용자가 아니라 **다음 세션의 에이전트**다. 명령·값·규칙·⚠ 1줄만 남기고 경위·근거 서술은 넣지 않는다(같은 절의 서술 밀도 규칙).
...
템플릿의 `## 위키` 절은 **자리만 있고 경로는 비어 있다**. 이 스킬은 위키에 등록하지 않는다 — 등록은 `pjc:plan-feature` Step 1의 자동 등록 소관이고, 여기서 등록까지 하면 두 스킬 사이에 순서 의존이 생긴다.
...
3. **vault 없음·경로 부재** → `## 위키` 절을 **생성물에서 제거**하고 저장 후 1줄 보고한다: *"위키 vault가 없어 `## 위키` 절은 넣지 않았습니다 — 설정 후 `pjc:plan-feature`가 자동 등록합니다."* (없는 곳을 가리키는 포인터를 남기면 다음 세션이 그것을 찾다 실패한다.)
