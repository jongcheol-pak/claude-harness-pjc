...
6. **프로젝트 정보 절을 만들지 않는다.** 프로젝트 성격·기술 스택·디렉터리 구조·**아키텍처 상세**는 **위키가 정본**이므로 생성물에 `## Stack`·`## Repository Structure` 같은 절을 새로 만들지 않는다(템플릿에도 없다). **무엇을 담고 무엇을 담지 않는지의 정본은 `docs/harness-conventions.md`의 「AGENTS.md 내용 경계」다** — 이 스킬은 그 표를 재서술하지 않는다.
