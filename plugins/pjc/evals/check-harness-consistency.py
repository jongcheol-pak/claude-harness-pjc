#!/usr/bin/env python3
r"""하니스 전역 정합 셀프체크 — 문서가 서로 어긋나는 것을 축 19개로 잰다.
...
# 근거 게이트(③)가 보는 둘. 머리 정규식을 `MARKER_HEAD_RX` 와 나눠 두는 이유는 그쪽이
#   **네 마커를 함께** 잡는 서식 검사용이라, 여기서 재사용하면 `[등재]`·`[미등재]` 까지
#   근거 필드를 요구하게 되기 때문이다(그 둘의 근거는 대장 항목의 `(실해: …)` 가 맡는다).
NEXT_ROUND_HEAD_RX = re.compile(r"^-\s*`?\[다음 회차\]")
EVIDENCE_FIELD_RX = re.compile(r"\(근거:\s*\S")
...
def check_ledger_marker_sync():
    ...
    **근거 게이트(③)가 같은 축에 있는 이유**: 서식 게이트와 같다 — 「마커가 주장하는 것이
    실재하는가」를 재는 축인데, `[다음 회차]` 의 주장(*"다음 세션이 곧바로 착수한다"*)은
    대장에 대조할 상대가 없어 ② 가 닿지 못한다. 그 자리를 근거 필드의 **존재**로 대신
    막는다. **새 축으로 가르지 않은 것은 축 수 표현이 docstring·README·계수 정합 3곳으로
    밀리기 때문**이고, 회차 81 이 같은 이유로 축 ⑩ 확장을 택했다.
    ...
    if sec:
        for line in sec.group(1).splitlines():
            if not NEXT_ROUND_HEAD_RX.match(line):
                continue
            n += 1
            if not EVIDENCE_FIELD_RX.search(line):
                issues.append(
                    "`[다음 회차]` 에 근거 필드 없음: %s — `(근거: <커밋 sha|실측 출력 요지>)`"
                    " 를 붙여라. 이 마커는 「다음 세션이 곧바로 착수한다」는 주장이고, 근거가"
                    " 없으면 이미 끝난 일을 미완으로 읽는다"
                    " (형식 정본은 `plan/references/deferred-rules.md`)" % line.strip()[:70])
...
#   기재 의무·서식도 같은 절이 정본이다 — 배포 `plan-template.md` 는 일반 훅(「착수 전
#   검토 항목」)만 갖는다. 배포물이라 이 레포의 검사기를 전제할 수 없어서다.
...
#  꼬리의 `-->` 는 표식을 감싼 주석의 것이라 사유에서 떼어내고, **길이를 120자로 끊는다** —
