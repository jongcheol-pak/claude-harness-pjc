...
# --- 산문 경로 축 (회차 65) ---------------------------------------------------
# 전체 `*.md` 로 넓히지 않는다 — 후보 706 중 미실존 191 이 나오는데(회차 65 실측) 그 191 은
#   결함이 아니라 다른 성격의 경로다: 위키 vault 경로 · lint 픽스처의 예시 소스. 각각을
#   가르려면 문서 종류별 기준이 필요하고, 그것은 이 축이 아니라 별개 축이다.
# **늘릴 때는 그 문서 하나의 오탐율을 먼저 재고 10% 미만일 때만 넣는다.** 회차 67 이 후보
#   여덟을 재서 전부 그 문턱을 넘겼다(문서별 착수 오탐율 — WIKI 0.0% · BUDGET 3.4% ·
#   golden-runner 3.7% · harness-consistency-rationale 3.7% · AGENTS 6.0% · AUTHORING 7.1% ·
#   DESIGN 7.7% · README 9.5%). 그때 남은 오탐 14 중 3 은 아래 구조 규칙 둘이, 11 은 **문서에서
#   백틱을 벗겨** 닫았다 — 면제 목록을 만들지 않는 것이 이 축의 설계다.
PROSE_TARGETS = (
    'docs/harness-conventions.md',
    'AGENTS.md',
    'README.md',
    'docs/golden-runner.md',
    'plugins/pjc/skills/DESIGN.md',
    'plugins/pjc/skills/AUTHORING.md',
...
# 문자셋을 통과하지만 경로가 아닌 둘 — **구조 규칙이지 토큰 목록이 아니다**(회차 67).
#   ⓐ 분수·비율 — 골든 문서가 경계값을 `29/30`·`13/14` 로 적는다. 슬래시로 이어진 숫자뿐이라
#     경로일 수 없다. ⓑ URL — 첫 세그먼트가 도메인이면 레포 밖이라 실존을 물을 상대가 없다.
#   이 둘이 회차 67 확대 시점의 오탐 14 중 3 을 닫았고, 나머지 11 은 문서 쪽을 고쳐 닫았다.
...
def prose_candidates(text):
    ...
    for t in sorted(set(PROSE_TOKEN_RX.findall("\n".join(prose_lines(text))))):
        ...
        if PROSE_NUMERIC_RX.match(t) or PROSE_URL_RX.match(t):
            continue   # 분수·비율과 URL — 슬래시를 담을 뿐 경로가 아니다
        out.append(t)
    return out
