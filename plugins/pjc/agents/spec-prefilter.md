...
- **이 필터에서는 큰 `--stat`이 그 자체로 ESCALATE 신호이기도 하다** — 대상이 Type B(단일 파일·호출자 변경 없음)인데 여러 파일이나 큰 증감이 보이면 **Type 분류가 실제와 어긋난 것**이므로, 나눠 읽어 판정하려 들기 전에 그 사실을 근거로 escalate하는 편이 옳다(예산 8 turn은 애초에 큰 diff를 볼 몫이 아니다). 위 규칙을 대체하는 것이 아니라 **이 단계에만 더해지는 판정**이다.
