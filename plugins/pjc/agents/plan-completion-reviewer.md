...
**이 호출의 도구 호출 예산은 36회다**(`maxTurns: 40` − 4 — 판정문을 쓸 몫을 남긴 값. **이 상한은 v1.179.0에서도 40을 유지했다**: 전체 plan × 누적 diff 대조라 검토량이 가장 커 처음부터 다른 셋보다 높게(40 vs 30) 잡혀 있었고, 이번에 그 셋을 36~40으로 올린 것이 **이 값에 맞춘 것**이다 — 기준이 된 쪽을 다시 올릴 근거가 없다). **예산을 세는 것은 당신 자신이다.**
...
**Follow-up 항목에 사유(`기존 문제`/`범위 밖`/`개선 제안`)가 없음 → MAJOR**: 그 셋은 규칙 4-1이 이연 가능하다고 정한 전부이므로, 사유를 붙일 수 없는 항목은 이연 대상이 아니라 자기 유발이다 — 표기 누락은 아래 판정을 회피하는 경로가 된다.

**자기 유발 결함의 이연 → BLOCKER (완료 선언 자격)**: Follow-up·Deferred 항목 중 **이번 diff가 원인인 것**(회귀·미갱신 호출부·어긋난 문서/규약·무효화된 기존 동작)이 남아 있으면 **BLOCKER**다 — 고치지 않은 것을 완료라 부르는 것이므로(implement-task 규칙 4-1). 이연이 정당한 것은 셋뿐이다: **기존 문제**(이번 변경과 인과 없음) · **범위 밖**(사용자가 명시적으로 제외) · **개선 제안**(MINOR·SUGGEST 등 결함이 아닌 것).

- **근거 지목 기준을 그대로 적용한다**(자기 유발 판정의 정본은 `implement-task` 규칙 4-1) — diff의 **어느 변경이** 그 항목을 유발했는지 파일:라인으로 지목할 수 있을 때만 보고한다. 인과를 대지 못하면 보고하지 않는다(무관한 follow-up을 자기 유발로 몰면 완료가 부당하게 막힌다).
