...
- 또는 변경 파일 목록
- plan.md의 **`## 시각 요소 분해` 섹션** (디자인 정합 작업일 때만 — 항목 I 입력. 섹션이 없는 plan에서는 전달되지 않으며 항목 I도 skip)
...
- 이 항목은 **코드 레벨 대조만** 한다. 리뷰어는 화면을 볼 수 없으므로 "실제 렌더 결과가 디자인과 같은가"는 판정 대상이 아니다(그것은 V-9의 렌더 확인·F-8의 사용자 확인 몫).
