...
- **문구 불일치**(의역·축약·재작성)와 **컨트롤 타입 대체**(ComboBox → TextBox 등)는 **MAJOR**다. 이 둘은 **거짓양성 억제표의 MINOR 강등 대상이 아니다** — 원문과 다른 문자열, 원본과 다른 타입은 해석 여지가 없는 사실 불일치이며, 강등해 흘려보내면 "대충 비슷한" 구현이 통과한다. "기능적으로 동등하다"는 판정 근거가 되지 못한다.
