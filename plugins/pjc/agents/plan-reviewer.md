...
- [ ] **Step 6.5 Halt Forecast의 각 항목이 plan에서 사전 해소(결정)되었거나 불가피한 Halt로 명시되었는가?** 둘 다 아닌 미해소 예측 halt가 있으면 **MAJOR** (자율 루프 중단·추측 유발). 단 불가피한 Halt(파괴적·새 의존성·승인 필수 등)를 자동 실행으로 바꾸라는 뜻이 아니다 — "명시"만 요구한다(안전 게이트 보존).
