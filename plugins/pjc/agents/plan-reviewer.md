...
- **상호 모순(동시 만족 불가) → BLOCKER**: 한 task의 acceptance 항목들이 **동시에 만족 가능한지** 자문한다. 하나를 지키면 다른 하나가 깨지는 조합은 실현 불가능한 계획이다 — 예: "① PowerShell 프로세스 1개만 기동"과 "⑤ 개별 스크립트 무수정"이 한 task에 있는데 스크립트 구조상 무수정으로는 단일 프로세스 실행이 불가한 경우(실측 사례: 이 모순이 계획 검토를 통과해 구현 착수 후에야 드러났고 task가 스킵·이관됐다). 모순 지적에도 confidence 문턱을 그대로 적용한다 — 구조적 근거(코드·파일에서 확인한 양립 불가 이유)를 지목할 수 있을 때만 보고하고, 추측성 모순 지적은 하지 않는다.
...
- **BLOCKER 후보 강등 금지 예외.** **호출자 누락(항목 3)·acceptance 미충족급**의 안전·정합 직결 결함은, confidence가 50-79라도 **MINOR로 강등하지 않는다** — 심각도를 유지하되 `(confidence: N — 확인 요청)`으로 **불확실성을 표시**해 메인이 확인하게 한다. 이런 결함은 강등해 흘려보내면 빌드·런타임 실패로 직결되므로, 거짓양성 억제보다 누락 방지가 우선이다(강등은 취향·품질 지적에만 적용). confidence<50이면 근거 부족이므로 보고하지 않는 것은 동일.
