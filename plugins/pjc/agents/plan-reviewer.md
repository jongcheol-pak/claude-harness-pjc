...
- [ ] **Step 6.5 Halt Forecast의 각 항목이 (i) 사전 해소(결정) / (ii-a) `## 사전 승인 항목 (일괄 승인 대상)` 등록 / (ii-b) `## 불가피한 Halt (위임 불가)` 명시 중 하나로 분류되었는가?** 셋 다 아닌 미분류 예측 halt가 있으면 **MAJOR** (자율 루프 중단·추측 유발). **파괴적 작업·외부/비가역(push·병합·태그·릴리즈·PR)·인증정보 필요 신규 외부 서비스가 (ii-a)/사전 승인 항목에 잘못 들어가 있으면 MAJOR** (안전 게이트 약화 — 이들은 항상 (ii-b)). (ii-b) 위임 불가 항목을 자동 실행으로 바꾸라는 뜻이 아니다 — "명시"만 요구한다(안전 게이트 보존). (ii-a) 사전 승인 항목은 plan 승인 시 일괄 위임되는 정상 항목이다.
