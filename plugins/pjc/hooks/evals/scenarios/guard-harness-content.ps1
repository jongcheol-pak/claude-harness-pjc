# scenarios/guard-harness.ps1 — guard-harness-content 시나리오 (AGENTS.md 내용 경계 게이트) (dot-source 전용, 단독 실행 금지)
...
Assert-Case -Name "guard-harness-content: 진행 상태 헤딩 차단" -R $r -ExpectExit 2 -ExpectContains 'AGENTS.md 에 plan 진행 상태·세션 인계 서술을 넣으려 합니다'

$r = Invoke-Hook 'guard-harness.ps1' (New-GacWrite $agents "# A`n`n**다음 작업**: T7 진행`n")
Assert-Case -Name "guard-harness-content: '다음 작업' 볼드 제목 차단" -R $r -ExpectExit 2

$r = Invoke-Hook 'guard-harness.ps1' (New-GacWrite $agents "# A`n`n> **part5가 남긴 것 — 다음 회차가 알아야 할 것**`n")
Assert-Case -Name "guard-harness-content: '남긴 것/다음 회차' 인용 볼드 차단" -R $r -ExpectExit 2

# ---- 차단 양성 (2) 디렉터리 트리 블록 ----
$tree = "# A`n`n``````" + "`n├── src/`n│   └── app/`n└── docs/`n" + "``````" + "`n"
$r = Invoke-Hook 'guard-harness.ps1' (New-GacWrite $agents $tree)
Assert-Case -Name "guard-harness-content: 디렉터리 트리 차단" -R $r -ExpectExit 2 -ExpectContains 'AGENTS.md 에 디렉터리 트리를 넣으려 합니다'

# Edit 도구 경로도 같은 판정 (new_string 검사)
$r = Invoke-Hook 'guard-harness.ps1' (New-GacEdit $agents "├── src/`n│   └── app/`n└── docs/`n")
Assert-Case -Name "guard-harness-content: Edit new_string 트리 차단" -R $r -ExpectExit 2

# ---- 델타 음성 (오차단 0 실증) ----
$r = Invoke-Hook 'guard-harness.ps1' (New-GacWrite $agents "# A`n`n## Build & Test`n- Build: ``dotnet build```n`n## DO NOT`n- 시크릿 커밋`n")
Assert-Case -Name "guard-harness-content: 정상 AGENTS.md 통과" -R $r -ExpectExit 0 -ExpectSilent $true

$r = Invoke-Hook 'guard-harness.ps1' (New-GacWrite "$gacFwd/plan.md" "## Next Steps`n`n**다음 작업**: T7 진행`n")
Assert-Case -Name "guard-harness-content: plan.md의 같은 문구 통과 (대상 파일 한정)" -R $r -ExpectExit 0 -ExpectSilent $true

$r = Invoke-Hook 'guard-harness.ps1' (New-GacWrite $agents "# A`n`n## Conventions`n- 커밋 전 빌드하고 다음 작업으로 넘어간다.`n")
Assert-Case -Name "guard-harness-content: 본문 산문의 '다음 작업' 스침 통과 (헤딩 아님)" -R $r -ExpectExit 0 -ExpectSilent $true

$r = Invoke-Hook 'guard-harness.ps1' (New-GacWrite $agents "# A`n`n## Build & Test`n- ``ls | grep x```n- ``cat a | wc -l```n")
Assert-Case -Name "guard-harness-content: 명령 예시 파이프 통과 (트리 오탐 방지)" -R $r -ExpectExit 0 -ExpectSilent $true

$r = Invoke-Hook 'guard-harness.ps1' (New-GacWrite $agents "# A`n`n| 항목 | 값 |`n|---|---|`n| a | b |`n| c | d |`n")
Assert-Case -Name "guard-harness-content: 마크다운 표 통과 (트리 오탐 방지)" -R $r -ExpectExit 0 -ExpectSilent $true

$r = Invoke-Hook 'guard-harness.ps1' (New-GacWrite "$gacFwd/AGENTS-old.md" "## 현재 진행 상태`n")
Assert-Case -Name "guard-harness-content: 파일명 불일치(AGENTS-old.md) 통과" -R $r -ExpectExit 0 -ExpectSilent $true
...
Assert-Case -Name "guard-harness-content: 하위 경로 AGENTS.md 도 차단 (basename 판정)" -R $r -ExpectExit 2

# ---- 우회 변수 (guard-write의 AGENTS 게이트와 같은 CLAUDE_HARNESS_QUICK) ----
...
try {
    ...
    Assert-Case -Name "guard-harness-content: CLAUDE_HARNESS_QUICK=1 우회" -R $r -ExpectExit 0
} finally {
    $env:CLAUDE_HARNESS_QUICK = $savedQuick
}

# ---- guard-harness 공존 (같은 matcher에 직렬로 걸린다) ----
# ⓐ 정상 AGENTS.md Write — 두 hook 모두 통과해야 한다.
$r = Invoke-Hook 'guard-harness.ps1' (New-GacWrite $agents "# A`n`n## Build & Test`n- Build: x`n")
Assert-Case -Name "guard-harness-content: 공존ⓐ guard-harness도 정상 AGENTS.md 통과" -R $r -ExpectExit 0 -ExpectSilent $true
# ⓑ 설치본 hook 경로 Write — guard-harness가 차단하고, 신규 hook은 그 판정을 가리지 않는다(파일명 불일치로 통과).
...
Assert-Case -Name "guard-harness-content: 공존ⓑ 설치본 hook 경로는 guard-harness가 차단" -R $r -ExpectExit 2
...
Assert-Case -Name "guard-harness-content: JSON 파싱 실패 fail-open" -R $r -ExpectExit 0 -ExpectSilent $true
$r = Invoke-Hook 'guard-harness.ps1' (@{ tool_name = 'Write'; cwd = $gac; tool_input = @{ file_path = $agents } } | ConvertTo-Json -Compress)
Assert-Case -Name "guard-harness-content: content 없음 통과" -R $r -ExpectExit 0 -ExpectSilent $true
}
