# scenarios/guard-harness-installed.ps1 — guard-harness 설치본 캐시 경로 개조 차단 시나리오 (이름 집합 합류 실증) (dot-source 전용, 단독 실행 금지)
...
# ---- [T1] guard-harness: 신규 hook(warn-version-drift) 설치본 개조 차단 (이름 집합 합류 실증) ----
...
Assert-Case -Name "guard-harness: 설치본 warn-version-drift 개조 차단" -R $r -ExpectExit 2 -ExpectContains 'BLOCKED'
...
Assert-Case -Name "guard-harness: 설치본 session-context 개조 차단" -R $r -ExpectExit 2 -ExpectContains 'BLOCKED'
}   # ---- 게이트 끝 (guard-harness) ----
