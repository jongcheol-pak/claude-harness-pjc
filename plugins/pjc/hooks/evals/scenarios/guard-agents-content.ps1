...
# =====================================================================
if (Test-HookSelected @('guard-agents-content')) {
...
$r = Invoke-Hook 'guard-agents-content.ps1' (New-GacWrite $agents "# A`n`n## 현재 진행 상태`n`n| 단계 | 상태 |`n|---|---|`n| M5 | 진행 중 |`n")
Assert-Case -Name "guard-agents-content: 진행 상태 헤딩 차단" -R $r -ExpectExit 2 -ExpectContains '인계'

$r = Invoke-Hook 'guard-agents-content.ps1' (New-GacWrite $agents "# A`n`n**다음 작업**: T7 진행`n")
Assert-Case -Name "guard-agents-content: '다음 작업' 볼드 제목 차단" -R $r -ExpectExit 2

$r = Invoke-Hook 'guard-agents-content.ps1' (New-GacWrite $agents "# A`n`n> **part5가 남긴 것 — 다음 회차가 알아야 할 것**`n")
Assert-Case -Name "guard-agents-content: '남긴 것/다음 회차' 인용 볼드 차단" -R $r -ExpectExit 2

# ---- 차단 양성 (2) 디렉터리 트리 블록 ----
$tree = "# A`n`n``````" + "`n├── src/`n│   └── app/`n└── docs/`n" + "``````" + "`n"
$r = Invoke-Hook 'guard-agents-content.ps1' (New-GacWrite $agents $tree)
Assert-Case -Name "guard-agents-content: 디렉터리 트리 차단" -R $r -ExpectExit 2 -ExpectContains '트리'

# Edit 도구 경로도 같은 판정 (new_string 검사)
$r = Invoke-Hook 'guard-agents-content.ps1' (New-GacEdit $agents "├── src/`n│   └── app/`n└── docs/`n")
Assert-Case -Name "guard-agents-content: Edit new_string 트리 차단" -R $r -ExpectExit 2

# ---- 델타 음성 (오차단 0 실증) ----
$r = Invoke-Hook 'guard-agents-content.ps1' (New-GacWrite $agents "# A`n`n## Build & Test`n- Build: ``dotnet build```n`n## DO NOT`n- 시크릿 커밋`n")
Assert-Case -Name "guard-agents-content: 정상 AGENTS.md 통과" -R $r -ExpectExit 0 -ExpectSilent $true

$r = Invoke-Hook 'guard-agents-content.ps1' (New-GacWrite "$gacFwd/plan.md" "## Next Steps`n`n**다음 작업**: T7 진행`n")
Assert-Case -Name "guard-agents-content: plan.md의 같은 문구 통과 (대상 파일 한정)" -R $r -ExpectExit 0 -ExpectSilent $true

$r = Invoke-Hook 'guard-agents-content.ps1' (New-GacWrite $agents "# A`n`n## Conventions`n- 커밋 전 빌드하고 다음 작업으로 넘어간다.`n")
Assert-Case -Name "guard-agents-content: 본문 산문의 '다음 작업' 스침 통과 (헤딩 아님)" -R $r -ExpectExit 0 -ExpectSilent $true

$r = Invoke-Hook 'guard-agents-content.ps1' (New-GacWrite $agents "# A`n`n## Build & Test`n- ``ls | grep x```n- ``cat a | wc -l```n")
Assert-Case -Name "guard-agents-content: 명령 예시 파이프 통과 (트리 오탐 방지)" -R $r -ExpectExit 0 -ExpectSilent $true

$r = Invoke-Hook 'guard-agents-content.ps1' (New-GacWrite $agents "# A`n`n| 항목 | 값 |`n|---|---|`n| a | b |`n| c | d |`n")
...
try {
    ...
    $r = Invoke-Hook 'guard-agents-content.ps1' (New-GacWrite $agents "# A`n`n## 현재 진행 상태`n")
...
$r = Invoke-Hook 'protect-harness.ps1' (New-GacWrite $agents "# A`n`n## Build & Test`n- Build: x`n")
...
$r = Invoke-Hook 'protect-harness.ps1' (New-GacWrite $installed "x")
Assert-Case -Name "guard-agents-content: 공존ⓑ 설치본 hook 경로는 protect-harness가 차단" -R $r -ExpectExit 2
$r = Invoke-Hook 'guard-agents-content.ps1' (New-GacWrite $installed "x")
Assert-Case -Name "guard-agents-content: 공존ⓑ 같은 경로를 신규 hook은 통과(대상 아님)" -R $r -ExpectExit 0 -ExpectSilent $true

# ---- fail-open ----
$r = Invoke-Hook 'guard-agents-content.ps1' 'not-a-json'
Assert-Case -Name "guard-agents-content: JSON 파싱 실패 fail-open" -R $r -ExpectExit 0 -ExpectSilent $true
$r = Invoke-Hook 'guard-agents-content.ps1' (@{ tool_name = 'Write'; cwd = $gac; tool_input = @{ file_path = $agents } } | ConvertTo-Json -Compress)
