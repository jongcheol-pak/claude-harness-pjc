...
#   - **파일명(basename)이 정확히 AGENTS.md 인 것만** 본다 — 경로는 보지 않으므로 하위 디렉터리의
#     AGENTS.md(모노레포의 `packages/x/AGENTS.md` 등)도 같은 경계 정책을 받는다. 내용 경계는 파일의
#     위치가 아니라 **그 파일이 무엇인가**에 딸린 규약이기 때문이다.
#     `AGENTS-old.md`·`AGENTS.template.md` 처럼 basename 이 다른 것은 비대상.
