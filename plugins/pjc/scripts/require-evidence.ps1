...
if ($data.stop_hook_active -ne $true -and $env:CLAUDE_HARNESS_QUICK -ne '1') {
    ...
    #   ⓐ 세션·컨텍스트 명사 → ⓑ 전환·판단 위임 어미. 명사 단독으로 잡지 않는 이유:
    #   "plan.md의 컨텍스트를 다시 읽었습니다"처럼 **규칙 4를 수행하는 정상 보고**에도
    #   그 명사가 나오므로, 단독 매치는 정답 행동을 차단한다.
    $rxHandoff = '(새\s*세션|/clear|컨텍스트|대화가\s*길)[^\r\n]{0,40}(옮길지|옮길까|계속할지|새로\s*시작하시|권합니다|알려주세요|이어가시)'
    ...
    #   미탐(2인칭 지칭형·어순 역전)은 의도적으로 수용한다 — 1차 방어선은 문서 규칙이고
    #   오차단은 즉시 피해인 반면 미탐은 규칙이 받는다.
    $rxManualAsk = '(직접|한번|한 번|여기서|대신)[^\r\n]{0,25}(실행|테스트|확인|돌려|띄워|구동)[^\r\n]{0,25}(보시|주시|주세요|부탁|하시겠|해\s*보세요)'
