...
function Get-LedgerSignal {
    param([string]$cwd)
    $ledgerLine = $null
            # ---- Deferred 대장 최고령 「마지막 판정일」 — 근거는 `rules/session-context-rationale-plan.md`의 「§12 ---- Deferred 대장 최고령 「마지막 판정일」」
            try {
                ...
                if ((Test-Path -LiteralPath $ledgerPluginJson -PathType Leaf) -and (Test-Path -LiteralPath $ledgerPath -PathType Leaf)) {
                    ...
                    # 종료 앵커 `^## `는 현재 대장에 `## 대기` 하나뿐이라 실질적으로 `\z`로 끝난다. — 근거는 `rules/session-context-rationale-plan.md`의 「§13 종료 앵커 `^## `는 현재 대장에 `## 대기` 하나뿐이라 실질적으로 `\z`로 끝난다.」
                    ...
                    if ($ledgerWait.Success) {
                        ...
                        # vN 부기는 형식이 둘이다 — ⓐ 날짜 접두 — 근거는 `rules/session-context-rationale-plan.md`의 「§14 vN 부기는 형식이 둘이다 — ⓐ 날짜 접두」
                        $ledgerVnPrefixRx = [regex]'^- \[\d{4}-\d{2}-\d{2}, '
                        $ledgerVnTailRx = [regex]'\(v\d+\.\d+\.\d+[^)]*해소\)'
                        # 항목이 아닌 구조 줄 — 앵커 — 근거는 `rules/session-context-rationale-plan.md`의 「§15 항목이 아닌 구조 줄 — 앵커」
                        ...
                        if ($ledgerItems.Count -gt 0 -and $ledgerJudged.Count -gt 0) {
                            ...
                            # 이 라인은 **vault 게이팅 신호가 아니다** — 근거는 `rules/session-context-rationale-plan.md`의 「§16 이 라인은 **vault 게이팅 신호가 아니다**」
                            }
                    }
                }
            } catch {}
