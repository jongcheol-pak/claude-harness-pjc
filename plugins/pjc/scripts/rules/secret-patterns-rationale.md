...
**회차 44 — 차단 등급 두 라벨에도 같은 술어를 건다.** 「DB 연결 문자열」·「DB/서비스 URI 인증정보」는 값 검사 없이 키 존재(`Password=` · `user:pass@`)만 봐서, `Server=…;` 뒤 `Password=` 에 `${DB_PASSWORD}` 를 적은 연결 문자열 · `postgres://app:` 뒤 비밀번호 자리에 `${DB_PASS}` 를 적은 URI 같은 **이 hook 이 권하는 참조형**이 차단됐다(예시를 조각으로 적는 이유는 이 문서 자신이 그 게이트에 걸리기 때문이다)(회차 44 검토 실측 — 차단 메시지대로 고쳐도 다시 차단). 그래서 §6 의 술어를 `Test-ReferenceValue` 로 묶어 두 라벨의 **값 자리**(연결 문자열은 `Password=` 또는 `Pwd=` 뒤 `;` 앞, URI 는 `:` 뒤 `@` 앞)에 적용한다. 키가 `User Id` 뿐인 연결 문자열은 값이 없으므로 종전대로 라벨을 낸다. 트레이드오프는 §6 과 같다 — `$` 로 시작하는 실값은 함께 미탐이며, 그 희생은 골든 음성(참조형 통과)과 양성(평문 차단 유지)으로 보이게 둔다.
