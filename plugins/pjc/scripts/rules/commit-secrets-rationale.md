...
**회차 44 — `.env.example`·`.env.sample` 은 `.env` 스테이징 경고 대상에서 뺀다.** 그 둘은 값이 아니라 템플릿이고, `guard-write.ps1` 이 같은 두 이름을 plan 없이 허용한다 — 커밋 게이트만 경고하면 두 hook 의 판정이 갈린다(검토 실측: `.env.example` 스테이징이 `.env 파일 스테이징` 경고를 냈다).
...
## §3a 커밋 판정에서 heredoc 본문을 빼는 이유

**실해**: 2026-09-10 회차 55 에서 위키 큐에 커밋 규약을 적으려고 그 문구를 본문에 담은 `python3 - <<PY` 호출이 **커밋으로 판정**돼 50파일 상한 차단에 걸렸다. 실제 커밋이 아니었고 스테이징에는 무관한 파일이 쌓여 있었다. 차단 사유가 「커밋 파일이 많다」로 나와 **원인(오탐)이 보이지 않았다.**

**처방(회차 57 — 부분해였다)**: 판정용 문자열만 `Remove-HeredocDataSink` 를 통과시켰다. 그러나 **그 헬퍼는 설계상 data sink(`cat >`·`tee`)만 본문을 지운다** — 위 실해의 실제 형태인 `python3 - <<PY` 는 그대로 남아 오탐이 계속됐다. 큐 항목이 제안한 처방이 부분해였음이 **착수해서야** 드러난 자리다.

**처방(회차 58 — 현행)**: `Remove-HeredocBodyForJudge` 를 따로 두고 판정용 문자열을 그쪽으로 통과시킨다. **`Remove-HeredocDataSink` 는 건드리지 않는다** — 두 함수의 판정 축이 다르기 때문이다:

| 함수 | 묻는 것 | 태그 종류를 보는가 |
|---|---|---|
| `Remove-HeredocDataSink` | 본문이 **파일로 가는가**(실행 판정) | **본다** — `python3 - <<PY` 본문은 실행되므로 남겨야 미탐이 없다 |
| `Remove-HeredocBodyForJudge` | 이 명령이 **커밋인가**(커밋 판정) | **안 본다** — 본문이 데이터든 코드든 그 자리에서 `git` 이 도는 것이 아니다 |

한 함수에 플래그를 더해 둘을 겸하게 하면 두 의미가 한 몸이 돼 다음 수정이 한쪽을 깬다.

**두 호출부를 같은 커밋에서 바꾼다** — `guard-commit-secrets` 와 `guard-stale-docs` 가 한 프로세스에서 **같은 `$cmd`** 를 본다. 회차 57 이 한쪽만 고쳐 「한쪽만 도는 조합」이 실제로 생겼고 완료 리뷰가 MAJOR 로 잡았다. 골든도 양쪽에 각각 둔다(HD3·HD4 와 `guard-stale-docs` 5d·5e).

**스캔 대상은 바꾸지 않는다** — 판정(「이것이 커밋인가」)과 스캔(「스테이징 diff 에 시크릿이 있는가」)은 다른 입력을 쓴다. 후자는 `git diff --cached` 라 명령 문자열과 무관하다. **보수측이 뒤집히지 않는다** — heredoc **밖**의 진짜 커밋은 그대로 잡히고, 줄어드는 것은 heredoc **안**의 문자열뿐이다.

**헬퍼 부재 시 원문으로 폴백한다** — 이 파일은 dot-source 전용이고 로드 순서가 바뀔 수 있다. 폴백이 없으면 판정 자체가 사라져 오탐이 아니라 **미탐**이 된다.
