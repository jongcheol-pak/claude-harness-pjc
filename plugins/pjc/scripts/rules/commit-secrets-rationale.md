...
## §1 guard-commit-secrets.ps1 — git commit 직전 스테이징 변경의 시크릿 검사
...
## §2 warn-commit-secrets: git commit 직전 스테이징될 변경에서 시크릿 패턴 경고
...
## §4 # 실패 경고 1회 억제 플래그를 **호출마다 리셋**한다 — hook 프로세스는 도구 호출당 새로 뜨지만
...
## §8 # 스캔 캡은 **추적 파일 분기와 공유한다** — 종전에는 아래 untracked 정독 루프에만
...
## §14 # 우회는 전용 변수로만 — CLAUDE_HARNESS_QUICK을 쓰지 않는다. QUICK은 add-vie
...
## §16 # 저신뢰 검출분·.env 스테이징은 원래 경고 경로로 나가던 정보다 — 캡 차단이 먼저 반환하므로
...
## §17 # 캡 부기는 **이 경로에도** 필요하다 — `$hits`가 있고 우회가 켜져 있으면 위 두 차단 블록을
...
```
## §3 ⚠ `Write-Warning`을 쓰지 않는다 — pwsh 기본 호스트의 Warning 스트림 취급
