...
```
                ...
                # **cwd 트리 판정이 붙는 이유 — 공유가 종전 동작을 바꾸는 자리가 하나 있다.**
                #   섹션 1 은 `git -C $fileDir`, 섹션 2 는 cwd 기준이다. 둘이 다른 레포일 때 종전에는
                #   cwd 에서 `git diff HEAD -- <다른 레포의 파일>` 이 실패해 심볼 0건·침묵이었는데,
                #   공유하면 **파일 레포의 diff 로 뽑은 심볼을 cwd 레포에서 caller 검색**하게 된다.
                #   심볼명이 두 레포에 겹치면 무관한 파일을 가리키는 IMPACT WARNING 이 뜬다.
                #   그래서 파일 경로가 cwd 아래일 때만 공유를 쓰고, 아니면 폴백이 종전 동작을 복원한다.
                #   판정을 문자열 접두로 하는 것은 의도다 — `rev-parse --show-toplevel` 두 번을 부르면
                #   이 회차가 없앤 git 호출 1회가 그대로 되돌아온다.
```
