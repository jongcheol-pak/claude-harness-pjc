...
완료 커밋은 **제목(첫 줄)** 이 `{유형}: T<N> — …`(글로벌 「Git」 의 다섯 유형 · 회차 44 정본) 또는 구형 `T<N>: …` 일 때만 판정한다 — 본문이나 괄호 안의 `T<N>` 언급으로 오탐하지 않기 위해서다.

**종전 정규식은 구형만 받아 사문화돼 있었다** — 최근 40커밋이 전부 신형이라 게이트가 한 번도 발화하지 않았다(회차 44 실측).

`-m "$(cat <<'EOF' … EOF)"` 형태는 **첫 줄이 heredoc 여는 줄이라 제목이 아니다** — 그다음 줄이 제목이다(회차 44 실측 미탐).

## §15 미완료 마커 매칭

미완료 마커 `[ ]`·`[/]` 와 해당 task 번호를 함께 본다. 불릿은 `-`·`*` 둘 다 받고, 템플릿 정본의 하위 항목 `- [ ] **T<N>-<M>**`(볼드)도 받는다.

**T<N> 완료 커밋의 대상은 T<N>-1…T<N>-k 전부라 하나라도 미완료면 차단한다**(회차 44 D2). `T$taskNum(-\d+)?\b` 라 **T1 이 T10 을 오매치하지 않는다.**

## §16 heredoc 본문이 분할을 깨뜨린다

`Split-TopLevel` 은 heredoc 을 모른다 — 그 본문의 `;`·`|` 를 최상위 구분자로 잘라, 쪼개진 조각의 첫 실효 토큰이 heredoc **본문의 단어**가 된다. 그러면 쓰기 동사 판정이 그 자리에서 발화해 **정상 명령이 막힌다**(2026-09-10 실측: `cat <<'EOF' > notes.md` / 본문 `hello; cp notes.md plan.md` / `EOF` 가 `exit 2`. 대상은 `notes.md` 인데 `;` 뒤 조각이 `cp … plan.md` 로 섰다).
