...
- **마커는 텍스트 줄의 시작이어야 한다** — transcript 한 줄은 JSON 객체 하나라 텍스트 내부 개행은
  ...
  허용). 이 규칙을 *설명하는* 산문은 마커로 줄을 시작하지 않으므로 이 조건 하나가 산문 오통과를 닫는다.
...
- **인정하는 것은 assistant 의 `"text"` 필드뿐이다** — user 줄(`"content"`)의 표식은 받지 않는다. 표식의 출처는 `pjc:plan` 이고, 사용자는 `A) 바로 수정` 을 고르는 쪽이지 표식을 내는 쪽이 아니다.
