...
**`MultiEdit` 은 지금 모델에 제공되지 않는다 — 이 분기와 §12 의 같은 분기는 도달 불가다**(2026-09-17 실측 · Claude Code v2.1.274). 근거 셋: 공식 hooks 문서의 매처 목록이 `Bash`·`Edit`·`Write`·`Bash|PowerShell` 만 열거하고 · 세션에 실리는 도구 목록에 없으며 · CLI 바이너리의 출현 7건이 전부 **권한 규칙 정규화·표시명 맵·구 트랜스크립트 호환**이고 도구 스키마가 없다(같은 집합에 이미 폐기된 `LS` 가 함께 있다). **그래도 지우지 않는 것은 이 hook 이 `AGENTS.md` 「DO NOT」의 차단 넷 중 하나이기 때문이다** — 그 조항의 허용 범주는 오탐 수정과 미탐 보완 둘뿐이고 「도달 불가 분기 제거」는 어느 쪽도 아니라, 지우려면 규약을 함께 고치고 별도 승인을 받아야 한다(v1.292.0 이 두 안을 재서 현행 유지를 택했다). **되살아날 수 있다는 것도 근거다** — 권한 규칙과 트랜스크립트 호환에 이름이 남아 있어, 분기를 지운 뒤 그 도구가 돌아오면 그때는 우회 경로가 된다. **다시 판정할 때는 위 셋을 같은 순서로 돌린다.**
