...
$hooks = @('block-destructive.ps1', 'protect-harness.ps1', 'require-plan-for-write.ps1', 'require-task-checkbox.ps1', 'post-write-checks.ps1', 'require-evidence.ps1', 'warn-external-ops.ps1', 'suggest-agents-record.ps1', 'warn-commit-secrets.ps1', 'pre-bash-dispatch.ps1', 'warn-version-drift.ps1', 'session-context.ps1')
...
#   래퍼가 공유)·hook-event-log(이벤트 적재) / 수동 실행 도구: report-hook-events(이벤트 로그 집계 리포트).
$knownHelpers = @('secret-patterns.ps1', 'bash-hook-lib.ps1', 'hook-event-log.ps1', 'report-hook-events.ps1')
