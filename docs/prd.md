...
| FR-4 | auto-compact 직후 세션에 **진행 중 plan이 있으면**, 재읽어야 할 스킬 문서·reference 경로를 기계가 지정해 주입한다 | Must | hook 골든 케이스(`source=compact` + 미완료 task 있는 plan) 추가 → 출력에 재읽기 대상 경로가 포함됨을 ExpectContains로 고정 (SC14·SC14b·SC14c). **v1.212.0 강화 — 경로 지시에 더해 루프 제어 두 절의 원문을 주입한다**: 경로만 알려 주면 ⓐ 실제로 읽었는지 검증할 수단이 없고 ⓑ 지목한 3파일 합 172KB를 다시 읽으면 압축으로 확보한 여유를 도로 쓴다. 골든 SC39(SKILL.md 「자율 루프의 절대 규칙」 절)·SC39b(halt-conditions 「중단 조건 표」~「위임 경계」 절)·SC39c(startup 델타 음성)·SC39d(발췌 표기)로 고정하고, 앵커가 깨져 주입이 조용히 사라지는 것은 `check-harness-consistency.py`의 「추출 앵커 도달성」 축이 잡는다. **기존 경로 지시는 대체가 아니라 병존**이므로 위 3케이스는 그대로 유효하다 |
