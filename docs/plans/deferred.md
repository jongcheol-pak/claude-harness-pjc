...
- [2026-07-20] **`warn-commit-secrets`가 시크릿 탐지기 자신의 패턴 정의를 신고하는 오탐** — `lint.py`를 `git add`하자 `[COMMIT SECRET WARNING] password 값`이 2회 발생했으나, 원인은 `lint.py:79`의 **탐지 규칙 정의**(`("password", re.compile(...))`)였다(커밋 diff에 실제 시크릿 0건, grep 실증). 훅이 `git add` 대상의 **전체 파일 내용**을 스캔하는 규약(AGENTS.md:65)의 구조적 부작용이라, 보안 도구·탐지기 소스를 다루는 레포에서 반복된다. 기존 "suggest-agents-record 명령 오인 오탐"과 같은 계열 — 이벤트 로그 축적 후 근거 기반 정비 때 함께. (출처: v1.129.0 T3 구현 중 실관찰)
