...
- **결정: 분리하지 않음 (Q1 = "아니오").** D7 Deferred는 "검토 완료, 현행 유지(K 본체 수록)"로 종결한다. Q2~Q4(분리 범위·대상 파일·:44 문구)는 Q1이 부정이므로 소멸.
- **정량 근거** (2026-07-08 측정, SKILL.md 문자 수 기준):
  - SKILL.md 전체 17,939자 중 K 섹션(105~135행) 7,331자(41%), 그중 5·5-1·5-2 하위 3,658자(20%) — K는 크지만, 크기가 분리의 근거가 되지 못한다:
  - **최빈 경로(코드 세션의 절차 K 자동 참조)는 Skill 호출로 SKILL.md 본체를 어차피 로드**한다. K를 분리하면 그 경로는 "본체 축소분(−7.3K자) ≈ 분리 파일 Read(+7.3K자)"로 **토큰 중립 + Read 1회 지연만 추가** — 현 설계(SKILL.md:44)가 일부러 피한 비용이다.
  - 절감(−7.3K자/세션)은 **드문 위키 운영 세션(A~I·L)에만** 발생한다 — 그 세션들은 이미 절차별 reference를 lazy-load한다.
  - K 5-1(`[SKILL-IMPROVE]`)·5-2(`[DECISION]`)는 "어느 pjc 세션에서든" 발동하는 크로스-스킬 규칙(plan-feature Step 10이 5-2를 직접 참조)이라, 하위만 분리(C안)하면 그 경로들에도 Read가 추가돼 더 불리하다.
  - check_consistency.py 파싱 앵커(라우팅 표 `(이 문서)`·`### K.` 헤딩·산문 크로스파일 포인터) 연쇄 수정 위험을 이득 없이 짊어진다.
- `SKILL.md:44`의 "K를 본체에 둔다(컨텍스트 예산 절감의 핵심)" 설계 주석은 **여전히 유효** — 문구 수정 불필요.
