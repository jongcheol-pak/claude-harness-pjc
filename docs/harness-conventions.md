...
> | **리뷰어** (호출마다 별개) | 리뷰어 **1종**. 4종은 **서로 합산되지 않고 메인과도 합산되지 않는다** — 각 subagent는 자기 정의 파일만 로드한다(같은 문서 「리뷰어 4종 공통 규약」이 단일 소스화 불가의 근거로 쓰는 바로 그 사실) | 29,628 ~ 54,241B |
>
...
| `plugins/pjc/agents/plan-reviewer.md` | 54,285 | 438 | 77 |
