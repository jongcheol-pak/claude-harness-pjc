...
- **`-Resume`이 스킵하지 *않는* 범위**: 상태는 **필터 집합 + HEAD 커밋으로 격리**된다(디렉터리 이름에 각인 — `scope.txt`로 확인 가능). 그래서 **다른 필터로 돌린 판정이나 다른 커밋의 판정은 재사용되지 않는다** — 조건이 다르면 상태가 애초에 보이지 않아 그 그룹을 다시 돌린다. 이 격리가 없던 v1.159.0 개발 중에 **`-Filter`로 2그룹만 돌린 뒤 무필터 `-Resume`을 실행하자 12그룹을 전부 건너뛰고 `112/112 OK (FAIL 0)` exit 0**이 나왔다(419케이스가 조용히 빠졌고 부분 실행 경고도 없었다). 스킵이 일어나면 출력에 `[RESUME] … (스코프 filter=…|head=…)`가 찍히므로 재사용 사실과 범위를 그 줄로 확인한다.
