...
> |---|---|---|
> | **메인** | 작업 SKILL(`implement-task` **또는** `plan-feature`) **+ `llm-wiki`** — 절차 K(계획·구현 중 read-only 참조)·절차 M(F-6.5 큐 소비)로 **자동 호출**되므로 코드 작업 세션에서 함께 로드된다 | **최대 162,130B** (104,303 + 57,827 — 아래 표의 실측 합). 한 세션에서 계획 → 구현을 연달아 하면 `plan-feature` 89,509B가 더해진다. **이 세 수치는 아래 표에서 옮겨 적은 것이라 기계 대조 대상이 아니다** — 표를 갱신하는 task가 이 줄도 함께 고친다(v1.186.0 전까지 실제로 어긋나 있었다) |
...
| `plugins/pjc/skills/llm-wiki/SKILL.md` | 57,827 | 188 | 80 |
...
- **현행 케이스 수 643이 회귀 기준선이다 — 케이스를 추가·삭제하면 이 값을 함께 갱신할 것.** (2026-08-15 무인자 전체 실행 실측 `535/535 OK (FAIL 0)`. 직전 기록값 528은 그 사이 늘어난 케이스가 반영되지 않아 낡은 값이었다 — `docs/plans/deferred.md`가 535로 적고 있었던 것이 맞다.) **(2026-08-18 재실측 `539/539 OK (FAIL 0)` — v1.180.0이 SessionStart `fork` matcher 케이스 4건(SC2b·SC2c·SC22b·SC22c)을 더했다.)** **(2026-08-19 재실측 `596/596 OK (FAIL 0)` — v1.182.0이 57건을 더했다: secret-patterns 라벨 판정 24건 · block-destructive 8건 · `.state` TTL 2건 · `$autoStage` 호출 횟수 8건 · 폴백 경고 가드 3건 · 그 밖 회귀 케이스. 그중 **오차단을 고정하는 델타 음성이 다수**다 — 그 자리가 비어 있던 것이 F-7 BLOCKER 2회의 원인이었다.)** **(2026-08-21 재실측 `643/643 OK (FAIL 0)` — 자기보호 이름 집합에 고아 프로세스 회수 계열 2종을 합류시키며 8건을 더했다: protect-harness 양성 3(리터럴 2 + 8.3 마스킹 1) · **델타 음성 3**(개발 repo · 유사 이름 · 캐시 밖 CLAUDE~1) · post-write H2 탐지 양성 2.)** 이 숫자는 「문서 로드 예산 기준선」처럼 기계 대조되지 않으므로(그 표는 스킬·리뷰어 파일 바이트만 추적한다) **문서를 갱신하지 않으면 드리프트를 막을 장치가 없다** — v1.182.0이 실제로 이 줄을 지나쳐 F-7 2R에 잡혔다.
