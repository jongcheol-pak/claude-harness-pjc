...
>
> **`AGENTS.md` 행만 성격이 다르다 — 상한 대비도 함께 잰다.** 나머지 8행은 「표 기록값 == 실측」만 보지만, 이 파일은 SessionStart가 **전문을 주입할지 목차 폴백으로 내려갈지**가 `session-context.ps1`의 `$agentsMaxBytes`로 갈린다 — 넘기면 **모든 세션이 보는 내용이 통째로 달라지는데** 그것을 알려 주는 장치가 없었다(대장 `[2026-08-19]`). 그래서 검사기가 그 상한을 **hook에서 파싱해** 초과 시 FAIL한다(값을 코드에 박지 않는다). **이 행의 「9,000B 경계 행」 값은 판정 의미가 없다** — 그 열은 *"auto-compact 후 **스킬**이 앞 5,000토큰만 재부착된다"*는 사양에서 온 것인데 `AGENTS.md`는 스킬이 아니라 주입 텍스트다(형식 유지를 위한 실측치일 뿐이다).
>
