...
> |---|---|---|
> | **메인** | 작업 SKILL(`implement-task` **또는** `plan-feature`) **+ `llm-wiki`** — 절차 K(계획·구현 중 read-only 참조)·절차 M(F-6.5 큐 소비)로 **자동 호출**되므로 코드 작업 세션에서 함께 로드된다 | **최대 164,596B** (105,863 + 58,733 — 아래 표의 실측 합). 한 세션에서 계획 → 구현을 연달아 하면 `plan-feature` 96,072B가 더해진다. **이 세 수치는 아래 표에서 옮겨 적은 것이라 기계 대조 대상이 아니다** — 표를 갱신하는 task가 이 줄도 함께 고친다(v1.186.0 전까지 실제로 어긋나 있었다) |
...
| `plugins/pjc/skills/implement-task/SKILL.md` | 105,863 | 633 | 66 | 117,000 |
| `plugins/pjc/skills/plan-feature/SKILL.md` | 96,072 | 521 | 83 | 106,000 |
...
| 파일 | 파일 바이트 | 상한 |
|---|---|---|
| `docs/harness-conventions.md` | 102,543 | 137,000 |

> **왜 예산 표와 나눴는가 — 「임계가 없다」가 아니라 「재는 것이 다르다」다.** 위 표가 재는 것은 **상시 로드**다: 스킬 본체는 발동만 해도 전문이 올라가고 `AGENTS.md`는 매 세션 주입된다. 이 표가 재는 것은 **조건부 참조** — `AGENTS.md`가 필요할 때 가리키는 문서라, 커져도 **그 절을 읽으러 들어온 세션만** 비용을 진다. 같은 표에 섞으면 위 표의 정의(*"상시 로드되는 것만 추적한다"*)가 깨지고, 그 표에서 파생되는 「메인 조합」 합산에 **상시 비용이 아닌 바이트가 섞인다**.
>
> **상한 137,000B의 근거.** 상한 137,000B는 **선정 시점 실측 99,412B(2026-08-25)** 대비 여유 37,588B ≈ **10일 창**으로 잡았다(하루 +3,700B 실측 — 2026-08-05 25,241B → 2026-08-25 99,412B, 20일간 +74,171B). **기준값을 문장에 못 박은 것은 의도다** — 이 서술은 *"그때 그렇게 정했다"*는 기록이라 파일이 커져도 낡지 않고, 그래서 **재측정 대상은 위 표의 파일 바이트 하나**로 좁혀진다.
>
> **여유를 퍼센트가 아니라 「경보 창 일수」로 잡은 이유.** 위 예산 표의 `+10%`도 **근거 자체는 경보 창**이다(*"활성 파일 기준 약 6.5~12일"*). 같은 +10%를 이 파일에 적용하면 여유가 9,941B ≈ **2.9일**이라, **같은 규칙이 전혀 다른 의미**가 된다 — 이 파일이 훨씬 빠르게 자라기 때문이다. 그래서 이 절은 **같은 원칙(경보 창)을 일수로 직접 적용**한다. 두 표가 다른 공식을 쓰는 것이 아니다.
>
