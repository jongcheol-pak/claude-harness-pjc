...
|---|---|---|---|
| **병렬 (기본)** | `run-hook-evals.ps1` | **약 6~15분**(898초·922초 — 531케이스 시점 / **338초** — 527케이스, 2026-08-09) | **판정 정본.** 시나리오를 13그룹으로 나눠 자식 프로세스에서 동시 실행(동시 상한 6) |
...
- **⚠ 눈에 띄게 느려지면 고아 `more.com` 페이저부터 센다** — 2026-08-19에 전체 실행이 37분에서 2시간 이상으로 늘어 「멈췄는가」를 반복 판정하는 데 시간을 썼는데, 원인은 **고아 `more.com` 54개**였다(각 누적 CPU 20,000초+). 정리 직후 러너가 곧바로 완주했다. 진단은 `Get-CimInstance Win32_Process -Filter "Name='more.com'"`로 세고 `taskkill /PID <id> /F`로 지운다. **발생원은 페이저를 붙이는 명령이며**(subagent가 도는 동안 늘었다) 근본 처방은 `docs/plans/deferred.md`에 등재돼 있다. **이 축을 먼저 보지 않으면 러너 자체를 의심하게 된다** — 실제로 그 오진에 시간을 썼다.
