...
| 저장처 | 담는 것 | 근거 |
|---|---|---|
| **`AGENTS.md`** | 위키 포인터 1줄 · `## Build & Test`(빌드/실행/테스트 **명령** + ⚠ 1줄) · **검증 매핑 표** · `## Conventions`(인코딩·줄바꿈·주석 언어) · `## DO NOT` · `## Plan Location`(경로만) · `## 데이터 접근`(명령·환경변수 **이름**만) | 검증 단계마다 실행하거나, 파일을 쓸 때마다 적용되거나, 어기면 되돌리기 어렵다. **검증 매핑 표는 여기 있어야 게이트가 켜진다** — `implement-task` V-2가 그 표의 존재를 조건으로 쓰므로 옮기면 문서 이동이 아니라 동작 변경이다 |
