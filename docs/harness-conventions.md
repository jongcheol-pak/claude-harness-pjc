...
> 위 「문서 로드 예산 기준선」이 **상한을 넘겼는지 재는** 장치라면, 이 절은 **넘기기 전에 알리고 넘겼을 때 해소하는** 경로다. 둘의 관계는 계측과 처방이다.
...
**해소는 이관이며, 절차의 정본은 `pjc:record-project-fact`의 「Step 5. 주입 상한 점검·이관」이다** — 발동 조건 · 잔류 절 4종(`## Stack`·`## Build & Test`·`## DO NOT`·`## Plan Location`, **절 단위 통째**) · 바이트 기준 대상 선정 · 이관처 결정 규칙 · 포인터 형식 · 사본과 원복 · 검증 · 사후 보고 · 이관 불가 판정이 거기 있다. **여기에 절차를 복제하지 않는다**(두 곳에 적으면 갈린다).
