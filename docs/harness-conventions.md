...
|---|---|---|---|
| `plugins/pjc/skills/implement-task/SKILL.md` | 91,950 | 612 | 75 |
| `plugins/pjc/skills/plan-feature/SKILL.md` | 72,765 | 466 | 83 |
| `plugins/pjc/skills/llm-wiki/SKILL.md` | 52,617 | 218 | 81 |
...
- **표의 케이스 수(531)는 회귀 기준선이다 — 케이스를 추가·삭제하면 이 표의 값을 함께 갱신할 것.** 이 숫자는 「문서 로드 예산 기준선」처럼 기계 대조되지 않으므로(그 표는 스킬·리뷰어 파일 바이트만 추적한다) **문서를 갱신하지 않으면 드리프트를 막을 장치가 없다.**

### 실행·대기 절차 (정본)

**쓰지 않는 방법이 셋이다** — 각각 다른 이유로 실패한다:

- **전경 실행** · **`run_in_background`** — 둘 다 **도구 시간 캡**에 걸린다. `run_in_background`가 예외가 아닌 이유는 Bash 도구의 `timeout_ms` 상한이 **전경·배경 모두 같은 10분**이어서, 백그라운드로 띄워도 그 시점에 killed되기 때문이다(병렬 기본 약 15분조차 완주 전에 죽는다).
...
**단 plan이 마지막 task에 무인자 전체 실행을 명시적으로 배치한 경우, 중간 task의 검증은 부분 실행으로 갈음할 수 있다.** 전체 스위트가 1회 **약 15분**(위 「실행 모드 4종」의 실측 — v1.159.0 병렬화 이전에는 20~30분이었다)이라 task마다 전량을 도는 것은 같은 검증의 반복이고, 그 plan에서는 최종 판정이 이미 전체 실행에 걸려 있기 때문이다(implement-task V-2의 "전체 스위트는 Phase F-2에서 1회 보장" 규정과 같은 구조).
