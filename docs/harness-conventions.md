...
>
> **그래서 이 문서(`docs/harness-conventions.md`) 자신은 이 표의 대상이 아니다 — 「임계 없음」이 아니라 「축이 다르다」다.** 이 파일은 상시 로드되지 않는다: `AGENTS.md`가 필요할 때 가리키는 **조건부 참조 문서**라, 커져도 그 회차에 그 절을 읽는 세션만 비용을 진다. 위 축이 재는 것은 *"발동만 해도 무조건 올라가는 양"*이므로 성격이 다르다. **대장 `[2026-08-22]`가 이 파일과 `SKILL.md`를 함께 지목했는데 v1.199.0 T3이 세운 상한은 `SKILL.md` 쪽에만 붙는다** — 그 항목을 반영 종결로 삭제하면서 이 절반의 판단을 남기지 않으면, 다음 회차가 "왜 빠졌는지"를 재구성할 수 없다(F-7 M2).
>
> **다만 무신호는 아니어야 한다 — 관측값을 남긴다.** v1.199.0 회차에서 이 파일은 **88,474B → 99,412B(+10,938B · +12.4%)** 로 늘었다. 상한 축이 이 파일을 보지 않으므로 **자동 경보가 없다** — 조건부 참조라 상시 비용은 아니지만, 한 절을 읽으러 들어온 세션이 마주하는 문서가 계속 두꺼워지는 것은 사실이다. 이 파일에 임계를 세울지는 **신규 정책 판단**이라 그 회차의 몫으로 남긴다.
>
