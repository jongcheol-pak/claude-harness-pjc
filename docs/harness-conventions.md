...
>
> **여유 폭은 「현행 + 10%, 1,000B 올림」이다.** 근거는 실측 증가율이다 — 2026-07-28~08-25 약 29일간 `implement-task/SKILL.md` +32,335B(하루 약 1,115B) · `plan-feature/SKILL.md` +26,915B(약 928B) · `llm-wiki/SKILL.md` +14,623B(약 487B) · `plan-reviewer.md` +24,198B(약 834B)로, 10% 여유는 활성 파일 기준 **약 6.5~12일**의 경보 창이다(증가가 느린 `pjc-systematic-debugging`은 약 45일). `AGENTS.md` 목표선이 *"여유 6,384B ≈ 약 37일"*로 근거를 남긴 것과 같은 형식이며, 그보다 짧게 잡은 이유는 이 파일들이 훨씬 빠르게 자라기 때문이다. **착수 시점에는 전 행이 통과한다** — 이 열은 감시 축이지 감축 지시가 아니고, 실제 감축은 초과가 관측되는 회차의 몫이다.
>
