...
> **`AGENTS.md`의 `## Build & Test`에서 옮겨 온 것이다** — 그 절에는 **명령과 ⚠ 1줄만** 두고(「AGENTS.md 내용 경계」의 증가 억제 규칙) 여기에 상세를 둔다. 명령을 실행할 때가 아니라 **결과를 해석할 때** 필요한 내용이기 때문이다.
>
...
**하니스 정합 셀프체크** — 열 축(문서 로드 예산 · 리뷰어 각주 앵커 · **실행 예산 수치** · 포인터 도달성 · 마커 동기 · **개념 정본** · Deferred 잔량 · **볼드 마커 짝** · **한 줄 문장 중복** · **착수 조건 동기**)을 대조한다. **exit 0 일치 / 1 불일치 / 2 앵커 파싱 실패**(2는 통과가 아니다). **열 축 중 여섯**(문서 로드 예산·리뷰어 각주·개념 정본·볼드 마커 짝·한 줄 문장 중복·착수 조건 동기)**만 이 문서에 기준표·설명 절이 있다** — 실행 예산 수치·포인터 도달성·마커 동기는 판정 규칙이 곧 코드라 그 함수 주석이 정본이고, **Deferred 잔량은 대장 자신의 「카운트 기준」 블록**이 정본이다.
...
> |---|---|---|
> | **메인** | 작업 SKILL(`implement-task` **또는** `plan-feature`) **+ `llm-wiki`** — 절차 K(계획·구현 중 read-only 참조)·절차 M(F-6.5 큐 소비)로 **자동 호출**되므로 코드 작업 세션에서 함께 로드된다 | **최대 163,263B** (104,530 + 58,733 — 아래 표의 실측 합). 한 세션에서 계획 → 구현을 연달아 하면 `plan-feature` 92,921B가 더해진다. **이 세 수치는 아래 표에서 옮겨 적은 것이라 기계 대조 대상이 아니다** — 표를 갱신하는 task가 이 줄도 함께 고친다(v1.186.0 전까지 실제로 어긋나 있었다) |
> | **메인에 상시 추가** | `AGENTS.md` — SessionStart가 **매 세션 주입**하므로 위 「메인」 조합에 **항상 더해진다**. 단 상한을 넘기면 전문 대신 **목차만** 들어가 그 값이 크게 준다 — 그때는 명령·규약이 통째로 빠진 상태이므로 바이트가 줄었다고 좋은 것이 아니다. **이 수치도 위 「메인」과 같이 아래 표에서 옮겨 적은 것이라 기계 대조 대상이 아니다** — `AGENTS.md` 예산 행을 고치는 task가 이 줄도 함께 갱신한다 | **+8,056B** |
...
|---|---|---|---|
| `AGENTS.md` | 8,056 | 93 | 0 |
| `plugins/pjc/skills/implement-task/SKILL.md` | 104,530 | 632 | 66 |
| `plugins/pjc/skills/plan-feature/SKILL.md` | 92,921 | 519 | 83 |
...
> 위 「문서 로드 예산 기준선」이 **얼마나 큰지**를 재고 아래 「주입 상한 관리」가 **넘쳤을 때**를 다룬다면, 이 절은 **애초에 무엇이 들어가는가**를 정한다. 셋은 계측 → 경계 → 처방의 한 벌이며, 경계가 없으면 나머지 둘은 매번 같은 자리로 돌아온다(실측: `AGENTS.md`가 `2026-08-05 16,249B → 08-06 13,965B → 08-20 16,348B → 08-21 14,783B → 08-22 13,192B` — 하루 평균 +170B로 자라 2주에 이관 3회).

**판정 기준은 하나다 — `AGENTS.md`는 매 세션 자동 주입되고 위키는 필요할 때 읽는다.** 그래서 *"매 세션 예외 없이 필요한가 · 틀리면 즉시 사고나는가"* 가 갈림선이다.

| 저장처 | 담는 것 | 근거 |
|---|---|---|
| **`AGENTS.md`** | 위키 포인터 1줄 · `## Build & Test`(빌드/실행/테스트 **명령** + ⚠ 1줄) · **검증 매핑 표** · `## Conventions`(인코딩·줄바꿈·주석 언어 + **아키텍처 선언 1줄** — "어디에 코드를 둘지") · `## DO NOT` · `## Plan Location`(경로만) · `## 데이터 접근`(명령·환경변수 **이름**만) · `## 산출물·파일 관리`(산출물·캐시 **경로**만) | 검증 단계마다 실행하거나, 파일을 쓸 때마다 적용되거나, 어기면 되돌리기 어렵다. **검증 매핑 표는 여기 있어야 게이트가 켜진다** — `implement-task` V-2가 그 표의 존재를 조건으로 쓰므로 옮기면 문서 이동이 아니라 동작 변경이다 |
| **이 문서**(레포 상세) | 각 검증 명령이 **무엇을 대조하는가** · **함정** · 축의 설명 | 명령을 실행할 때가 아니라 결과를 해석할 때 필요하다 |
...
**증가 억제 규칙.** `## Build & Test`에는 **명령과 ⚠ 경고 1줄만** 둔다 — *"그 명령이 무엇을 대조하는가"*·*"어떤 함정이 있는가"* 의 상세는 이 문서로 보내고 포인터만 남긴다. 검증 축이 늘 때마다 이 절이 함께 자라는 것이 위 증가율의 정체다.

**서술 밀도 규칙.** `AGENTS.md`의 독자는 사용자가 아니라 **다음 세션의 에이전트 자신**이다. 설득·경위·근거 서술을 쓰지 않고 **명령·값·규칙·⚠ 1줄**만 적는다. *"왜 그런가"* 가 필요하면 이 문서나 위키로 보내고 포인터를 남긴다.

**이 표는 여기가 정본이다** — `bootstrap-agents-md`·`record-project-fact`·`llm-wiki`·`plan-feature`는 이 절을 가리키기만 하고 각자 재서술하지 않는다.

## AGENTS.md 주입 상한 관리 (임박 경고 · 이관)

> 위 「문서 로드 예산 기준선」이 **상한을 넘겼는지 재는** 장치라면, 이 절은 **넘기기 전에 알리고 넘겼을 때 해소하는** 경로다. 둘의 관계는 계측과 처방이다. **무엇을 담는가는 위 「AGENTS.md 내용 경계」가 정본**이다.
...
**해소는 이관이며, 절차의 정본은 `pjc:record-project-fact`의 「Step 5. 주입 상한 점검·이관」이다** — 발동 조건 · **잔류 절**(위 「AGENTS.md 내용 경계」 표의 `AGENTS.md` 행이 정본 — 이관해도 남는 절, **절 단위 통째**) · 바이트 기준 대상 선정 · 이관처 결정 규칙 · 포인터 형식 · 사본과 원복 · 검증 · 사후 보고 · 이관 불가 판정이 거기 있다. **여기에 절차를 복제하지 않는다**(두 곳에 적으면 갈린다).
...
- **현행 케이스 수 665가 회귀 기준선이다 — 케이스를 추가·삭제하면 이 값을 함께 갱신할 것.** (2026-08-15 무인자 전체 실행 실측 `535/535 OK (FAIL 0)`. 직전 기록값 528은 그 사이 늘어난 케이스가 반영되지 않아 낡은 값이었다 — `docs/plans/deferred.md`가 535로 적고 있었던 것이 맞다.) **(2026-08-18 재실측 `539/539 OK (FAIL 0)` — v1.180.0이 SessionStart `fork` matcher 케이스 4건(SC2b·SC2c·SC22b·SC22c)을 더했다.)** **(2026-08-19 재실측 `596/596 OK (FAIL 0)` — v1.182.0이 57건을 더했다: secret-patterns 라벨 판정 24건 · block-destructive 8건 · `.state` TTL 2건 · `$autoStage` 호출 횟수 8건 · 폴백 경고 가드 3건 · 그 밖 회귀 케이스. 그중 **오차단을 고정하는 델타 음성이 다수**다 — 그 자리가 비어 있던 것이 F-7 BLOCKER 2회의 원인이었다.)** **(2026-08-21 재실측 `643/643 OK (FAIL 0)` — 자기보호 이름 집합에 고아 프로세스 회수 계열 2종을 합류시키며 8건을 더했다: protect-harness 양성 3(리터럴 2 + 8.3 마스킹 1) · **델타 음성 3**(개발 repo · 유사 이름 · 캐시 밖 CLAUDE~1) · post-write H2 탐지 양성 2.)** **(2026-08-21 재실측 `647/647 OK (FAIL 0)` — v1.190.0 T1이 `session-context`의 AGENTS.md 주입 상한 임박 경고 케이스 4건(SC9a 비율 축 양성 · SC9b 전문 주입 유지 · SC9c 음성 · SC9d 초과 폴백 문구)을 더했다. 이 줄은 T1에서 함께 갱신됐어야 했는데 누락돼 F-2 재실행에서 643 ↔ 647 드리프트로 드러났다 — 위 v1.182.0 선례와 같은 자리다.)** **(2026-08-23 재실측 `665/665 OK (FAIL 0)` — `guard-agents-content` hook 신설로 시나리오 1개·케이스 18건을 더했다: 차단 양성 5(인계 서술 3 · 트리 2) · **델타 음성 6**(정상 AGENTS.md · plan.md 같은 문구 · 본문 산문 스침 · 명령 파이프 · 마크다운 표 · basename 불일치) · 우회 1 · protect-harness 공존 3 · fail-open 2 · basename 판정으로 하위 경로 AGENTS.md 차단 1.)** 이 숫자는 「문서 로드 예산 기준선」처럼 기계 대조되지 않으므로(그 표는 스킬·리뷰어 파일 바이트만 추적한다) **문서를 갱신하지 않으면 드리프트를 막을 장치가 없다** — v1.182.0이 실제로 이 줄을 지나쳐 F-7 2R에 잡혔다.
...
- **무엇을 대조하는가**: 착수 조건 **수치 4종**(잔량 임계·신규 등재분·날짜·절대 상한)과 **판정일 도출 문장** 하나. 후자를 함께 보는 이유는 **v1.188.0이 고친 결함이 수치가 아니라 그 문장에 있었기 때문**이다(부기 형식 미인식) — 수치만 대조하면 그 문장이 한쪽에서만 지워져도 통과한다.
