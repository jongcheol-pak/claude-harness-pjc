...
|---|---|---|---|
| `plugins/pjc/skills/implement-task/SKILL.md` | 98,747 | 621 | 67 |
...
|---|---|---|---|
| **병렬 (기본)** | `run-hook-evals.ps1` | **약 6~15분**(898초·922초 — 531케이스 시점 / **338초** — 527케이스, 2026-08-09 / **약 37분** — 596케이스, 2026-08-19 서술 환산·초 단위 실측 아님·고아 페이저 오염 여부 미상) | **판정 정본.** 시나리오를 13그룹으로 나눠 자식 프로세스에서 동시 실행(동시 상한 6) |
...
- **현행 케이스 수 596이 회귀 기준선이다 — 케이스를 추가·삭제하면 이 값을 함께 갱신할 것.** (2026-08-15 무인자 전체 실행 실측 `535/535 OK (FAIL 0)`. 직전 기록값 528은 그 사이 늘어난 케이스가 반영되지 않아 낡은 값이었다 — `docs/plans/deferred.md`가 535로 적고 있었던 것이 맞다.) **(2026-08-18 재실측 `539/539 OK (FAIL 0)` — v1.180.0이 SessionStart `fork` matcher 케이스 4건(SC2b·SC2c·SC22b·SC22c)을 더했다.)** **(2026-08-19 재실측 `596/596 OK (FAIL 0)` — v1.182.0이 57건을 더했다: secret-patterns 라벨 판정 24건 · block-destructive 8건 · `.state` TTL 2건 · `$autoStage` 호출 횟수 8건 · 폴백 경고 가드 3건 · 그 밖 회귀 케이스. 그중 **오차단을 고정하는 델타 음성이 다수**다 — 그 자리가 비어 있던 것이 F-7 BLOCKER 2회의 원인이었다.)** 이 숫자는 「문서 로드 예산 기준선」처럼 기계 대조되지 않으므로(그 표는 스킬·리뷰어 파일 바이트만 추적한다) **문서를 갱신하지 않으면 드리프트를 막을 장치가 없다** — v1.182.0이 실제로 이 줄을 지나쳐 F-7 2R에 잡혔다.
