...
- **⚠ 앵커 없는 5번째 복제가 있다 (검사 대상 아님)**: 리뷰어 4종의 `## 출력 형식` 절 첫 문단(**판정문 출력 의무** — 조사 서술만으로 끝내지 말 것)이 **같은 4파일에 같은 문면으로 복제돼 있으나 각주가 없다**(v1.173.0 신설). 따라서 **위 24지점은 전수가 아니다** — 그 문단은 한 파일만 고쳐도 검사기가 침묵한다. 앵커를 붙이려면 기대값을 24 → 28로 함께 올려야 하며, 그 판단은 다음에 리뷰어 4종을 손볼 때 한다.
- 기대값은 **4파일 × 6블록 = 24지점**이며, 파일당 앵커 6회다.

### 블록별 문면 판정 (24지점 실측 — 2026-08-06)

> **왜 판정했나**: 위 「닫지 못하는 것」이 남긴 물음(문면 차이가 역할 차이인가 드리프트인가)에 답하지 않으면 **본문 축 검사·각주 동기 토큰을 넣을지 결정할 수 없다.** 4파일 × 6블록을 블록 단위로 추출해 diff로 대조한 결과가 아래다. **판정 결과: 드리프트 0건 — 본문 축 검사와 동기 토큰은 도입하지 않는다.**
