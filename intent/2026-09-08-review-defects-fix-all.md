...
## Constraints
안전 임계 hook 변경은 「오탐 수정·미탐 보완」 두 종류만이며 델타 음성 케이스 실증이 필수다. 게이트 등급 문서 예산(SKILL 12,000 B 등)을 넘지 않는다. 워킹트리 CRLF 를 유지한다. description 수정은 회차 ③에 몰아 트리거 eval 을 스킬당 1회만 돌린다. 인터뷰 확정: 완료 커밋 제목은 `{유형}: T<N> — …` 로 통일 · F-6.5 자동 소비는 복원하지 않고 잔존 참조를 삭제 · 디버깅 스킬은 원인 규명까지 맡고 비trivial 수정은 `pjc:plan`→`pjc:implement` 로 넘김 · 계획 단계 위키 조회 정본은 `WIKI.md` 하나이고 절차 K 는 디버깅 세션 전용으로 좁힘. 회차 ② 착수 인터뷰 확정: project 허브는 `RELOCATE_TYPES` 에서 **제외**하고 (하위 `type` 재지정을 쓰지 않는다 — 하위가 `type: project` 면 허브 검사 5종이 그것을 별개 프로젝트로 잡는다) · 이관처 배제에 vault 상대경로·절대·`..` 에 더해 추적되지 않는 작업 문서(`plan.md`·`notes.md`)를 포함한다. 회차 ③ 착수 인터뷰 확정: 절차 M 의 **「코드 세션 자동 경로」 문단을 통째로 삭제**한다(호출자가 실재하지 않는다 — `implement/SKILL.md` 「완료 시 위키 기록」은 `WIKI.md` 만 가리키고 그 §3 은 `pending.md` 1줄 append 만 규정한다) · `plan-neg-7` 은 **케이스를 본문 「종류 기준」에 맞춰** 질의를 1종류·여러 곳으로 바꾸고 description 에 「종류」를 명시한다 · `DESIGN.md` 13,355 B 초과는 **이번 회차에 항목을 빼서 감량**한다(통지 등급이지만 이번 intent 가 대장 등재를 배제한다).
