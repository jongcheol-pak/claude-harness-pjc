...
## Problem
원문 요청: *"1·3항이랑 중간 업데이트 문구까지 계획 세워줘"* — 앞선 검토(`prompting-claude-opus-5-5` 가이드 대조)의 1항(정지 형태 누락)·3항(DESIGN.md 세대 표기)과 「중간 업데이트」 문구 조정이다. Opus 5.5 가이드가 조기 정지를 네 형태로 들었는데 `loop-stop-patterns.md` 에는 「나머지를 막지 않는 결정 목록으로 정지」가 없고, `implement` 의 「중간 업데이트를 내지 않는다」는 5.5 의 기본 동작·Claude Code 의 「몇 마디로 말하고 계속하라」 리마인더와 문면상 부딪친다. 실사용 모델이 5.5 로 바뀐 지금 맞춰 둔다.
