...
**「완전 배제」는 링크 대상 우주(`existing`)도 줄인다** — `main()` 에서 `md` 목록이 곧 `existing` 의 원천이다. 다만 **지금 이 vault 에서는 관측 가능한 차이가 없다**: gemma-wiki 를 가리키는 인바운드 링크는 Karina 답 노트 1파일의 `sources:` 3개뿐인데 전부 `/` 없는 bare 형태라 `lint.py:3024-3028` 의 ERR 분기에 닿지 않고 언제나 WARN 으로 떨어진다(계획 초안이 이것을 「ERR 이 된다」로 잘못 적었다가 plan 리뷰에서 바로잡았다). 아웃바운드도 전부 `gemma-wiki/` 안을 가리켜 실제 페이지가 고아로 오탐되지 않는다. 둘이 갈리는 것은 앞으로 **경로형**(`[[gemma-wiki/cards/x]]`) 참조가 생길 때뿐이고, 생성물을 참조하지 못하게 하는 쪽이 이 결정의 취지다.
