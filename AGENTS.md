...
- **하니스 정합 셀프체크 (`plugins/pjc/evals/**`·예산 표·리뷰어 각주·`deferred.md` 수정 시 필수)**:
  ...
  ```
  여섯 축(문서 로드 예산 · 리뷰어 각주 앵커 · **실행 예산 수치** · 포인터 도달성 · 마커 동기 · Deferred 잔량)을 **문서 기록값 ↔ 실측**으로 대조한다. **exit 0 일치 / 1 불일치 / 2 앵커 파싱 실패**(2는 통과가 아니다). 축별 기준표는 `docs/harness-conventions.md`.
