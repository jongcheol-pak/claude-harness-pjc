...
- **`block-destructive.ps1`·`protect-harness.ps1`의 차단 동작 변경** — 안전 임계 hook(끌 수 없음, 마지막 방어선). **단 두 종류는 사용자 승인 선례로 허용된다**: ① **오탐 수정** — 골든 회귀(신규 통과 케이스 + 수정 전 차단 음성 대조)로 실증하는 조건 ② **미탐 보완**(차단 범위 확대) — 같은 조건 + **새 경계가 실제로 발화하는 「델타 음성」 케이스로 오차단 0을 반드시 실증**할 것(통과만 확인하는 무회귀 케이스는 근거가 못 된다).
