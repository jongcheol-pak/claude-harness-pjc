...
- **hook 출력 규약**: 경고는 `exit 0` 비차단 + stderr + additionalContext. 차단은 **`exit 2`** 하나이고 그것을 내는 hook은 넷이다 — `block-destructive`(파괴적 명령) · `guard-bash`(task 체크박스 게이트 + 커밋 시크릿(조건부)) · `guard-write`(plan 게이트) · `guard-harness`(자기보호 + AGENTS.md 신규·내용 경계). **커밋 시크릿의 조건부 세부·스캔 범위는 `docs/harness-conventions.md`가 정본** — hook 수정 전 반드시 읽을 것. **우회 변수는 둘이며 서로 대체되지 않는다**: `CLAUDE_HARNESS_QUICK`(`guard-write`·`guard-harness`의 AGENTS.md 게이트·`guard-bash`의 체크박스 게이트) / `CLAUDE_HARNESS_ALLOW_SECRET`(커밋 시크릿 차단 전용).
...
- **PRD는 쓰지 않는다** — 요구는 `plan.md`의 `## 요구 이해`, 결정 이력은 위키 `decisions.md`, 미착수 항목은 `docs/plans/deferred.md`가 담는다. `docs/prds/` 3파일은 회차 3에서 이관·폐기 판정한다.
