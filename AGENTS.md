...
- **병행 세션의 커밋은 `git commit -- <경로…>` 다** — `git add <경로>` 로 한정해도 소용없다. add 는 **인덱스에 더하는** 것이고 commit 은 **인덱스 전체**를 담아, 상대가 이미 staged 해 둔 것이 함께 들어간다. `-A` 를 안 써도 일어난다(회차 54 `199e1e0a` · 회차 55 `d3fad946` 두 번 관측 — 두 번째는 경로 한정 규약을 지킨 상태였다). **단 `commit -- <경로>` 는 인덱스가 아니라 그 경로의 워킹트리 내용을 담으므로 부분 스테이징(`git add -p`)과 함께 쓰지 않는다** — add 뒤에 더 고친 내용이 조용히 들어간다.
