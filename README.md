...
- **테스트 범위 비례**: 프로젝트 가이드(AGENTS.md)에 영향 범위 필터 명령이나 검증 매핑 표가 있으면 각 작업에서는 **영향받는 부분만** 테스트하고, **전체 테스트는 마지막 통합 검증에서 1회** 보장합니다(축소 시 커밋에 범위를 명시). 근거가 없으면 스위트 크기와 무관하게 전체를 돕니다.
