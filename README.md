...
- **같은 지적이 두 번 반복되면 진단만 하는 검토자를 새로 부름**: 같은 문제를 두 번 고쳤는데도 남아 있다면, 세 번째 수정도 **이미 두 번 빗나간 판단을 그대로 물려받은 채** 하게 됩니다. 그래서 중단 직전에 앞선 시도를 물려받지 않은 검토자를 한 번 불러 **원인만** 진단받고(수정은 하지 않습니다 — 도구 수준에서 차단), 그 진단을 근거로 다시 고칩니다. 이 단계는 중단 기준을 늦추지 않습니다(작업당 1회, 기존 3회 기준은 그대로).
