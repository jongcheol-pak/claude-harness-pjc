...
- **자율 루프가 중간에 멈추면 되돌림** — 계획서에 아직 남은 작업이 있는데 Claude가 아래 넷 중 하나로 대화를 끝내면, 종료를 막고 다음 작업을 시작하게 합니다. ① *"이어서 진행하겠습니다"* 같은 **예고만 남기기**(질문이 아니라 평서문이라 겉보기엔 "묻지 않고 진행하는" 것처럼 보이지만 결과는 똑같은 정지) ② *"새 세션으로 옮길까요?"* 처럼 **컨텍스트를 이유로 세션 전환 제안**(맥락이 통째로 버려집니다 — 정답은 압축하고 계속하는 것) ③ *"여기서 한번 직접 실행해 보시겠어요?"* 처럼 **기계로 확인되는 검증을 사용자에게 넘기기** ④ *"T3 완료. 변경 파일 3개, 빌드 통과."* 처럼 **진행 상황만 말하고 끝내기**(묻지도 예고하지도 않아 규칙을 어긴 티가 안 나지만, 말로 끝내면 거기서 멈추는 것은 같습니다 — 진행 상세는 대화가 아니라 계획서에 적어야 합니다). **당신이 "그만"·"여기까지"·"T3만"·"새 세션으로" 처럼 멈추거나 옮기라고 했거나, 방금 무언가를 물어봐서 Claude가 그 답을 한 경우, 또는 Claude가 중단 보고·완료 보고·확인 대기·승인 요청처럼 규칙이 정한 지점에서 멈춘 경우에는 발동하지 않습니다** — 그런 지점은 제목에 전용 표시(⛔ 중단 · 🎉 완료 · ⏸️ 확인 대기)가 붙어 있어 구분됩니다. 판단이 애매하면 막지 않는 쪽이며, 같은 세션에서 3회를 넘겨 되돌리지도 않습니다(세션을 끝낼 수 없게 되는 상황 방지)
