...
- 위키 폴더가 git으로 관리되지 않으면, Claude가 기존 파일을 수정·삭제하기 전에 사본을 위키 안 백업 폴더(`90_archive/backup/{날짜}/`)에 남깁니다 (30일 지난 백업은 자동 정리). 잘못 갱신됐을 때 "위키 되돌려줘"라고 요청하면 이 백업으로 복구합니다 (차이를 보여주고 확인 후 되돌림). git 관리 중인 폴더면 git이 이력을 보존하므로 백업·복구 모두 git으로 대신합니다.
