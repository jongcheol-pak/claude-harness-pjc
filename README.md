...
- 검토는 최대 2라운드입니다. **그 안에 수렴하지 못한 계획도 승인 화면에서 구분됩니다** — 지적이 남은 채 끝났는지, 지적은 다 고쳤지만 그 고친 결과를 검토 담당이 다시 보지는 못했는지가 각각 한 줄로 표시됩니다. 라운드를 더 도는 것이 답이 아니라는 것은 실측으로 확인됐습니다(3라운드·5라운드를 돌려도 닫히지 않았고, 그 구간의 수정이 새 결함을 만들었습니다) — 그래서 **수렴하지 못했다는 사실 자체를 보이게** 합니다
