...
- **자율 루프가 중간에 멈추면 되돌림** — 계획서에 아직 남은 작업이 있는데 Claude가 *"이어서 진행하겠습니다"* 같은 **예고만 남기고 대화를 끝내면**, 종료를 막고 다음 작업을 시작하게 합니다. 질문이 아니라 평서문이라 겉보기엔 "묻지 않고 진행하는" 것처럼 보이지만 결과는 똑같은 정지이기 때문입니다. **당신이 "그만"·"여기까지"·"T3만" 처럼 멈추라고 했거나, Claude가 중단 보고·완료 보고·확인 요청을 한 경우에는 발동하지 않습니다** — 판단이 애매하면 막지 않는 쪽이며, 같은 세션에서 3회를 넘겨 되돌리지도 않습니다(세션을 끝낼 수 없게 되는 상황 방지)
