...
> 기능 문서마다 "관련 파일" 지도(그 기능을 구성하는 파일 목록)를 기록해, 기능 이름으로 검색하면 관련 파일에 바로 도달합니다. 코드 작업 중 위키와 코드가 어긋난 걸 발견하면 위키의 대기 목록(pending)에 남겨 다음 위키 갱신 때 자동으로 챙기고, 위키 점검(lint)은 파일 경로 실존·시크릿(비밀번호·API 키) 유입까지 기계적으로 검사합니다.
