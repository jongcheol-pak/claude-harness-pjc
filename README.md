...
- 위키 폴더가 git으로 관리되지 않으면, Claude가 기존 파일을 수정·삭제하기 전에 사본을 위키 안 백업 폴더(`90_archive/backup/{날짜}/`)에 남깁니다. **git으로 관리되는 위키에서는 백업을 쌓아두지 않습니다** — git 이력이 그 역할을 하므로, 점검 도구가 만든 백업 폴더는 다음 실행 때 정리됩니다 (30일 지난 백업은 자동 정리 — 단 **삭제된 프로젝트 백업은 유일한 사본이라 보존**합니다). 잘못 갱신됐을 때 "위키 되돌려줘"라고 요청하면 이 백업으로 복구합니다 (차이를 보여주고 확인 후 되돌림 — 복구 직전 상태도 별도로 백업해 복구 자체를 되돌릴 수 있습니다). git 관리 중인 폴더면 git이 이력을 보존하므로 백업·복구 모두 git으로 대신합니다 — 다만 **커밋이 하나도 없으면 git 복구가 불가**하므로, git 관리 위키는 주기적으로 커밋하시길 권합니다.
