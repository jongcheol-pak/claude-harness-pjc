...
- 변경이 영향을 주는 곳 자동 경고
- **자율 루프가 중간에 멈추는 것은 이제 스킬 문면이 막습니다** — v1.225.0에서 그 판정을 기계가 하지 않습니다. 정지 의도는 어휘가 아니라 「도구 호출 없이 말만 하고 turn을 끝냈다」는 사실에 있는데, 한국어 어미 조합이 무한해 정규식이 수렴하지 않았기 때문입니다. 멈추는 네 형태와 그 예시는 `plugins/pjc/skills/implement/references/loop-stop-patterns.md`에 있습니다.
...
쌓인 로그는 그 jsonl 을 직접 읽어 집계합니다 — Claude에게 "hook 이벤트 로그 집계해줘"라고 하면 됩니다(전용 도구는 v1.225.0에서 제거했습니다).
