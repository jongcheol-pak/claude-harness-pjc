...
- 계획 없이 코드 쓰는 것 방지
- AGENTS.md(프로젝트 가이드) 신규 생성을 전용 스킬 경유로 유도 (사용자 승인 없이 직접 만들려 하면 차단)
...
pjc는 **특정 언어 전용이 아닙니다.** .NET·Android·Node/TS·Python·Go·Rust는 표식 파일(`*.csproj`·`package.json`·`go.mod` 등)로 감지해 빌드·테스트 명령까지 자동으로 채우고, 그 외 모든 언어(Flutter·Swift·Java·C++·Ruby·Elixir 등)는 알려진 추측치를 「(추측)」 표시와 함께 제시하거나 네 가지만 물어서 채웁니다. pjc의 계획·검증·자율 루프는 언어를 가리지 않습니다.
