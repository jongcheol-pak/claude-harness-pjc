...
- **등록 안 된 프로젝트의 지식**: 위키에 없는 프로젝트의 결정·작업 사실은 대기 목록에 쌓지 않고 **그 저장소의 `AGENTS.md`에 기록**하도록 안내합니다(소비할 주체가 없는 목록은 쌓이기만 하기 때문입니다). 물을 수 없는 자동 실행 중에는 종전대로 대기 목록에 넣되 저장소 경로를 함께 적어, 프로젝트 이름이 바뀌어도 대상을 잃지 않습니다.
