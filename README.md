...
처음 코드 작업을 시도하면, pjc가 **작업에 필요한 명령·규칙**(빌드·테스트 명령, 인코딩 규약, 금지 항목)을 담은 `AGENTS.md` 파일이 없다며 생성을 제안합니다. **빌드·테스트 명령만 채우고 나머지 절은 비워 만듭니다** — 그 명령이 없으면 검증이 성립하지 않아 즉시 필요하고, 나머지는 작업하며 확인되는 대로 채워집니다(추측해 채우지 않습니다).
