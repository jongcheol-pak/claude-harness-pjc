...
**Claude Code**: 최소 v2.0 · **권장 v2.1.219+** — 완료 검토자(`completion-reviewer`)가 쓰는 `opus` 별칭이 **Claude Opus 5**로 해소되는 버전이다. 그 미만에서도 동작하지만 두 리뷰어가 이전 세대 Opus로 실행된다(나머지 리뷰어는 sonnet·haiku 지정이라 무관). 상세는 [호환 환경](#호환-환경).
