...
> **`AGENTS.md`에는 프로젝트 정보를 넣지 않습니다.** 프로젝트 성격·기술 스택·디렉터리 구조·아키텍처는 **위키가 정본**이고 `AGENTS.md`에는 위키로 가는 한 줄만 둡니다. 이 파일은 매 세션 전문이 주입되므로, 코드를 읽으면 알 수 있는 것(구조)이나 계속 쌓이는 것(진행 상태)을 담으면 곧 주입 한도를 넘겨 **그 세션이 파일을 통째로 못 보게 됩니다**. 위키에 등록되지 않은 프로젝트는 계획을 세울 때 자동으로 등록합니다.
