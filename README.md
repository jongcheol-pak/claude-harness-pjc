...
- **원인을 아는 재시도에는 멈추지 않음**: 자율 루프는 계획이 어긋나거나 같은 지적이 쌓일 때 멈추는데, **원인이 규명됐고 같은 수단을 반복하지 않는 경우**에는 그 시점에 멈추지 않고 계속합니다(기능·계약이 바뀌거나 안전에 닿으면 종전대로 멈춥니다). 다만 **재시도 횟수는 그대로 세므로** 상한(무한 그라인딩 10회 등)은 살아 있고, 검토자 판정이 두 번 상충하거나 원인을 모른 채 5회 실패하면 판정 없이 멈춥니다.
...
- **검토자는 읽기만 합니다**: 아래 「모델 라우팅」 표의 검토자 7종 전부가 파일을 고칠 수 없게 정의돼 있고(`Write`·`Edit`·`NotebookEdit` 제거), 지적을 반영하는 것은 Claude 본체입니다. 그래서 검토자가 파일을 고치려 하면 쓰기 오류가 나는데 **그것이 설계된 동작**입니다 — 검토와 수정의 주체를 나눠야 "자기가 쓴 것을 자기가 통과시키는" 경로가 막힙니다.
- **계획 질문의 추천(★)은 질문당 하나, 근본 해결에**: 쉽게 끝나는 선택지가 아니라 문제를 근본적으로 해결하는 선택지를 추천하고, **★은 질문당 정확히 하나**입니다 — 선택지 여럿이 다 근본이면 그중 최선 하나에만 붙이고 나머지를 고르지 않은 이유를 적습니다(전부에 붙이면 추천이 아무것도 가리키지 않습니다). **각 선택지에 〔근본〕/〔우회〕 판정이 함께 표시**되며, 복잡도·작업량 차이는 트레이드오프로 함께 표시합니다.
...
- **하네스 레포에서만 — 미뤄 둔 일감(Deferred 대장)의 잔량과 「가장 오래 방치된 항목」을 1줄로 알림**. 다음에 하기로 미룬 항목들은 계속 쌓이는데, 그 대장을 언제 한 번 비울지를 정하는 기준 중 하나가 **가장 오래 판정을 안 받은 항목이 며칠 됐는가**입니다. 그 날짜를 사람이 손으로 세다가 같은 오산이 두 번 났습니다(항목마다 재확인 기록이 여러 개인데 그중 **가장 최근 것**을 봐야 하는 걸 놓쳤습니다) — 이제 기계가 세어 세션 시작에 알려 줍니다. 값을 재는 규칙은 대장 규약이 정본이고, 규칙과 다르게 처리하는 부분(릴리즈 번호로만 적힌 기록)이 있으면 **그 건수도 함께 표시**해 그 항목만 사람이 확인하게 합니다.
